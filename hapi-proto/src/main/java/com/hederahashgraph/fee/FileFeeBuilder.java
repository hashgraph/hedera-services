...
public class FileFeeBuilder extends FeeBuilder {
	...
	public FeeData getFileCreateTxFeeMatrices(TransactionBody txBody, SigValueObj sigValObj)
		...
		rbs = (getBaseTransactionRecordSize(
				txBody) + BASIC_ENTITY_ID_SIZE/* Added for File ID size*/) * RECIEPT_STORAGE_TIME_SEC;
	...
	private long getFileCreateStorageBytesSec(TransactionBody txBody) {
		long storageSize = getFileCreateTxSize(
				txBody) + LONG_SIZE + BOOL_SIZE; // add Expiration Time and Deleted flag space
	...
	public FeeData getFileDeleteTxFeeMatrices(TransactionBody txBody,
			SigValueObj sigValObj) throws InvalidTxBodyException {
