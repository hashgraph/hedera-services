...
class FeeSchedules {
  ...
  Map<ResourceProvider, Map<UsableResource, Long>> canonicalPricesFor(
      HederaFunctionality function, SubType type) throws IOException {
  ...
  private Map<ResourceProvider, Map<UsableResource, BigDecimal>> genericPricesFor(
      HederaFunctionality function) throws IOException {
    ...
    for (var provider : ResourceProvider.class.getEnumConstants()) {
      ...
      providerGenerics.put(
          CONSTANT, nonConstantGenerics.multiply(constW).divide(oneMinusConstW, DECIMAL128));
