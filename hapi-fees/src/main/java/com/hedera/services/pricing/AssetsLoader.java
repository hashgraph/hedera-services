...
class AssetsLoader {
  ...
  Map<HederaFunctionality, BigDecimal> loadConstWeights() throws IOException {
    try (var fin =
        AssetsLoader.class.getClassLoader().getResourceAsStream(CONSTANT_WEIGHTS_RESOURCE)) {
  ...
  /**
   * Loads a map that, for each resource type available in the network, gives the "capacity" of that
   * resource. These capacities do not have an absolute meaning, and are just compared to infer the
   * relative scarcity of each resource.
   *
  ...
  Map<HederaFunctionality, Map<SubType, BigDecimal>> loadCanonicalPrices() throws IOException {
    try (var fin =
        AssetsLoader.class.getClassLoader().getResourceAsStream(CANONICAL_PRICES_RESOURCE)) {
      final var om = new ObjectMapper();
