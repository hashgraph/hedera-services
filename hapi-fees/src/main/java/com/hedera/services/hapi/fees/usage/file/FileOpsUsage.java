...
import static com.hedera.services.hapi.utils.fee.FeeBuilder.BASE_FILEINFO_SIZE;
...
import static com.hedera.services.hapi.utils.fee.FeeBuilder.getAccountKeyStorageSize;

import com.hedera.services.hapi.fees.usage.*;
...
public class FileOpsUsage {
    ...
    public FeeData fileCreateUsage(TransactionBody fileCreation, SigUsage sigUsage) {
        ...
        customBytes +=
                TxnUsage.keySizeIfPresent(
                        op, FileCreateTransactionBody::hasKeys, body -> asKey(body.getKeys()));

        var lifetime =
                SingletonEstimatorUtils.ESTIMATOR_UTILS.relativeLifetime(fileCreation, op.getExpirationTime().getSeconds());

        var estimate = txnEstimateFactory.get(sigUsage, fileCreation, SingletonEstimatorUtils.ESTIMATOR_UTILS);
        ...
        estimate.addNetworkRbs(BASIC_ENTITY_ID_SIZE * SingletonUsageProperties.USAGE_PROPERTIES.legacyReceiptStorageSecs());
    ...
    public FeeData fileUpdateUsage(
            TransactionBody fileUpdate, SigUsage sigUsage, ExtantFileContext ctx) {
        ...
        var estimate = txnEstimateFactory.get(sigUsage, fileUpdate, SingletonEstimatorUtils.ESTIMATOR_UTILS);
        ...
        long newLifetime =
                SingletonEstimatorUtils.ESTIMATOR_UTILS.relativeLifetime(fileUpdate, op.getExpirationTime().getSeconds());
        long sbsDelta =
                SingletonEstimatorUtils.ESTIMATOR_UTILS.changeInBsUsage(
