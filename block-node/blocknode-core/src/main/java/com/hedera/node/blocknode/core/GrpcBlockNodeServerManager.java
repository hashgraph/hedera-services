 ...
 *
 * <p>Our node will run at least one gRPC server on the port specified in the {@link BlockNodeGrpcConfig}. It will also attempt
 * to run a gRPC server on the tls-port specified in the same config. If it fails to start the tls-port, it will log
 * a warning and continue.
 */
public interface GrpcBlockNodeServerManager {
    /**
