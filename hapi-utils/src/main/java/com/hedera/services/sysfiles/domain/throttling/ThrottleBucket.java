...
public class ThrottleBucket {
  ...
  public static ThrottleBucket fromProto(com.hederahashgraph.api.proto.java.ThrottleBucket bucket) {
    ...
    pojo.throttleGroups.addAll(
        bucket.getThrottleGroupsList().stream().map(ThrottleGroup::fromProto).collect(toList()));
    return pojo;
  }
  ...
  public com.hederahashgraph.api.proto.java.ThrottleBucket toProto() {
    return com.hederahashgraph.api.proto.java.ThrottleBucket.newBuilder()
        ...
        .addAllThrottleGroups(throttleGroups.stream().map(ThrottleGroup::toProto).collect(toList()))
        .build();
  }
  ...
  /**
   * Returns a deterministic throttle scoped to 1/nth of the nominal milliOpsPerSec in each throttle
   * group; and a list that maps each relevant {@code HederaFunctionality} to the number of logical
   * operations it requires from the throttle.
   *
  ...
  private DeterministicThrottle throttleFor(long mtps, int n) {
    try {
      return DeterministicThrottle.withMtpsAndBurstPeriodMsNamed(
          mtps / n, impliedBurstPeriodMs(), name);
