...
public class HapiThrottleUtils {
    public static ThrottleBucket<HederaFunctionality> hapiBucketFromProto(
        ...
        return new ThrottleBucket<>(
                ...
                bucket.getName(),
                bucket.getThrottleGroupsList().stream().map(HapiThrottleUtils::hapiGroupFromProto).toList());
    }

    public static com.hederahashgraph.api.proto.java.ThrottleBucket hapiBucketToProto(final ThrottleBucket<HederaFunctionality> bucket) {
        return com.hederahashgraph.api.proto.java.ThrottleBucket.newBuilder()
                ...
                .addAllThrottleGroups(bucket.getThrottleGroups().stream()
                        .map(HapiThrottleUtils::hapiGroupToProto).toList())
                .build();
    }

    public static ThrottleGroup<HederaFunctionality> hapiGroupFromProto(final com.hederahashgraph.api.proto.java.ThrottleGroup group) {
        return new ThrottleGroup<>(group.getMilliOpsPerSec(), group.getOperationsList());
    }

    public static com.hederahashgraph.api.proto.java.ThrottleGroup hapiGroupToProto(final ThrottleGroup<HederaFunctionality> group) {
