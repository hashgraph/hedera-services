...
public class BucketThrottle {
    ...
    private void validateCapacityForRequested(long mtps, long burstPeriodMs) {
        if (CommonUtils.productWouldOverflow(mtps, NTPS_PER_MTPS * CAPACITY_UNITS_PER_NANO_TXN)) {
            throw new IllegalArgumentException(
    ...
    boolean allowInstantaneous(int n) {
        if (CommonUtils.productWouldOverflow(n, CAPACITY_UNITS_PER_TXN)) {
            return false;
        }
    ...
    private long effectiveLeak(final long elapsedNanos) {
        return CommonUtils.productWouldOverflow(elapsedNanos, mtps)
                ? bucket.totalCapacity()
                : elapsedNanos * mtps;
    }
