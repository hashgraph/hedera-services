...
public class ConsensusServiceFeeBuilder extends FeeBuilder {
  ...
  public static FeeData getConsensusCreateTopicFee(TransactionBody txBody, SigValueObj sigValObj)
    ...
    if (createTopicTxBody.hasAutoRenewPeriod()) {
      extraRbsServices =
          getTopicRamBytes(variableSize) * createTopicTxBody.getAutoRenewPeriod().getSeconds();
    }
  ...
  public static FeeData getConsensusUpdateTopicFee(
      TransactionBody txBody, long rbsIncrease, SigValueObj sigValObj)
      throws InvalidTxBodyException {
  ...
  public static long getUpdateTopicRbsIncrease(
      ...
      Key oldSubmitKey,
      String oldMemo,
      boolean hasOldAutoRenewAccount,
  ...
  private static long calculateRbsIncrease(
    ...
    long oldLifetime =
        Math.min(MAX_ENTITY_LIFETIME, Duration.between(txValidStart, oldExpiration).getSeconds());
    long newLifetime =
        Math.min(MAX_ENTITY_LIFETIME, Duration.between(txValidStart, newExpiration).getSeconds());
  ...
  /**
   * Given transaction specific additional rbh and bpt components, computes fee components for node,
  ...
  private static FeeData getTxFeeMatrices(
      ...
      int txBodyDataSize,
      long extraRbsServices,
      long extraRbsNetwork)
      throws InvalidTxBodyException {
    FeeComponents.Builder feeComponentsBuilder =
  ...
  /**
   * @param variableSize value returned by {@link #computeVariableSizedFieldsUsage(Key, Key, String,
   *     boolean)}.
