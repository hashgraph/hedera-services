...
class ThrottleBucketTest {
  ...
  @Test
  void failsWhenConstructingMilliOpsThrottlesThatNeverPermitAnOperationAtNodeLevel()
      throws IOException {
    // setup:
    int n = 2;
  ...
  void failsWhenConstructingThrottlesWithZeroGroups() {
    ...
    Assertions.assertThrows(
        IllegalStateException.class, () -> new ThrottleBucket().asThrottleMapping(1));
  }
  ...
  private int opsForFunction(
      List<Pair<HederaFunctionality, Integer>> source, HederaFunctionality function) {
    for (var pair : source) {
