...
public class ReviewMainnetEntities extends HapiApiSuite {
  ...
  public HapiApiSpec stablenetCreateAccountWithExplicitKey() {
    ...
    return customHapiSpec("StablenetCreateAccountWithExplicitKey")
        ...
        .given(keyFromLiteral(testVectorKey, explicit))
        .when()
        .then(cryptoCreate("another").balance(1L).receiverSigRequired(true).key(testVectorKey));
  }
    ...
    return customHapiSpec("cryptoCreatePrice")
        .withProperties(
            Map.of(
                ...
                "default.payer.pemKeyPassphrase", "<secret>"))
        .given(cryptoCreate("civilian").balance(ONE_HUNDRED_HBARS))
        .when(
            cryptoCreate("another")
                ...
                .via("civilianCreate"))
        .then(getTxnRecord("civilianCreate").logged());
  }
  ...
  private HapiApiSpec doSomething() {
    final String PROXIES =
        "35.237.194.97:0.0.3,35.186.171.76:0.0.4,35.194.0.222:0.0.5,35.197.115.53:0.0.6,"
    ...
    final String NODES =
        "35.237.208.135:0.0.3,35.245.226.22:0.0.4," + "34.68.9.203:0.0.5,34.83.131.197:0.0.6";
    final String DIRECT_NODES =
        "35.237.194.97:0.0.3,13.71.127.1:0.0.6,27.110.33.145:0.0.7,20.49.137.94:0.0.12,"
            + "35.245.226.22:0.0.4,34.72.55.137:0.0.5,35.203.26.115:0.0.8,34.77.3.213:0.0.9,"
            + // Ubuntu
    ...
    return customHapiSpec("xfer")
        ...
        .given()
        .when()
        .then(cryptoTransfer(tinyBarsFromTo(GENESIS, FUNDING, 1L)));
  }
  ...
  private HapiApiSpec customPayerOp() {
    ...
    return customHapiSpec("xfer")
        ...
        .given(getAccountBalance(payer).logged())
        .when(
            cryptoTransfer(tinyBarsFromTo(DEFAULT_PAYER, "0.0.950", 1))
                .signedBy(DEFAULT_PAYER)
                .logged())
        .then(getAccountBalance(payer).logged());
  }

  private HapiApiSpec checkTls() {
    final String MAINNET_NODES =
    ...
    return customHapiSpec("CheckTls")
        ...
        .then(
            IntStream.range(3, 16)
                .mapToObj(i -> getFileInfo("0.0.101").setNode(String.format("0.0.%d", i)).logged())
