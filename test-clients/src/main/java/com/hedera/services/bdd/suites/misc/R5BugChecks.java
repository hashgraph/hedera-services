...
public class R5BugChecks extends HapiApiSuite {
  ...
  private HapiApiSpec contractCannotTransferToReceiverSigRequired() {
    return defaultHapiSpec("ContractCannotTransferToReceiverSigRequired")
        .given(
            ...
            contractCreate("sponsor").bytecode("bytecode").balance(1))
  ...
  private HapiApiSpec enforcesSigRequirements() {
    ...
    return defaultHapiSpec("EnforcesSigRequirements")
        .given(
            ...
            fileCreate("bytecode").path(ContractResources.LAST_TRACKING_SENDER_BYTECODE_PATH),
            contractCreate("sponsor").bytecode("bytecode").balance(10),
        ...
        .when(
            contractCall(
                    ...
                    spec ->
                        new Object[] {(int) spec.registry().getAccountID("sr").getAccountNum(), 5})
                .hasKnownStatus(INVALID_SIGNATURE),
            contractCall(
                    "sponsor",
                    ContractResources.TRACKING_SEND_ABI,
                ...
                .hasKnownStatus(INVALID_SIGNATURE),
            contractCallLocal("sponsor", ContractResources.HOW_MUCH_ABI)
                .has(
                    resultWith()
                        .resultThruAbi(
                            ContractResources.HOW_MUCH_ABI,
        ...
        .then(
            contractCall(
                ...
                ContractResources.TRACKING_SEND_ABI,
                spec ->
                    new Object[] {(int) spec.registry().getAccountID("noSr").getAccountNum(), 1}),
            contractCall(
                    "sponsor",
                    ContractResources.TRACKING_SEND_ABI,
                    spec ->
    ...
    return defaultHapiSpec("CannotTransferToDeleted")
        ...
        .when(
            contractCall(
                ...
                ContractResources.TRACKING_SEND_ABI,
                spec ->
                    new Object[] {(int) spec.registry().getAccountID("tbd").getAccountNum(), 1}),
            cryptoDelete("tbd"))
        .then(
            contractCall(
                    "sponsor",
                    ContractResources.TRACKING_SEND_ABI,
                    spec ->
                        new Object[] {(int) spec.registry().getAccountID("tbd").getAccountNum(), 2})
  ...
  private HapiApiSpec genesisUpdatesFeesForFree() {
    ...
    return defaultHapiSpec("GenesisUpdatesFeesForFree")
        .given(
            withOpContext(
                (spec, opLog) -> {
                  ...
                  var contents =
                      lookup.getResponse().getFileGetContents().getFileContents().getContents();
        ...
        .then(getAccountBalance(GENESIS).hasTinyBars(changeFromSnapshot("preUpdate", 0)));
  }
  ...
  private HapiApiSpec genRecordWithCreations() {
    return defaultHapiSpec("CreateRecordViaExpensiveSubmit")
        .given(fileCreate("bytecode").path(ContractResources.FUSE_BYTECODE_PATH))
        .when(contractCreate("fuse").bytecode("bytecode"))
        .then(freeze().startingIn(1).minutes().andLasting(1).minutes());
  }
