...
public class RecordPurgeSuite extends HapiApiSuite {
  ...
  private HapiApiSpec differentExpiriesArePurgedAtDifferentTimes() {
    ...
    return defaultHapiSpec("DifferentExpiriesArePurgedAtDifferentTimes")
        .given(cryptoCreate("target").sendThreshold(1L))
        .when(
            cryptoTransfer(tinyBarsFromTo("target", FUNDING, 555L)).payingWith("target").via("txn"))
        .then(
            getAccountRecords("target").has(inOrder(recordWith().txnId("txn"))),
            sleepFor(CACHE_RECORD_TTL_MS + EPSILON_MS),
            cryptoTransfer(tinyBarsFromTo(GENESIS, FUNDING, 555L)),
            getAccountRecords("target").has(inOrder(recordWith().txnId("txn"))),
