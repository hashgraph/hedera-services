...
public class UniqueTokenManagementSpecs extends HapiApiSuite {
  ...
  private HapiApiSpec baseUniqueWipeOperationIsChargedExpectedFee() {
    ...
    return defaultHapiSpec("BaseUniqueWipeOperationIsChargedExpectedFee")
        .given(
            ...
            mintToken(uniqueToken, List.of(ByteString.copyFromUtf8("token_to_wipe"))),
            cryptoTransfer(movingUnique(uniqueToken, 1L).between(TOKEN_TREASURY, civilian)))
        .when(
            wipeTokenAccount(uniqueToken, civilian, List.of(1L))
                ...
                .blankMemo()
                .via(wipeTxn))
        .then(validateChargedUsdWithin(wipeTxn, expectedNftWipePriceUsd, 0.01));
  }
        ...
        .when(
            mintToken(
                ...
                List.of(
                    ...
                    metadata("some metadata2"),
                    metadata("some metadata3"))))
        .then(getAccountNftInfos(TOKEN_TREASURY, 0, 6).hasCostAnswerPrecheck(INVALID_QUERY_RANGE));
  }

  private HapiApiSpec failsWithAccountWithoutNfts() {
    return defaultHapiSpec("FailsWithAccountWithoutNfts")
        .given(cryptoCreate(FIRST_USER))
        .when()
        .then(getAccountNftInfos(FIRST_USER, 0, 2).hasCostAnswerPrecheck(INVALID_QUERY_RANGE));
  }
  ...
  private HapiApiSpec getAccountNftInfosWorksWithMixedOwnerships() {
    ...
    return defaultHapiSpec("GetAccountNftInfosWorksWithMixedOwnerships")
        ...
        .when(
            ...
            mintToken(
                "non-fungible-2",
                List.of(metadata("nft2-1"), metadata("nft2-2"), metadata("nft2-3"))),
            mintToken(
                "non-fungible-3",
                List.of(metadata("nft3-1"), metadata("nft3-2"), metadata("nft3-3"))),
            ...
            tokenUpdate("tbu").treasury("newTreasury").hasKnownStatus(SUCCESS),
            getTokenInfo("tbu").hasTreasury("newTreasury"),
            cryptoTransfer(
                    movingUnique("non-fungible-2", 1, 2).between("oldTreasury", "newTreasury"),
                    movingUnique("non-fungible-3", 1).between("oldTreasury", "newTreasury"))
                .logged(),
            getAccountInfo("newTreasury").logged())
        .then(
            getAccountInfo("oldTreasury").logged(),
            ...
            getAccountNftInfos("newTreasury", 0, 5)
                .hasNfts(
                    ...
                    newTokenNftInfo(
                        "treasury-token-2", 1, "newTreasury", metadata("treasury-meta")))
                .logged(),
  ...
  private HapiApiSpec getAccountNftInfosFailsWithDeletedAccount() {
    return defaultHapiSpec("GetAccountNftInfosFailsWithDeletedAccount")
        .given(cryptoCreate(FIRST_USER), cryptoDelete(FIRST_USER))
        .when()
        .then(getAccountNftInfos(FIRST_USER, 0, 2).hasCostAnswerPrecheck(ACCOUNT_DELETED));
  }
  ...
  private HapiApiSpec getAccountNftInfosFailsWithInexistentAccount() {
    return defaultHapiSpec("GetAccountNftInfosFailsWithInexistentAccount")
        .given()
        .when()
        .then(getAccountNftInfos("0.0.123", 0, 2).hasCostAnswerPrecheck(INVALID_ACCOUNT_ID));
  }
  ...
  private HapiApiSpec getAccountNftInfosFailsWithInvalidQueryBoundaries() {
    return defaultHapiSpec("GetAccountNftInfosFailsWithInvalidQueryBoundaries")
        ...
        .when()
        .then(
            getAccountNftInfos(FIRST_USER, 2, 0).hasCostAnswerPrecheck(INVALID_QUERY_RANGE),
    ...
    return defaultHapiSpec("BurnFailsOnInvalidSerialNumber")
        ...
        .then(
            ...
            getAccountNftInfos(TOKEN_TREASURY, 0, 1)
                .hasNfts(newTokenNftInfo(NFT, 1, TOKEN_TREASURY, metadata("memo")))
                .logged());
  }
  ...
  private HapiApiSpec burnRespectsBurnBatchConstraints() {
    return defaultHapiSpec("BurnRespectsBurnBatchConstraints")
        ...
        .when()
        .then(
            burnToken(NFT, LongStream.range(0, 1000).boxed().collect(Collectors.toList()))
                .via("burnTxn")
    ...
    return defaultHapiSpec("CanOnlyBurnFromTreasury")
        ...
        .then(
            ...
            getTokenNftInfos(NFT, 0, 2)
                ...
                .logged(),
            getAccountNftInfos(TOKEN_TREASURY, 0, 1)
                .hasNfts(newTokenNftInfo(NFT, 1, TOKEN_TREASURY, metadata("1"))),
            getAccountNftInfos(nonTreasury, 0, 1)
                .hasNfts(newTokenNftInfo(NFT, 2, nonTreasury, metadata("2"))));
  }
            ...
            UtilVerbs.withOpContext(
                (spec, opLog) -> {
                  ...
                  Assertions.assertNotEquals(
                      nftFee, fungibleFee, "NFT Fee should NOT equal to the Fungible Fee!");
                }));
  }
    ...
    return defaultHapiSpec("MintUniqueTokenHappyPath")
        ...
        .then(
            ...
            getTokenNftInfos(NFT, 1, 2)
                .hasNfts(newTokenNftInfo(NFT, 2, TOKEN_TREASURY, metadata("memo1")))
                .logged(),
            getTokenNftInfos(NFT, 0, 2)
                .hasNfts(
  ...
  private HapiApiSpec wipeHappyPath() {
    return defaultHapiSpec("WipeHappyPath")
        .given(
            ...
            mintToken(
                NFT, List.of(ByteString.copyFromUtf8("memo"), ByteString.copyFromUtf8("memo2"))),
    ...
    return defaultHapiSpec("WipeRespectsConstraints")
        ...
        .when(
            wipeTokenAccount(
                ...
                .hasPrecheck(BATCH_SIZE_LIMIT_EXCEEDED),
            getAccountNftInfos("account", 0, 2)
                .hasNfts(
  ...
  private HapiApiSpec commonWipeFailsWhenInvokedOnUniqueToken() {
    return defaultHapiSpec("CommonWipeFailsWhenInvokedOnUniqueToken")
        ...
        .when()
        .then(
            wipeTokenAccount(NFT, "account", 1L).hasKnownStatus(FAIL_INVALID).via("wipeTxn"),
            // no new totalSupply
            getTokenInfo(NFT).hasTotalSupply(1),
            // no tx record
                            ...
                            tokenTransferList.getNftTransfersList().stream()
                                .forEach(
                                    nftTransfers -> {
                                      ...
                                      Assertions.assertEquals(
                                          TxnUtils.asId(TOKEN_TREASURY, spec),
        ...
        .when(
            mintToken(
                ...
                List.of(
                    ...
                    metadata("some metadata2"),
                    metadata("some metadata3"))))
        .then(getTokenNftInfos(NFT, 0, 6).hasCostAnswerPrecheck(INVALID_QUERY_RANGE));
  }
    ...
    return defaultHapiSpec("GetTokenNftInfosFailsWithTokenWithoutNfts")
        .given(
            ...
            tokenCreate(NFT)
                ...
                .treasury(TOKEN_TREASURY))
        .when()
        .then(getTokenNftInfos(NFT, 0, 2).hasCostAnswerPrecheck(INVALID_QUERY_RANGE));
  }
  ...
  private HapiApiSpec getTokenNftInfosFailsWithInvalidQueryBoundaries() {
    return defaultHapiSpec("GetTokenNftInfosFailsWithInvalidQueryBoundaries")
        ...
        .when()
        .then(
            getTokenNftInfos(NFT, 2, 0).hasCostAnswerPrecheck(INVALID_QUERY_RANGE),
            getTokenNftInfos(NFT, 0, 100000000).hasCostAnswerPrecheck(INVALID_QUERY_RANGE));
  }
    ...
    return defaultHapiSpec("GetTokenNftInfosFailsWithDeletedTokenNft")
        .given(
            ...
            tokenCreate(NFT)
                ...
                .treasury(TOKEN_TREASURY))
        .when(tokenDelete(NFT))
        .then(getTokenNftInfos(NFT, 0, 2).hasCostAnswerPrecheck(TOKEN_WAS_DELETED));
  }
  ...
  public HapiApiSpec failsWithFungibleTokenGetNftInfos() {
    return defaultHapiSpec("FailsWithFungibleTokenGetNftInfos")
        ...
        .when(tokenCreate(A_TOKEN).initialSupply(1_000).treasury(TOKEN_TREASURY))
        .then(getTokenNftInfos(A_TOKEN, 0, 2).hasCostAnswerPrecheck(NOT_SUPPORTED));
  }
    ...
    return defaultHapiSpec("BaseUniqueMintOperationIsChargedExpectedFee")
        ...
        .when(
            mintToken(uniqueToken, List.of(standard100ByteMetadata))
                ...
                .blankMemo()
                .via(baseTxn))
        .then(validateChargedUsdWithin(baseTxn, expectedNftMintPriceUsd, 0.01));
  }
    ...
    return defaultHapiSpec("BaseUniqueBurnOperationIsChargedExpectedFee")
        ...
        .when(
            burnToken(uniqueToken, List.of(1L))
                ...
                .blankMemo()
                .via(baseTxn))
        .then(validateChargedUsdWithin(baseTxn, expectedNftBurnPriceUsd, 0.01));
  }
