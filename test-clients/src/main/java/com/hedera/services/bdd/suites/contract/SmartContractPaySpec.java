...
public class SmartContractPaySpec extends HapiApiSuite {
  ...
  protected List<HapiApiSpec> getSpecsInSuite() {
    return List.of(new HapiApiSpec[] {smartContractPaySpec()});
  }
    ...
    return defaultHapiSpec("smartContractPaySpec")
        ...
        .then(
            assertionsHold(
                (spec, ctxLog) -> {
                  var subop1 =
                      getContractInfo("payReceivable")
                          .nodePayment(10L)
                          .saveToRegistry("payReceivableKey");

                  var subop2 = getAccountInfo("receiver").saveToRegistry("receiverAccountInfoKey");
                  CustomSpecAssert.allRunFor(spec, subop1, subop2);

                  ContractGetInfoResponse.ContractInfo payReceivableContratInfo =
                      spec.registry().getContractInfo("payReceivableKey");
                  CryptoGetInfoResponse.AccountInfo receiverAccountInfo =
                      spec.registry().getAccountInfo("receiverAccountInfoKey");
                  ...
                  var subop3 =
                      contractCall("payReceivable", ContractResources.DEPOSIT_ABI, 10_000L)
                          .payingWith("payer")
                          .gas(300_000L)
                          .via("depositTxn")
                          .sending(10_000L);
                  ...
                  var subop4 =
                      contractCall(
                              "payReceivable",
                              ContractResources.SEND_FUNDS_ABI,
                              receiverAcctAddress,
                              10_000L)
                          .sending(10_000L)
                          .via("sendFundTxn")
                  ...
                  var subop5 =
                      getTxnRecord("sendFundTxn").saveTxnRecordToRegistry("sendFundTxnRecord");

                  CustomSpecAssert.allRunFor(spec, subop3, subop4, subop5);
                  TransactionRecord txnRecord =
                      spec.registry().getTransactionRecord("sendFundTxnRecord");

                  // validate transfer list
                  List<AccountAmount> expectedTransfers = new ArrayList<>(3);
                  AccountAmount receiverTransfer =
                      AccountAmount.newBuilder()
                          .setAccountID(receiverAccountInfo.getAccountID())
                          .setAmount(10_000L)
                          .build();
                  expectedTransfers.add(receiverTransfer);
                  AccountAmount contractTransfer =
                      AccountAmount.newBuilder()
                          .setAccountID(payReceivableContratInfo.getAccountID())
                          .setAmount(15_000L)
                          .build();
                  ...
                  var subop6 = getAccountInfo("receiver").saveToRegistry("receiverAccountInfoKey");
                  ...
                  Assertions.assertEquals(
                      10000, receiverAccountInfo.getBalance(), "Receiver balance at the end: ");
                }));
  }
