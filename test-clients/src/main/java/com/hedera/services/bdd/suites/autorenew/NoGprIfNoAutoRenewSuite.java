...
public class NoGprIfNoAutoRenewSuite extends HapiApiSuite {
  ...
  private HapiApiSpec contractCallRestrictionsNotEnforced() {
    ...
    return defaultHapiSpec("ContractCallRestrictionsNotEnforced")
        .given(
            ...
            contractCreate(contract).balance(ONE_HBAR).bytecode(bytecode),
            cryptoCreate(civilian).balance(0L),
            cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(1))
        .when(
            sleepFor(1_500L),
            ...
            withOpContext(
                (spec, opLog) -> {
                  detachedNum.set(
                      (int) spec.registry().getAccountID(notDetachedAccount).getAccountNum());
            ...
            sourcing(
                () ->
                    contractCall(
                        contract,
                        SEND_TO_TWO_ABI,
                        new Object[] {civilianNum.get(), detachedNum.get()})))
        .then(
  ...
  private HapiApiSpec cryptoUpdateRestrictionsNotEnforced() {
    ...
    return defaultHapiSpec("CryptoUpdateRestrictionsNotEnforced")
        ...
        .when(
            ...
            cryptoUpdate(notDetachedAccount).memo("Can update key").key("ntb"),
            cryptoUpdate(notDetachedAccount)
                .memo("Can update auto-renew period")
                .autoRenewPeriod(THREE_MONTHS_IN_SECONDS),
            cryptoUpdate(notDetachedAccount).memo("Can update memo").entityMemo("NOPE"),
            cryptoUpdate(notDetachedAccount)
                .memo("Can't pass precheck with past expiry")
                .expiring(certainlyPast)
                .hasPrecheck(INVALID_EXPIRATION_TIME))
        .then(
            cryptoUpdate(notDetachedAccount).memo("CAN extend expiry").expiring(certainlyDistant),
            cryptoUpdate(notDetachedAccount)
  ...
  private HapiApiSpec payerRestrictionsNotEnforced() {
    ...
    return defaultHapiSpec("PayerRestrictionsEnforced")
        .given(cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(1))
        .when(sleepFor(1_500L), cryptoTransfer(tinyBarsFromTo(DEFAULT_PAYER, FUNDING, 1L)))
        .then(
  ...
  private HapiApiSpec topicAutoRenewOpsNotEnforced() {
    ...
    return defaultHapiSpec("TopicAutoRenewOpsNotEnforced")
        .given(newKeyNamed(adminKey), cryptoCreate(civilian))
        .when(
            cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(2),
  ...
  private HapiApiSpec tokenAutoRenewOpsNotEnforced() {
    ...
    return defaultHapiSpec("TokenAutoRenewOpsNotEnforced")
        ...
        .when(
            ...
            tokenCreate(tokenSansDetachedAsAutoRenew).autoRenewAccount(civilian).adminKey(adminKey),
            sleepFor(1_500L))
        .then(
  ...
  private HapiApiSpec treasuryOpsRestrictionNotEnforced() {
    ...
    return defaultHapiSpec("TreasuryOpsRestrictionNotEnforced")
        .given(newKeyNamed(tokenMultiKey), cryptoCreate(civilian))
        .when(
            cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(2),
            tokenCreate(aToken)
  ...
  private HapiApiSpec tokenMgmtRestrictionsNotEnforced() {
    ...
    return defaultHapiSpec("TokenMgmtRestrictionsNotEnforced")
        .given(
            ...
            tokenCreate(tokenNotYetAssociated).adminKey(tokenMultiKey),
            tokenCreate(tokenAlreadyAssociated).freezeKey(tokenMultiKey).kycKey(tokenMultiKey))
        .when(
            cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(2),
            tokenAssociate(notDetachedAccount, tokenAlreadyAssociated),
            sleepFor(1_500L))
        .then(
            tokenCreate(onTheFly).treasury(notDetachedAccount),
  ...
  private HapiApiSpec cryptoDeleteRestrictionsNotEnforced() {
    ...
    return defaultHapiSpec("CryptoDeleteRestrictionsNotEnforced")
        .given(
            cryptoCreate(civilian), cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(2))
  ...
  private HapiApiSpec cryptoTransferRestrictionsNotEnforced() {
    ...
    return defaultHapiSpec("CryptoTransferRestrictionsNotEnforced")
        .given(
            cryptoCreate(civilian),
            cryptoCreate(notDetachedAccount).balance(0L).autoRenewSecs(2),
            tokenCreate(aToken).treasury(notDetachedAccount),
            tokenAssociate(civilian, aToken))
        .when(sleepFor(1_500L))
        .then(
