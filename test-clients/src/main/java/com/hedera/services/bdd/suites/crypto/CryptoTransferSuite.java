...
public class CryptoTransferSuite extends HapiApiSuite {
    ...
    return defaultHapiSpec("BaseCryptoTransferIsChargedAsExpected")
        ...
        .when(
            ...
            cryptoTransfer(movingUnique(nonFungibleToken, 1).between(sender, receiver))
                ...
                .via(nftXferTxn),
            cryptoTransfer(
                    moving(1, fungibleTokenWithCustomFee).between(nonTreasurySender, receiver))
                .blankMemo()
                .fee(ONE_HBAR)
        ...
        .then(
            ...
            validateChargedUsdWithin(
                nftXferTxnWithCustomFee, expectedNftXferWithCustomFeePriceUsd, 0.3));
  }
    ...
    return defaultHapiSpec("TokenTransferFeesScaleAsExpected")
        ...
        .when(
            ...
            cryptoTransfer(moving(1, "A").between("a", "c"), moving(1, "B").between("b", "d"))
                ...
                .payingWith("a"),
            cryptoTransfer(
                    moving(1, "A").between("a", "c"), moving(2, "B").distributing("b", "d", "e"))
        ...
        .then(
            withOpContext(
                (spec, opLog) -> {
                  ...
                  Assertions.assertEquals(10.0, pureOneTokenTwoAccountsUsd / pureHbarUsd, 1.0);
                  Assertions.assertEquals(20.0, pureTwoTokensFourAccountsUsd / pureHbarUsd, 2.0);
                  Assertions.assertEquals(30.0, pureThreeTokensSixAccountsUsd / pureHbarUsd, 3.0);
                }));
  }
  ...
  private HapiApiSpec transferToTopicReturnsInvalidAccountId() {
    ...
    return defaultHapiSpec("TransferToTopicReturnsInvalidAccountId")
        ...
        .then(
            sourcing(
                () ->
                    cryptoTransfer(tinyBarsFromTo(DEFAULT_PAYER, invalidAccountId.get(), 1L))
                        .signedBy(DEFAULT_PAYER)
                        .hasKnownStatus(INVALID_ACCOUNT_ID)),
            sourcing(
                () ->
                    cryptoTransfer(
                            moving(1, "token").between(DEFAULT_PAYER, invalidAccountId.get()))
                        .signedBy(DEFAULT_PAYER)
                        .hasKnownStatus(INVALID_ACCOUNT_ID)));
  }
  ...
  private HapiApiSpec twoComplexKeysRequired() {
    ...
    SigControl receiverSigs =
        KeyShape.threshSigs(
            3, KeyShape.threshSigs(2, ON, ON), KeyShape.threshSigs(3, OFF, OFF, ON, ON, ON), ON);
