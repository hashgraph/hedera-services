...
public class ExchangeRateControlSuite extends HapiApiSuite {
  ...
  final HapiFileUpdate resetRatesOp =
    ...
    return defaultHapiSpec("MidnightRateChangesWhenAcct50UpdatesFile112")
        ...
        .when(
            fileUpdate(EXCHANGE_RATES)
                .contents(
                    spec -> {
                      ByteString newRates = spec.ratesProvider().rateSetWith(1, 25).toByteString();
                      spec.registry().saveBytes("newRates", newRates);
                      return newRates;
                    })
        ...
        .then(
            ...
            fileUpdate(EXCHANGE_RATES)
                .contents(
                    spec -> {
                      ByteString newRates =
                          spec.ratesProvider().rateSetWith(1, 12, 1, 15).toByteString();
                      spec.registry().saveBytes("newRates", newRates);
                      return newRates;
                    })
                .payingWith(SYSTEM_ADMIN)
  ...
  private HapiApiSpec acct57CantMakeLargeChanges() {
    return defaultHapiSpec("Acct57CantMakeLargeChanges")
        ...
        .then(
            fileUpdate(EXCHANGE_RATES)
                .contents(spec -> spec.ratesProvider().rateSetWith(1, 25).toByteString())
                .payingWith(EXCHANGE_RATE_CONTROL)
                .hasKnownStatus(EXCHANGE_RATE_CHANGE_LIMIT_EXCEEDED));
  }
