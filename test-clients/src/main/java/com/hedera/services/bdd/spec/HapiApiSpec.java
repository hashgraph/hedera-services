...
public class HapiApiSpec implements Runnable {
  ...
  private static Map<String, String> ciPropOverrides() {
    if (ciPropsSource == null) {
      ...
      dynamicNodes =
          Stream.of(dynamicNodes.split(",")).map(s -> s + ":" + port).collect(joining(","));
  ...
  public String toString() {
    SpecStatus resolved =
        ((status == FAILED) && OK(this))
            ? FAILED_AS_EXPECTED
            : (((status == PASSED) && NOT_OK(this)) ? PASSED_UNEXPECTEDLY : status);
    return MoreObjects.toStringHelper("Spec").add("name", name).add("status", resolved).toString();
  }
  ...
  private String costSnapshotFilePath() {
    ...
    ensureDir(dir);
    String path =
        String.format("cost-snapshots/%s/%s", hapiSetup.costSnapshotDir(), costSnapshotFile());
    return path;
  }
