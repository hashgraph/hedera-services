...
public class ScheduleCreateSpecs extends HapiApiSuite {
  ...
  private HapiApiSpec bodyOnlyCreation() {
    return defaultHapiSpec("BodyOnlyCreation")
        ...
        .when(
            scheduleCreate("onlyBody", cryptoTransfer(tinyBarsFromTo("sender", GENESIS, 1)))
                .recordingScheduledTxn())
        .then(getScheduleInfo("onlyBody").hasScheduleId("onlyBody").hasRecordedScheduledTxn());
  }
  ...
  private HapiApiSpec onlyBodyAndAdminCreation() {
    return defaultHapiSpec("OnlyBodyAndAdminCreation")
        ...
        .when(
            scheduleCreate(
                    "onlyBodyAndAdminKey", cryptoTransfer(tinyBarsFromTo("sender", GENESIS, 1)))
                .adminKey("admin")
                .recordingScheduledTxn())
        .then(
  ...
  private HapiApiSpec onlyBodyAndMemoCreation() {
    return defaultHapiSpec("OnlyBodyAndMemoCreation")
        .given(cryptoCreate("sender"))
        .when(
            scheduleCreate("onlyBodyAndMemo", cryptoTransfer(tinyBarsFromTo("sender", GENESIS, 1)))
                .recordingScheduledTxn()
  ...
  private HapiApiSpec bodyAndPayerCreation() {
    return defaultHapiSpec("BodyAndPayerCreation")
        ...
        .when(
            scheduleCreate(
                    "onlyBodyAndPayer",
                    cryptoTransfer(tinyBarsFromTo(DEFAULT_PAYER, GENESIS, 1)).memo("SURPRISE!!!"))
                .recordingScheduledTxn()
  ...
  private HapiApiSpec notIdenticalScheduleIfScheduledTxnChanges() {
    return defaultHapiSpec("NotIdenticalScheduleIfScheduledTxnChanges")
        .given(
            ...
            scheduleCreate(
                    "original",
                    cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).memo("A").fee(ONE_HBAR))
                .payingWith("firstPayer"))
        .when()
        .then(
            scheduleCreate(
                    "continue",
                    cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).memo("B").fee(ONE_HBAR))
                .payingWith("firstPayer"));
  }
  ...
  private HapiApiSpec notIdenticalScheduleIfMemoChanges() {
    return defaultHapiSpec("NotIdenticalScheduleIfMemoChanges")
        .given(
            cryptoCreate("sender").balance(1L),
            scheduleCreate(
                    "original", cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).fee(ONE_HBAR))
                .withEntityMemo("This was Mr. Bleaney's room. He stayed"))
        .when()
        .then(
            scheduleCreate(
                    "continue", cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).fee(ONE_HBAR))
  ...
  private HapiApiSpec recognizesIdenticalScheduleEvenWithDifferentDesignatedPayer() {
    return defaultHapiSpec("recognizesIdenticalScheduleEvenWithDifferentDesignatedPayer")
        ...
        .then(
            scheduleCreate(
                    "duplicate", cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).fee(ONE_HBAR))
  ...
  private HapiApiSpec rejectsSentinelKeyListAsAdminKey() {
    return defaultHapiSpec("RejectsSentinelKeyListAsAdminKey")
        ...
        .when()
        .then(
            scheduleCreate("creation", cryptoTransfer(tinyBarsFromTo(GENESIS, FUNDING, 1)))
  ...
  private HapiApiSpec rejectsMalformedScheduledTxnMemo() {
    return defaultHapiSpec("RejectsMalformedScheduledTxnMemo")
        ...
        .then(
            scheduleCreate(
                    "creation",
                    cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).memo(nAscii(101)))
                .hasPrecheck(MEMO_TOO_LONG),
            scheduleCreate(
                    "creationPartDeux",
  ...
  public HapiApiSpec detectsKeysChangedBetweenExpandSigsAndHandleTxn() {
    ...
    return defaultHapiSpec("DetectsKeysChangedBetweenExpandSigsAndHandleTxn")
        ...
        .then(
            ...
            scheduleCreate(
                    "outdatedXferSigs",
                    cryptoTransfer(tinyBarsFromTo("sender", "receiver", 1)).fee(ONE_HBAR))
  ...
  public HapiApiSpec onlySchedulesWithMissingReqSimpleSigs() {
    return defaultHapiSpec("OnlySchedulesWithMissingReqSimpleSigs")
        ...
        .when(
            scheduleCreate("basicXfer", cryptoTransfer(tinyBarsFromTo("sender", "receiver", 1)))
  ...
  public HapiApiSpec requiresExtantPayer() {
    return defaultHapiSpec("RequiresExtantPayer")
        ...
        .when()
        .then(
            scheduleCreate("neverToBe", cryptoCreate("nope").key(GENESIS).receiverSigRequired(true))
                .designatingPayer("1.2.3")
                .hasKnownStatus(INVALID_ACCOUNT_ID));
  }
