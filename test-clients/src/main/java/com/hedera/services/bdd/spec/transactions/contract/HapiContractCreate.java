...
public class HapiContractCreate extends HapiTxnOp<HapiContractCreate> {
  ...
  protected void updateStateOf(HapiApiSpec spec) {
    ...
    spec.registry()
        .saveKey(contract, (omitAdminKey || useDeprecatedAdminKey) ? MISSING_ADMIN_KEY : adminKey);
    spec.registry().saveContractId(contract, lastReceipt.getContractID());
    ContractGetInfoResponse.ContractInfo otherInfo =
  ...
  protected Consumer<TransactionBody.Builder> opBodyDef(HapiApiSpec spec) throws Throwable {
    ...
    Optional<byte[]> params =
        abi.isPresent()
            ? Optional.of(
                CallTransaction.Function.fromJsonInterface(abi.get()).encodeArguments(args.get()))
            : Optional.empty();
            ...
            .<ContractCreateTransactionBody, ContractCreateTransactionBody.Builder>body(
                ...
                b -> {
                  ...
                  autoRenewPeriodSecs.ifPresent(
                      p -> b.setAutoRenewPeriod(Duration.newBuilder().setSeconds(p).build()));
