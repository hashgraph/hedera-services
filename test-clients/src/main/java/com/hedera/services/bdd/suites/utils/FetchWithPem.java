...
public class FetchWithPem extends HapiApiSuite {
  ...
  private HapiApiSpec updateFeeSchedule() {
    try {
      ...
      ByteString fromDisk = ByteString.copyFrom(Files.readAllBytes(Paths.get(feeScheduleLoc)));
  ...
  private HapiApiSpec getSystemFiles() {
    ...
    return customHapiSpec("GetSystemKeys")
        ...
        .when()
        .then(
            getFileContents(EXCHANGE_RATES).saveTo("staging112.bin"),
            ...
            getFileContents(API_PERMISSIONS)
                ...
                .saveReadableTo(
                    uncheckedProps(ServicesConfigurationList::parseFrom),
                    "testnet-api-permission.properties")
                .logged());
  }
  ...
  private String literalFromPem(String loc) {
    try {
      ...
      return SpecUtils.asSerializedOcKeystore(
          f, "<SECRET>", HapiPropertySource.asAccount("0.0.50"));
  ...
  private Function<byte[], String> uncheckedProps(CheckedParser parser) {
    return bytes -> {
      try {
        ...
        ((ServicesConfigurationList) proto)
            .getNameValueList()
            .forEach(setting -> jutilConfig.setProperty(setting.getName(), setting.getValue()));
        return jutilConfig.stringPropertyNames().stream()
