...
public abstract class HapiApiSuite {
  ...
  public static HapiSpecOperation[] flattened(Object... ops) {
    return Stream.of(ops)
        .map(
            op ->
                (op instanceof HapiSpecOperation)
                    ? new HapiSpecOperation[] {(HapiSpecOperation) op}
                    : ((op instanceof List)
                        ? ((List) op).toArray(new HapiSpecOperation[0])
  ...
  private String asNormalApproximation(Stats stats) {
    if (stats.count() > 1) {
      return "~ \uD835\uDCA9(μ="
          + rounded(stats.mean())
          + "ms, σ="
          + rounded(stats.sampleStandardDeviation())
          + "ms)";
    } else {
  ...
  public static boolean cacheRecordsAreAddedToState() {
    return "true"
        .equals(Optional.ofNullable(System.getenv("ADD_CACHE_RECORD_TO_STATE")).orElse(""));
  }
}
