		...
		return defaultHapiSpec("TXRecordsContainValidTransfers")
				.given(
						...
						assertionsHold((spec, ctxLog) -> {
							...
							var subop4 = contractCall("payReceivable", ContractResources.SEND_FUNDS_ABI,
									receiverAcctAddress, 10_000L)
									.sending(10_000L)
									.via("sendFundTxn")
									.payingWith("payer");

							// Not sure why this doesn't work here.
							// var subop5 = validateTransferListForBalances("sendFundTxn", List.of(  "payer",
							// "receiver", NODE,  "payReceivable"));
							...
							AccountAmount receiverTransfer = AccountAmount.newBuilder().setAccountID(
									receiverAccountInfo.getAccountID()).setAmount(10_000L).build();
							expectedTransfers.add(receiverTransfer);
							AccountAmount contractTransfer = AccountAmount.newBuilder().setAccountID(
									payReceivableContratInfo.getAccountID()).setAmount(15_000L).build();
