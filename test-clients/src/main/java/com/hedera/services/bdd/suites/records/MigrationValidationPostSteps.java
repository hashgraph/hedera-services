...
/**
 * This suite fetches the entities mentioned in the config file passed in the arguments and verifies
 * if they are valid/persent and performs further operations on them.
 */
public class MigrationValidationPostSteps extends HapiApiSuite {
  ...
  public static final String MIGRATION_FILE =
      HapiSpecSetup.getDefaultInstance().migrationFileName();
  public static final String MIGRATION_ACCOUNT_A =
      HapiSpecSetup.getDefaultInstance().migrationAccountAName();
  public static final String MIGRATION_ACCOUNT_B =
      HapiSpecSetup.getDefaultInstance().migrationAccountBName();
  public static final String MIGRATION_SMART_CONTRACT =
      HapiSpecSetup.getDefaultInstance().migrationSmartContractName();
  ...
  public static void main(String... args) {
    ...
    if (args.length == 0) {
      log.info(
          "using default IDs for cryptoAccounts, files and smartContracts -- RUNNING CUSTOM NET MIGRATION ");
  ...
  protected List<HapiApiSpec> getSpecsInSuite() {
    return List.of(new HapiApiSpec[] {migrationPreservesEntitiesPostStep()});
  }

  /**
   * Builds a spec in which it... 1. Validate the file contents after migration 2. conduct some
   * crypto transfers among the test accounts and verify they are successful; 3. run the same smart
   * contract to verify successful runs;
   *
   * @return the spec.
   */
  private HapiApiSpec migrationPreservesEntitiesPostStep() {
    ...
    return HapiApiSpec.customHapiSpec("migrationPreservesEntitiesPostStep")
        .withProperties(migrationPropertyPath)
        .given()
        .when()
        .then(
  ...
  private HapiSpecOperation[] verifySmartContractActions() {
    return new HapiSpecOperation[] {
      TxnVerbs.contractCall(
              MIGRATION_SMART_CONTRACT,
              ContractResources.SIMPLE_STORAGE_SETTER_ABI,
              VALUE_TO_SET + 1)
          .gas(15000),
      sleepFor(2_000L),
      TxnVerbs.contractCall(MIGRATION_SMART_CONTRACT, ContractResources.SIMPLE_STORAGE_GETTER_ABI)
          .via(SC_getValue)
          .gas(20000),
      sleepFor(2_000L),
      QueryVerbs.getTxnRecord(SC_getValue)
          .hasPriority(
                      ...
                      ContractFnResultAsserts.resultWith()
                          .resultThruAbi(
                              ...
                              ContractFnResultAsserts.isLiteralResult(
                                  new Object[] {BigInteger.valueOf(VALUE_TO_SET + 1)}))))
    };
  }
  ...
  private HapiSpecOperation[] verifyFileActions() {
    return new HapiSpecOperation[] {
      QueryVerbs.getFileContents(MIGRATION_FILE).hasContents(ignore -> old4K), sleepFor(2_000L),
    };
  }

  private HapiSpecOperation[] verifyRecords() {
    return new HapiSpecOperation[] {
      QueryVerbs.getContractRecords(MIGRATION_SMART_CONTRACT).checkingAgainst(smartContractRecords),
      sleepFor(2_000L),
