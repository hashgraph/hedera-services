...
public class ContractDeleteSuite extends HapiApiSuite {
  ...
  HapiApiSpec rejectsWithoutProperSig() {
    return defaultHapiSpec("ScDelete")
        .given(contractCreate("tbd"))
        .when()
        .then(contractDelete("tbd").signedBy(GENESIS).hasKnownStatus(INVALID_SIGNATURE));
  }

  private HapiApiSpec deleteFailsWithImmutableContract() {
    return defaultHapiSpec("DeleteFailsWithImmutableContract")
        .given(contractCreate("immutable").omitAdminKey())
        .when()
        .then(contractDelete("immutable").hasKnownStatus(MODIFYING_IMMUTABLE_CONTRACT));
  }

  private HapiApiSpec deleteWorksWithMutableContract() {
    return defaultHapiSpec("DeleteWorksWithMutableContract")
        .given(contractCreate("toBeDeleted"))
        .when()
        .then(contractDelete("toBeDeleted"));
  }

  private HapiApiSpec systemDelThenUndelContractSanityChecks() {
    return defaultHapiSpec("SystemDelThenUndelContractSanityChecks")
        .given(
            fileCreate("conFile").path(PATH_TO_VALID_BYTECODE),
            contractCreate("test-contract").bytecode("conFile"))
        .when(systemContractDelete("test-contract").payingWith(SYSTEM_DELETE_ADMIN))
        .then(
