...
public class LazyCreateThroughPrecompileSuite extends HapiApiSuite {
  ...
  public static final ByteString SECOND_META =
      ByteString.copyFrom(FIRST.getBytes(StandardCharsets.UTF_8));
  private static final String TRANSFER_TO_ALIAS_PRECOMPILE_CONTRACT = "TransferToAliasPrecompileContract";
  ...
  private HapiApiSpec transferTokenToEVMAddressAliasRevertAndTransferAgainSuccessfully() {
    ...
    return defaultHapiSpec(
        ...
        .given(
            ...
            tokenAssociate(TRANSFER_TO_ALIAS_PRECOMPILE_CONTRACT, List.of(FUNGIBLE_TOKEN)),
            cryptoTransfer(
                moving(5, FUNGIBLE_TOKEN).between(TOKEN_TREASURY, TRANSFER_TO_ALIAS_PRECOMPILE_CONTRACT)))
        .when(
            withOpContext(
                (spec, opLog) -> {
                  final var ecdsaKey =
