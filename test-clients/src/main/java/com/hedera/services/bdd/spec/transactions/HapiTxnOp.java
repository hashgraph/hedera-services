...
public abstract class HapiTxnOp<T extends HapiTxnOp<T>> extends HapiSpecOperation {
  ...
  protected abstract Consumer<TransactionBody.Builder> opBodyDef(HapiApiSpec spec) throws Throwable;
    ...
    while (true) {
      ...
      try {
        ...
        if (isRecognizedRecoverable(msg)) {
          log.info(
              "Recognized recoverable runtime exception {}, retrying status resolution...", msg);
          continue;
        } else {
          ...
          log.error(
              ...
              CommonUtils.toReadableString(txn));
  ...
  private void rechargePayerFor(HapiApiSpec spec) {
    ...
    spec.registry()
        .setRechargingTime(payer.get(), Instant.now()); // record timestamp of last recharge event
  ...
  protected KeyGenerator effectiveKeyGen() {
    return (keyGen.orElse(KeyGenerator.Nature.RANDOMIZED)
        ...
        : KeyExpansion::genSingleEd25519Key;
  }
