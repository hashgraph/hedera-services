...
public class CryptoDeleteSuite extends HapiApiSuite {
  ...
  private HapiApiSpec deletedAccountCannotBePayer() {
    ...
    return defaultHapiSpec("DeletedAccountCannotBePayer")
        .given(cryptoCreate(ACCOUNT_TO_BE_DELETED), cryptoCreate(BENEFICIARY_ACCOUNT).balance(0L))
        .when()
        .then(
        ...
        .then(
            ...
            getTxnRecord("deleteTxn")
                .hasPriority(
                    recordWith()
                        .transfers(
                            including(tinyBarsFromTo("toBeDeleted", "transferAccount", B)))));
  }
  ...
  private HapiApiSpec cannotDeleteAccountsWithNonzeroTokenBalances() {
    return defaultHapiSpec("CannotDeleteAccountsWithNonzeroTokenBalances")
        ...
        .when(
            tokenCreate("misc").initialSupply(TOKEN_INITIAL_SUPPLY).treasury(TOKEN_TREASURY),
            tokenAssociate("toBeDeleted", "misc"),
            cryptoTransfer(
                moving(TOKEN_INITIAL_SUPPLY, "misc").between(TOKEN_TREASURY, "toBeDeleted")))
        .then(
  ...
  private HapiApiSpec cannotDeleteTreasuryAccount() {
    return defaultHapiSpec("CannotDeleteTreasuryAccount")
        ...
        .when(
            tokenCreate("toBeTransferred").initialSupply(TOKEN_INITIAL_SUPPLY).treasury("treasury"))
        .then(
