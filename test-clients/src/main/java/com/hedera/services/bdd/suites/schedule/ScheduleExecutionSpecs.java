...
public class ScheduleExecutionSpecs extends HapiApiSuite {
  ...
  private HapiApiSpec scheduledBurnWithInvalidTokenThrowsUnresolvableSigners() {
    return defaultHapiSpec("ScheduledBurnWithInvalidTokenThrowsUnresolvableSigners")
        .given(overriding("scheduling.whitelist", "TokenBurn"), cryptoCreate("schedulePayer"))
        .when(
    ...
    return defaultHapiSpec("ScheduledUniqueBurnFailsWithNftsDisabled")
        ...
        .when(
            scheduleSign("validSchedule")
                ...
                .hasKnownStatus(SUCCESS))
        .then(
            getTxnRecord(failingTxn).scheduled().hasPriority(recordWith().status(NOT_SUPPORTED)),
            getTokenInfo(A_TOKEN).hasTotalSupply(0),
            fileUpdate(APP_PROPERTIES)
                .payingWith(ADDRESS_BOOK_CONTROL)
    ...
    return defaultHapiSpec("ScheduledBurnFailsWithInvalidTxBody")
        .given(
            ...
            tokenCreate(A_TOKEN)
                ...
                .tokenType(TokenType.NON_FUNGIBLE_UNIQUE)
                .initialSupply(0),
            scheduleCreate("validSchedule", invalidBurnToken(A_TOKEN, List.of(1L, 2L), 123))
                .designatingPayer("schedulePayer")
                .via(failingTxn))
        .when(
            scheduleSign("validSchedule")
    ...
    return defaultHapiSpec("ScheduledUniqueMintFailsWithNftsDisabled")
        .given(
            ...
            tokenCreate(A_TOKEN)
                ...
                .initialSupply(0),
            scheduleCreate(
                    "validSchedule", mintToken(A_TOKEN, List.of(ByteString.copyFromUtf8("m1"))))
                .designatingPayer("schedulePayer")
                .via(failingTxn),
            fileUpdate(APP_PROPERTIES)
                .payingWith(ADDRESS_BOOK_CONTROL)
  ...
  private HapiApiSpec scheduledMintWithInvalidTokenThrowsUnresolvableSigners() {
    return defaultHapiSpec("ScheduledMintWithInvalidTokenThrowsUnresolvableSigners")
        .given(overriding("scheduling.whitelist", "TokenMint"), cryptoCreate("schedulePayer"))
        .when(
            scheduleCreate(
                    "validSchedule",
    ...
    return defaultHapiSpec("ScheduledUniqueBurnFailsWithInvalidBatchSize")
        .given(
            ...
            scheduleCreate(
                    "validSchedule",
                    burnToken(
                        A_TOKEN, List.of(1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L, 10L, 11L, 12L, 13L)))
                .designatingPayer("schedulePayer")
                .via(failingTxn))
        .when(
    ...
    return defaultHapiSpec("ScheduledUniqueMintFailsWithInvalidMetadata")
        .given(
            ...
            tokenCreate(A_TOKEN)
                ...
                .tokenType(TokenType.NON_FUNGIBLE_UNIQUE)
                .initialSupply(0),
            scheduleCreate("validSchedule", mintToken(A_TOKEN, List.of(metadataOfLength(101))))
                .designatingPayer("schedulePayer")
                .via(failingTxn))
        .when(
            scheduleSign("validSchedule")
    ...
    return defaultHapiSpec("ScheduledUniqueBurnFailsWithInvalidNftId")
        ...
        .when(
            scheduleSign("validSchedule")
                ...
                .hasKnownStatus(SUCCESS))
        .then(
            getTxnRecord(failingTxn).scheduled().hasPriority(recordWith().status(INVALID_NFT_ID)));
  }
  ...
  private HapiApiSpec scheduledMintFailsWithInvalidAmount() {
    ...
    return defaultHapiSpec("ScheduledMintFailsWithInvalidAmount")
        .given(
            ...
            cryptoCreate("schedulePayer"),
            newKeyNamed("supplyKey"),
            tokenCreate(A_TOKEN).supplyKey("supplyKey").treasury("treasury").initialSupply(101),
            overriding("scheduling.whitelist", "TokenMint"),
  ...
  private HapiApiSpec scheduledXferFailingWithDeletedAccountPaysServiceFeeButNoImpact() {
    ...
    return defaultHapiSpec("ScheduledXferFailingWithDeletedTokenPaysServiceFeeButNoImpact")
        ...
        .when(
            scheduleCreate(
                    validSchedule, cryptoTransfer(moving(1, xToken).between(xTreasury, xCivilian)))
                .via(successTxn)
            ...
            getTxnRecord(successTxn).scheduled().logged().revealingDebitsTo(successFeesObs::set),
  ...
  private HapiApiSpec
    ...
    return defaultHapiSpec(
        ...
        .then(
            ...
            assertionsHold(
                (spec, opLog) ->
                    Assertions.assertEquals(successFeesObs.get(), failureFeesObs.get())));
  }
  ...
  private HapiApiSpec scheduledSubmitThatWouldFailWithInvalidTopicIdCannotBeScheduled() {
    ...
    return defaultHapiSpec("ScheduledSubmitThatWouldFailWithInvalidTopicIdCannotBeScheduled")
        ...
        .then(
            ...
            getTxnRecord("nothingShouldBeCreated").revealingDebitsTo(failureFeesObs::set).logged(),
            assertionsHold(
                (spec, opLog) ->
                    assertBasicallyIdentical(successFeesObs.get(), failureFeesObs.get(), 1.0)));
  }
  ...
  public HapiApiSpec executionTriggersOnceTopicHasSatisfiedSubmitKey() {
    ...
    return defaultHapiSpec("ExecutionTriggersOnceTopicHasNoSubmitKey")
        ...
        .when(
            ...
            scheduleSign(schedule))
        .then(getScheduleInfo(schedule).isExecuted(), getTopicInfo(mutableTopic).hasSeqNo(1L));
  }
  ...
  private boolean transferListCheck(
    ...
    AccountAmount receivingAmount =
        AccountAmount.newBuilder().setAccountID(receivingAccountID).setAmount(amount).build();
    ...
    boolean payerHasPaid =
        accountAmountList.stream()
            .anyMatch(a -> a.getAccountID().equals(payingAccountID) && a.getAmount() < 0);
