...
public class CharacterizationSuite extends HapiApiSuite {
  ...
  private HapiApiSpec payerThresholdFeeIsIncludedInRecordTotal() {
    ...
    return defaultFailingHapiSpec("PayerThresholdFeeIncludedInRecordTotal")
        .given(cryptoCreate("payer").balance(INITIAL_BALANCE).sendThreshold(LOW_SEND_THRESHOLD))
        .when(
            cryptoTransfer(tinyBarsFromTo(GENESIS, NODE, 1_000L))
                .payingWith("payer")
                .via("transferTxn"))
        .then(
            QueryVerbs.getAccountRecords("payer")
                .has(
                    inOrder(
                        recordWith()
                            .fee(
                                spec -> {
                                  ...
                                  allRunFor(spec, op);
                                  long balance =
                                      op.getResponse().getCryptogetAccountBalance().getBalance();
  ...
  private HapiApiSpec triplicateThresholdRecordsGenerated() {
    ...
    return defaultFailingHapiSpec("TriplicateThresholdRecordsGenerated")
        .given(cryptoCreate("payer").balance(INITIAL_BALANCE).sendThreshold(LOW_SEND_THRESHOLD))
        .when(
            cryptoTransfer(tinyBarsFromTo("payer", NODE, 1_000L))
                .payingWith("payer")
                .via("transferTxn"))
        .then(
            QueryVerbs.getAccountRecords("payer")
                .has(inOrder(recordWith(), recordWith(), recordWith()))
                .logged());
  }
