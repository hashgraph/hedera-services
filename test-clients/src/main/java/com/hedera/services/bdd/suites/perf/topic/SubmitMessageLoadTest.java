...
public class SubmitMessageLoadTest extends LoadTest {
  ...
  public static void main(String... args) {
    ...
    // Usage
    //
    // 1) create new topic with auto generated key, an topicSubmitKey.pem will ge exported for later
    // use
    // args: [size]
    //
  ...
  private static Supplier<HapiSpecOperation> opSupplier(PerfTestLoadSettings settings) {
    ...
    if (settings.getTotalAccounts() > 1) {
      ...
      senderId =
          String.format(
              "0.0.%d", TEST_ACCOUNT_STARTS_FROM + r.nextInt(settings.getTotalAccounts()));
      topicId =
          String.format(
              "0.0.%d",
