...
public class FeesAndRatesProvider {
  ...
  private ExchangeRate activeRates() {
    boolean useCurrent =
        Instant.now().getEpochSecond() < rateSet.getCurrentRate().getExpirationTime().getSeconds();
  ...
  private void readRateSet() throws Throwable {
    ...
    log.info(
        "The exchange rates from '" + f.getAbsolutePath() + "' are :: " + rateSetAsString(rateSet));
  }
  ...
  private void downloadFeeSchedule() throws Throwable {
    ...
    log.info(
        "The fee schedule covers " + feeSchedule.getTransactionFeeScheduleList().size() + " ops.");
  }
