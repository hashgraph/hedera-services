...
public class UpdateFile150 extends HapiApiSuite {
    ...
    return defaultHapiSpec("updateWithWrongFileID")
        ...
        .when(
            freeze()
                ...
                .startingIn(60)
                .seconds()
                .andLasting(FREEZE_LAST_MINUTES)
                .minutes()
  ...
  private HapiApiSpec missingFileHash() {
    ...
    return defaultHapiSpec("missingFileHash")
        .given(UtilVerbs.updateLargeFile(GENESIS, fileIDString, ByteString.copyFrom(new4k)))
        .when(
            freeze()
  ...
  private HapiApiSpec updateWithShortContent() {
    ...
    return defaultHapiSpec("updateWithShortContent")
        .given(
            withOpContext(
                (spec, opLog) -> {
                  spec.registry().saveBytes("updateFileContent", ByteString.copyFrom(new4k));
                }))
        .when(
  ...
  private HapiApiSpec updateWithLargeContent() {
    ...
    return defaultHapiSpec("updateWithLargeContent")
        .given(
            withOpContext(
                (spec, opLog) -> {
                  spec.registry().saveBytes("updateFileContent", ByteString.copyFrom(largeContent));
                }))
        .when(
  ...
  private HapiApiSpec notAllowedToDelete() {
    return defaultHapiSpec("notAllowedToDelete")
        .given()
        .when(fileDelete(fileIDString).hasPrecheck(ENTITY_NOT_ALLOWED_TO_DELETE))
        .then();
  }
