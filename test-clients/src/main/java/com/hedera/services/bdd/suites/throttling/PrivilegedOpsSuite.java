...
public class PrivilegedOpsSuite extends HapiApiSuite {
  ...
  private HapiApiSpec systemAccountUpdatePrivilegesAsExpected() {
    ...
    return defaultHapiSpec("SystemAccountUpdatePrivilegesAsExpected")
        ...
        .when(cryptoUpdate("0.0.88").payingWith(GENESIS).signedBy(GENESIS, "new88").key("new88"))
        .then(
            cryptoUpdate("0.0.2")
            ...
            cryptoUpdate("0.0.2")
                ...
                .hasPrecheck(AUTHORIZATION_FAILED),
            cryptoUpdate("0.0.2").receiverSigRequired(false).payingWith(GENESIS).signedBy(GENESIS),
            cryptoUpdate("0.0.2")
                .key(tmpTreasury)
                .payingWith(GENESIS)
                .signedBy(GENESIS)
            ...
            cryptoUpdate("0.0.88").payingWith(GENESIS).signedBy(GENESIS, "new88").key("new88"),
            cryptoUpdate("0.0.88")
                .key(GENESIS)
                .payingWith(SYSTEM_ADMIN)
                .signedBy(SYSTEM_ADMIN, GENESIS));
