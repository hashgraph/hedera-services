...
public class HapiApiClients {
  ...
  private final ManagedChannel createNettyChannel(NodeConnectInfo node, boolean useTls) {
    try {
      ...
      List<String> ciphers =
          Arrays.asList("TLS_AES_256_GCM_SHA384", "TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384");
      ...
      if (useTls) {
        ...
        channel =
            NettyChannelBuilder.forAddress(node.getHost(), node.getPort()).usePlaintext().build();
      }
      return channel;
    } catch (Exception e) {
