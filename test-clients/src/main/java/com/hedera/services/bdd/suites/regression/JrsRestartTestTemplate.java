 ...
 *
 * <p>ACCOUNTS - sender (balance = 1tℏ) - receiver (balance = 99tℏ, receiverSigRequired = true) -
 * treasury (treasury account for token jrsToken) - autoRenew (auto-renew account for topic
 * ofGeneralInterest)
 *
 * <p>TOPICS - ofGeneralInterest (has submit key)
 *
 * <p>TOKENS - jrsToken
 *
...
public class JrsRestartTestTemplate extends HapiApiSuite {
  ...
  private HapiApiSpec enableHSS() {
    return defaultHapiSpec("enableHSS")
        .given(
            // Directly puting this request in the customHapiSpec before expectedEntitiesExist()
            // doesn't work
  ...
  private HapiSpecOperation[] postRestartAccountValidation() {
    return new HapiSpecOperation[] {getAccountBalance(RECEIVER).hasTinyBars(99L)};
  }

  private HapiSpecOperation[] postRestartFileValidation() {
    return new HapiSpecOperation[] {getFileInfo(BYTECODE_FILE).hasMemo(BYTECODE_FILE_MEMO)};
  }
  ...
  private HapiSpecOperation[] postRestartScheduleValidation() {
    return new HapiSpecOperation[] {
      getAccountInfo(SENDER).has(accountWith().balance(1L)),
      getAccountInfo(RECEIVER).has(accountWith().balance(99L)),
      scheduleSign(PENDING_XFER).alsoSigningWith(RECEIVER),
      getAccountInfo(RECEIVER).has(accountWith().balance(100L)),
      getAccountInfo(SENDER).has(accountWith().balance(0L))
    };
  }
