  ...
  public MissingPaymentAsserts(Function<HapiApiSpec, Map.Entry<AccountID, Long>>... providers) {
    registerProvider(
        (spec, o) -> {
          ...
          Set<String> missing =
              Stream.of(providers).map(provider -> asSig(provider.apply(spec))).collect(toSet());
          ...
          actual.getAccountAmountsList().stream()
              .forEach(
                  entry -> {
                    String sig =
                        asSig(
                            new AbstractMap.SimpleEntry<>(entry.getAccountID(), entry.getAmount()));
    ...
    registerProvider(
        (spec, o) -> {
          ...
          Assertions.assertTrue(
              !transfers.getAccountAmountsList().isEmpty(), "Transfer list cannot be empty!");
        });
  }
}
        ...
        (spec, o) -> {
          ...
          Assertions.assertTrue(
              transfers.getAccountAmountsList().stream()
                  .anyMatch(
                      aa -> aa.getAmount() == -amount && aa.getAccountID().getAccountNum() == num),
