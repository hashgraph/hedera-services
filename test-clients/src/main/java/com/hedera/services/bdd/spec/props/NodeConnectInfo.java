...
public class NodeConnectInfo {
  ...
  public NodeConnectInfo(String inString) {
    ...
    int[] ports =
        Stream.of(aspects).filter(TxnUtils::isPortLiteral).mapToInt(Integer::parseInt).toArray();
    if (ports.length > 0) {
      port = ports[0];
    } else {
      port = DEFAULT_PORT;
    ...
    account =
        Stream.of(aspects)
            ...
            .findAny()
            .orElse(
                HapiPropertySource.asAccount(String.format("0.0.%d", NEXT_DEFAULT_ACCOUNT_NUM++)));
    host =
        Stream.of(aspects)
