...
public class HapiFileAppend extends HapiTxnOp<HapiFileAppend> {
  ...
  protected long feeFor(HapiApiSpec spec, Transaction txn, int numPayerKeys) throws Throwable {
    ...
    FeeCalculator.ActivityMetrics metricsCalc =
        (txBody, sigUsage) -> fileFees.getFileAppendTxFeeMatrices(txBody, expiry, sigUsage);
    return spec.fees()
  ...
  protected List<Function<HapiApiSpec, Key>> defaultSigners() {
    return List.of(
        spec -> spec.registry().getKey(effectivePayer(spec)), spec -> spec.registry().getKey(file));
  }
