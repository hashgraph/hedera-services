...
public class HapiGetReceipt extends HapiQueryOp<HapiGetReceipt> {
  ...
  protected void submitWith(HapiApiSpec spec, Transaction payment) {
    ...
    Query query =
        forgetOp ? Query.newBuilder().build() : txnReceiptQueryFor(txnId, requestDuplicates);
    response =
        spec.clients().getCryptoSvcStub(targetNodeFor(spec), useTls).getTransactionReceipts(query);
    if (verboseLoggingOn) {
  ...
  protected void assertExpectationsGiven(HapiApiSpec spec) {
    ...
    if (expectedDuplicateStatuses.isPresent()) {
      var duplicates =
          response.getTransactionGetReceipt().getDuplicateTransactionReceiptsList().stream()
