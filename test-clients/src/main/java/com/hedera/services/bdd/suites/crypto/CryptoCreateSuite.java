...
public class CryptoCreateSuite extends HapiApiSuite {
    ...
    return defaultHapiSpec("usdFeeAsExpected")
        ...
        .when(
            ...
            cryptoCreate("ok")
                ...
                .signedBy("civilian")
                .payingWith("civilian"))
        .then(validateChargedUsd("txn", v13PriceUsd));
  }
  ...
  private HapiApiSpec xferRequiresCrypto() {
    return defaultHapiSpec("XferRequiresCrypto")
        .given(
            ...
            contractCreate("multiAdmin").bytecode("bytecode").balance(1_234),
            contractCreate("multiNoAdmin").bytecode("bytecode").balance(1_234),
            cryptoCreate("misc"))
        .when()
        .then(
  ...
  private HapiApiSpec vanillaCreateSucceeds() {
    return defaultHapiSpec("VanillaCreateSucceeds")
        .given(cryptoCreate("testAccount").via("txn"), UtilVerbs.sleepFor(2000))
        .when()
  ...
  private HapiApiSpec requiresNewKeyToSign() {
    ...
    return defaultHapiSpec("RequiresNewKeyToSign")
        ...
        .then(
            ...
            cryptoCreate("test").keyShape(shape).sigControl(forKey("test", validSig)));
  }
  ...
  private HapiApiSpec createAnAccountInvalidThresholdKey() {
    ...
    KeyList invalidKeyList =
        KeyList.newBuilder().addKeys(randomKey1).addKeys(randomKey2).addKeys(shortKey).build();
