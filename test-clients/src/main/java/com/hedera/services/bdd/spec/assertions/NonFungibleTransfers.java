...
public class NonFungibleTransfers implements ErroringAssertsProvider<List<TokenTransferList>> {
							...
							try {
								Assertions.fail(
										"Unexpected change to balance of account "
												+ asAccountString(sender) + " for token "
												+ asTokenString(token) + " with serial number --> "
					...
					if (!accountNameLookup.containsKey(receiver)) {
						if (noOtherChangesTolerated) {
							try {
								Assertions.fail(
										"Unexpected change to balance of account "
												+ asAccountString(receiver) + " for token "
												+ asTokenString(token) + " with serial number --> "
												+ nftChange.getSerialNumber());
							} catch (Throwable t) {
								wrongNFTChanges.add(t);
					...
					try {
						Assertions.assertEquals(
								...
								"Wrong change in account '"
										+ senderName + "' for token '" + tokenName + "'");
					} catch (Throwable t) {
						wrongNFTChanges.add(t);
					}
	...
	private long expectedChange(String token, String sender, String receiver) {
		...
		if (expectations == null) {
			throw new IllegalStateException("No expected NFT change from sender '" + sender + "' to receiver '" + receiver + "' for " +
					"token '" + token +
					"'");
		}
