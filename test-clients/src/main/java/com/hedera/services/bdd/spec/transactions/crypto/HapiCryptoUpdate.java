...
public class HapiCryptoUpdate extends HapiTxnOp<HapiCryptoUpdate> {
            ...
            .<CryptoUpdateTransactionBody, CryptoUpdateTransactionBody.Builder>body(
                ...
                builder -> {
                  ...
                  updSigRequired.ifPresent(
                      u -> builder.setReceiverSigRequiredWrapper(BoolValue.of(u)));
                  if (useContractKey) {
                    builder.setKey(
                  ...
                  newAutoRenewPeriod.ifPresent(
                      p -> builder.setAutoRenewPeriod(Duration.newBuilder().setSeconds(p)));
                  entityMemo.ifPresent(
                      m -> builder.setMemo(StringValue.newBuilder().setValue(m).build()));
                  sendThreshold.ifPresent(
                      v ->
                          builder.setSendRecordThresholdWrapper(
                              UInt64Value.newBuilder().setValue(v).build()));
                  newExpiry.ifPresent(
                      l -> builder.setExpirationTime(Timestamp.newBuilder().setSeconds(l).build()));
                });
  ...
  protected long feeFor(HapiApiSpec spec, Transaction txn, int numPayerKeys) throws Throwable {
    try {
      ...
      return spec.fees()
          .forActivityBasedOp(HederaFunctionality.CryptoUpdate, metricsCalc, txn, numPayerKeys);
