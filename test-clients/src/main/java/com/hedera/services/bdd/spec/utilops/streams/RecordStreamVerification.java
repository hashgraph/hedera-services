...
public class RecordStreamVerification extends UtilOp {
  ...
  protected boolean submitOp(HapiApiSpec spec) throws Throwable {
    ...
    Map<String, List<File>> sigFilesAvail =
        uniqRecordFiles.stream()
            .flatMap(
                rcd ->
                    verifier.nodes().stream().map(account -> Path.of(recordsDirFor(account), rcd)))
            .collect(
                groupingBy(
                    this::basename,
                    mapping(
  ...
  private void checkOverallValidity(
    ...
    List<String> orderedRcdNames =
        sigFilesAvail.keySet().stream()
            ...
            .collect(toList());
    log.info(
        "Record file running hashes will be checked in chronological order: {}", orderedRcdNames);
  ...
  private void checkSigValidity(String record, List<File> sigs, NodeSignatureVerifier verifier) {
    ...
    if (majority == null) {
      setUnGood(
          "The nodes did not find majority agreement on the hash of record file '{}'!", record);
