...
public class RecordCreationSuite extends HapiApiSuite {
  ...
  private HapiApiSpec confirmNftToggleIsWorksThenReenable() {
    ...
    return defaultHapiSpec("ConfirmNftToggleIsWorksThenReenable")
        ...
        .then(
            ...
            cryptoTransfer(
                movingUnique(acceptedTokenAttempt, 1L).between(TOKEN_TREASURY, miscAccount)),
  ...
  private HapiApiSpec submittingNodeStillPaidIfServiceFeesOmitted() {
    ...
    return defaultHapiSpec("submittingNodeStillPaidIfServiceFeesOmitted")
        .given(
            cryptoTransfer(tinyBarsFromTo(GENESIS, "0.0.3", ONE_HBAR)).payingWith(GENESIS),
            cryptoCreate("payer"),
            ...
            sourcing(
                () ->
                    getAccountBalance("0.0.3")
                        .hasTinyBars(changeFromSnapshot("before", +feeObs.get().getNodeFee()))),
            sourcing(
                () ->
    ...
    return defaultHapiSpec("SubmittingNodeChargedNetworkFeeForLackOfDueDiligence")
        ...
        .then(
            sourcing(
                () ->
                    getAccountBalance("0.0.3")
                        .hasTinyBars(changeFromSnapshot("before", -feeObs.get().getNetworkFee()))),
            sourcing(
                () ->
                    getAccountBalance("0.0.98")
                        .hasTinyBars(
                ...
                () ->
                    getTxnRecord("txnId")
                        ...
                        .hasPriority(
                            recordWith()
                                .transfers(
                                    includingDeduction(() -> 3L, feeObs.get().getNetworkFee()))
            ...
            sourcing(
                () ->
                    fileUpdate(APP_PROPERTIES)
                        ...
                        .contents(
                            rawConfigPlus(
                                origPropContents.get(), "cache.records.ttl", "" + abbrevCacheTtl))
                        .payingWith(GENESIS)),
            cryptoCreate("payer"))
        .when(
            ...
            cryptoTransfer(tinyBarsFromTo(GENESIS, ADDRESS_BOOK_CONTROL, 1L)).payingWith(GENESIS),
            getAccountRecords("payer").has(inOrder()))
