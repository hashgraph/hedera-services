  ...
  private HapiApiSpec rekeyTreasury() {
    ...
    return customHapiSpec("RekeyTreasury")
        ...
        .given(
            ...
            keyFromPem(devKeyPemLoc).passphrase("passphrase").name(newTreasuryKey),
            /* Use this for creating a new key */
            //						keyFromLiteral(newTreasuryKey, hexedNewEd25519PrivateKey),
            withOpContext(
                (spec, opLog) -> {
                  spec.keys()
                      .exportSimpleKeyAsLegacyStartUpAccount(
                          newTreasuryKey, treasury, newTreasuryStartUpAccountLoc);
