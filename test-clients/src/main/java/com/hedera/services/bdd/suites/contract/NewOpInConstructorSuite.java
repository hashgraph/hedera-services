    ...
    return List.of(
        new HapiApiSpec[] {
          contractCreateWithNewOpInConstructor(
              "ChildStorage", ContractResources.CHILD_STORAGE_BYTECODE_PATH, 2),
          contractCreateWithNewOpInConstructor(
              "AbandoningParent", ContractResources.ABANDONING_PARENT_BYTECODE_PATH, 5)
        });
  }

  private HapiApiSpec contractCreateWithNewOpInConstructor(
      final String testName, final String filePath, final long expectedChildrenCount) {
    ...
    return defaultHapiSpec("ContractCreateWithNewOpInConstructor-" + testName)
        .given(fileCreate(bc).path(filePath))
        .when()
        .then(
            contractCreate(testName).bytecode(bc).via(txn),
            getContractInfo(testName).saveToRegistry(parent).logged(),
            getTxnRecord(txn).saveCreatedContractListToRegistry(testName).logged(),
            UtilVerbs.contractListWithPropertiesInheritedFrom(
                testName + "CreateResult", expectedChildrenCount, parent));
  }
