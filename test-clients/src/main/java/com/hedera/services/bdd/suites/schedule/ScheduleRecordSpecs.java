...
public class ScheduleRecordSpecs extends HapiApiSuite {
  ...
  public HapiApiSpec noFeesChargedIfTriggeredPayerIsUnwilling() {
    return defaultHapiSpec("NoFeesChargedIfTriggeredPayerIsUnwilling")
        ...
        .when(
            scheduleCreate("schedule", cryptoTransfer(tinyBarsFromTo(GENESIS, FUNDING, 1)).fee(1L))
  ...
  public HapiApiSpec canScheduleChunkedMessages() {
    ...
    return defaultHapiSpec("CanScheduleChunkedMessages")
        .given(
            cryptoCreate("payingSender").balance(ONE_HUNDRED_HBARS), createTopic(ofGeneralInterest))
        .when(
            withOpContext(
                (spec, opLog) -> {
  ...
  public HapiApiSpec executionTimeIsAvailable() {
    return defaultHapiSpec("ExecutionTimeIsAvailable")
        .given(
            cryptoCreate("payer"), cryptoCreate("receiver").receiverSigRequired(true).balance(0L))
        .when(
            scheduleCreate(
                    "tb", cryptoTransfer(tinyBarsFromTo("payer", "receiver", 1)).fee(ONE_HBAR))
                .savingExpectedScheduledTxnId()
                .payingWith("payer")
                .via("creation"),
  ...
  public HapiApiSpec deletionTimeIsAvailable() {
    return defaultHapiSpec("DeletionTimeIsAvailable")
        ...
        .when(
            scheduleCreate(
                    "ntb", cryptoTransfer(tinyBarsFromTo("payer", "receiver", 1)).fee(ONE_HBAR))
                .payingWith("payer")
                .adminKey("admin")
                .via("creation"),
  ...
  public HapiApiSpec allRecordsAreQueryable() {
    return defaultHapiSpec("AllRecordsAreQueryable")
        ...
        .when(
            scheduleCreate(
                    "twoSigXfer",
                    cryptoTransfer(tinyBarsFromTo("payer", "receiver", 1)).fee(ONE_HBAR))
                .logged()
                .savingExpectedScheduledTxnId()
                .payingWith("payer")
                .via("creation"),
