...
public class AccountAutoRenewalSuite extends HapiApiSuite {
    ...
    return defaultHapiSpec("AccountAutoRenewal")
        .given(
            fileUpdate(APP_PROPERTIES)
                .payingWith(GENESIS)
                .overridingProps(
                    enablingAutoRenewWith(
                        briefAutoRenew, 0, DEFAULT_HIGH_SPIN_SCAN_COUNT, DEFAULT_HIGH_TOUCH_COUNT)),
            cryptoCreate(autoRenewedAccount).autoRenewSecs(briefAutoRenew).balance(initialBalance),
            getAccountInfo(autoRenewedAccount).saveToRegistry(autoRenewedAccount).logged())
        .when(
   ...
   *
   * <p>If run against a network that has existing funded accounts with very low auto-renew periods,
   * this test is just a minimal sanity check.
   */
    ...
    return defaultHapiSpec("MaxNumberOfEntitiesToRenewOrDeleteWorks")
        .given(
            fileUpdate(APP_PROPERTIES)
                .payingWith(GENESIS)
                .overridingProps(
                    enablingAutoRenewWith(briefAutoRenew, 0, DEFAULT_HIGH_SPIN_SCAN_COUNT, 2)),
  ...
  /**
   * Mostly useful to run from a blank state where the only accounts that exist are those created in
   * the "given" clause of this spec.
   *
   * <p>If run against a network that has existing funded accounts with very low auto-renew periods,
   * this test is just a minimal sanity check.
   */
                ...
                (spec, opLog) -> {
                  final HapiSpecOperation[] subOps =
                      IntStream.range(0, abbrevMaxToScan + 1)
                          .mapToObj(
                              i ->
                                  getAccountBalance(accountName.apply(i))
                  ...
                  final long numRemoved =
                      Stream.of(subOps)
                          .map(
                              op ->
                                  ((HapiQueryOp<HapiGetAccountBalance>) op)
                                      .getResponse()
                                      .getCryptogetAccountBalance()
                                      .getHeader()
                                      .getNodeTransactionPrecheckCode())
  ...
  private HapiApiSpec freezeAtTheEnd() {
    return defaultHapiSpec("freezeAtTheEnd")
        .given()
        .when(freeze().startingIn(0).minutes().andLasting(2).minutes().payingWith(GENESIS))
        .then(
