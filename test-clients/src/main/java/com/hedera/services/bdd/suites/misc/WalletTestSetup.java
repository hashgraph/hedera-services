...
public class WalletTestSetup extends HapiApiSuite {
	...
	static String mnemonic = "girl adjust asset interest razor thrive " +
...
//								.name("oldKey")
//								.linkedTo("0.0.39280"),
//						keyFromMnemonic(DETERMINISTIC_WALLET, stagingMnemonic).linkedTo("0.0.1115")
	...
	private HapiApiSpec fundDeterministicWallet() {
		...
		return customHapiSpec("FundDeterministicWallet")
				.withProperties(Map.of(
						...
						cryptoTransfer(HapiCryptoTransfer.tinyBarsFromTo(GENESIS, DETERMINISTIC_WALLET, amount))
				).then(
						QueryVerbs.getAccountBalance(GENESIS)
				);
	}
	...
	private HapiApiSpec reviewDeterministicWallet() {
		return customHapiSpec("ReviewDeterministicWallet")
				.withProperties(Map.of(
						...
						cryptoTransfer(HapiCryptoTransfer.tinyBarsFromTo(DETERMINISTIC_WALLET, GENESIS, 1))
								.payingWith(DETERMINISTIC_WALLET)
				).then(
						QueryVerbs.getAccountInfo("0.0.1113")
								.logged()
								.plusCustomLog((info, opLog) ->
										opLog.info(Hex.encodeHexString(info.getKey().getEd25519().toByteArray())))
				);
	}
