...
public class HapiGetFileContents extends HapiQueryOp<HapiGetFileContents> {
  ...
  protected void submitWith(HapiApiSpec spec, Transaction payment) throws Throwable {
    ...
    if (snapshotPath.isPresent() || readablePath.isPresent()) {
      try {
        if (snapshotPath.isPresent()) {
          if (saveIn4kChunks) {
            ...
            while (i < bytes.length) {
              File snapshotFile =
                  new File(String.format("part%d-%s", suffix++, snapshotPath.get()));
        ...
        if (readablePath.isPresent()) {
          ...
          log.info(
              "Saved parsed contents of '" + fileName + "' to " + readableFile.getAbsolutePath());
        }
      } catch (Exception e) {
    ...
    if (registryEntry.isPresent()) {
      spec.registry()
          .saveBytes(
              registryEntry.get(), response.getFileGetContents().getFileContents().getContents());
    }
