...
public class OneOffValidation extends HapiApiSuite {
  ...
  private static final String STAGINGLG_NODES =
      "35.237.182.66:0.0.3,35.245.226.22:0.0.4,34.68.9.203:0.0.5,"
  ...
  private static final String STAGINGLG_BOOTSTRAP_ACCOUNT_LOC =
      "src/main/resource/MainnetStartupAccount.txt";

  private static final String MAINNET_NODES =
      "35.237.200.180:0.0.3,35.186.191.247:0.0.4,"
  ...
  private static final String MAINNET_BOOTSTRAP_ACCOUNT_LOC =
      "src/main/resource/MainnetStartupAccount.txt";

  private static final String TESTNET_NODES =
      "34.94.106.61:0.0.3,35.196.34.86:0.0.4," + "35.194.75.187:0.0.5,34.82.241.226:0.0.6";
  ...
  private static final String TESTNET_BOOTSTRAP_ACCOUNT_LOC =
      "src/main/resource/TestnetStartupAccount.txt";
  private static final String TESTNET_CIVILIAN_BOOTSTRAP_ACCOUNT_LOC =
      "src/main/resource/TestnetCivilianStartupAccount.txt";
  ...
  private HapiApiSpec createAContract() {
    return defaultHapiSpec("CreateAContract")
        .given(fileCreate("bytecode").path(ContractResources.MULTIPURPOSE_BYTECODE_PATH))
        .when(contractCreate("something").bytecode("bytecode").via("createTxn"))
        .then(getTxnRecord("createTxn").logged());
  }
  ...
  private HapiApiSpec checkAppProperties() {
    ...
    return customHapiSpec("CheckAppProperties")
        .withProperties(
            Map.of(
  ...
  private HapiApiSpec createAnAccount() {
    return customHapiSpec("CreateAnAccount")
        .withProperties(
            Map.of(
                "nodes", NODES,
        ...
        .when()
        .then(cryptoCreate("bootstrap").key("misc"));
  }

  private HapiApiSpec xferWithTls() {
    return customHapiSpec("tryWithTls")
        .withProperties(
            Map.of(
                "nodes", NODES,
                "default.payer", BOOTSTRAP_ACCOUNT,
        ...
        .given()
        .when()
        .then(cryptoTransfer(tinyBarsFromTo(GENESIS, ADDRESS_BOOK_CONTROL, 1)));
  }

  private HapiApiSpec bootstrapBalanceCheck() {
    final String DEFAULT_DIR = "./system-files";

    return customHapiSpec("balanceCheck")
        .withProperties(
            Map.of(
                "nodes",
                NODES,
                "default.payer",
                BOOTSTRAP_ACCOUNT,
                "startupAccounts.path",
                BOOTSTRAP_ACCOUNT_LOC,
                "client.feeSchedule.fromDisk",
                "true",
                "client.feeSchedule.path",
                path(DEFAULT_DIR, "feeSchedule.bin"),
                "client.exchangeRates.fromDisk",
                "true",
                "client.exchangeRates.path",
                path(DEFAULT_DIR, "exchangeRates.bin")))
        .given()
        .when()
        .then(QueryVerbs.getAccountInfo(GENESIS).logged());
  }
