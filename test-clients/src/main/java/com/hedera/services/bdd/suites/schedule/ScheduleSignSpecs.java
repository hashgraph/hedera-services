...
public class ScheduleSignSpecs extends HapiApiSuite {
  ...
  private HapiApiSpec changeInNestedSigningReqsRespected() {
    ...
    var sigOne =
        senderShape.signedWith(sigs(sigs(OFF, OFF, ON), sigs(OFF, OFF, OFF), sigs(OFF, OFF, OFF)));
    var sigTwo =
        senderShape.signedWith(sigs(sigs(OFF, OFF, OFF), sigs(ON, ON, ON), sigs(OFF, OFF, OFF)));
    var firstSigThree =
        senderShape.signedWith(sigs(sigs(OFF, OFF, OFF), sigs(OFF, OFF, OFF), sigs(ON, OFF, OFF)));
    var secondSigThree =
        senderShape.signedWith(sigs(sigs(OFF, OFF, OFF), sigs(OFF, OFF, OFF), sigs(ON, ON, OFF)));
    String sender = "X",
        receiver = "Y",
        schedule = "Z",
        senderKey = "sKey",
  ...
  private HapiApiSpec nestedSigningReqsWorkAsExpected() {
    ...
    var sigOne =
        senderShape.signedWith(sigs(sigs(OFF, OFF, ON), sigs(OFF, OFF, OFF), sigs(OFF, OFF, OFF)));
    var sigTwo =
        senderShape.signedWith(sigs(sigs(OFF, OFF, OFF), sigs(OFF, ON, OFF), sigs(OFF, OFF, OFF)));
    var sigThree =
        senderShape.signedWith(sigs(sigs(OFF, OFF, OFF), sigs(OFF, OFF, OFF), sigs(ON, OFF, OFF)));
  ...
  public HapiApiSpec sharedKeyWorksAsExpected() {
    return defaultHapiSpec("RequiresSharedKeyToSignBothSchedulingAndScheduledTxns")
        .given(newKeyNamed("sharedKey"), cryptoCreate("payerWithSharedKey").key("sharedKey"))
        .when(
            scheduleCreate(
                    "deferredCreation",
  ...
  public HapiApiSpec okIfAdminKeyOverlapsWithActiveScheduleKey() {
    ...
    return defaultHapiSpec("OkIfAdminKeyOverlapsWithActiveScheduleKey")
        ...
        .when(
            scheduleCreate(
                    "deferredXfer",
                    cryptoTransfer(tinyBarsFromTo("sender", ADDRESS_BOOK_CONTROL, 1)))
                .adminKey(adminKey))
        .then();
  }
  ...
  public HapiApiSpec retestsActivationOnSignWithEmptySigMap() {
    return defaultHapiSpec("RetestsActivationOnCreateWithEmptySigMap")
        ...
        .when(
            ...
            scheduleCreate(
                    "deferredFall",
                    cryptoTransfer(tinyBarsFromTo("sender", FUNDING, 1)).fee(ONE_HBAR))
                .alsoSigningWith("a"),
  ...
  public HapiApiSpec signFailsDueToDeletedExpiration() {
    ...
    return defaultHapiSpec("SignFailsDueToDeletedExpiration")
        .given(
            sleepFor(
                SCHEDULE_EXPIRY_TIME_MS), // await any other scheduled expiring entity to expire
            cryptoCreate("sender").balance(1L),
            cryptoCreate("receiver").balance(0L).receiverSigRequired(true))
        .when(
