...
public class SysFilesUpdate extends HapiApiSuite {
  ...
  static final long TINYBARS_PER_HBAR = 100_000_000L;
  static final long DEFAULT_FEE_IN_HBARS = 100L;

  static final String DEV_TARGET_DIR = "/Users/tinkerm/Dev/misc/tools/scratch";
  ...
  static EnumSet<SystemFile> BOOK_FILES =
      EnumSet.of(SystemFile.ADDRESS_BOOK, SystemFile.NODE_DETAILS);

  static EnumMap<SystemFile, String> humanReadableFileNames =
      new EnumMap<>(
          Map.of(
  ...
  static EnumMap<SystemFile, String> protoFileNames =
      new EnumMap<>(
          Map.of(
  ...
  static EnumMap<SystemFile, String> registryNames =
      new EnumMap<>(
          Map.of(
  ...
  static EnumMap<SystemFile, Function<BookEntryPojo, Stream<NodeAddress>>> updateConverters =
      new EnumMap<>(
          Map.of(
  ...
  private static void writeDefaultSysFilesPem() throws IOException {
    var pemOut =
        java.nio.file.Files.newOutputStream(java.nio.file.Paths.get(DEFAULT_SYSFILE_PEM_LOC));
        ...
        .when(
            withOpContext(
                (spec, opLog) -> {
                  ...
                  spec.keys()
                      .exportSimpleKey(
                          String.format("new-account%s.pem", accountToReKey), "replacementKey");
                  var loc = String.format("new-account%s-passphrase.txt", accountToReKey);
                  try (BufferedWriter out =
                      Files.newWriter(new File(loc), Charset.defaultCharset())) {
  ...
  private HapiApiSpec downloadTargetAsHumanFriendly() {
    ...
    String humanReadableFile =
        qualifiedPath(nodesOverride, humanReadableFileNames.get(target), true);
  ...
  private HapiApiSpec updateTargetFromHumanReadable() {
    ...
    return customHapiSpec(String.format("Update-%s-FromHumanReadable", target))
        ...
        .given(
            withOpContext(
                (spec, opLog) -> {
                  var sysFileOcKeystore =
                      SpecUtils.asOcKeystore(
                          new File(DEFAULT_SYSFILE_PEM_LOC), KeyFactory.PEM_PASSPHRASE);
                  var sysFileKey =
                      Key.newBuilder()
                          .setEd25519(
                              ByteString.copyFrom(
                          ...
                          .build();
                  opLog.info(
                      "Will ensure default public key :: "
                          + sysFileKey
                          + " (hex = "
                          + sysFileOcKeystore.getPublicKeyAbyteStr()
                          + ")");
            ...
            withOpContext(
                (spec, opLog) -> {
                  if (toUpload.length < (6 * 1024)) {
                    var singleOp =
                        fileUpdate(registryNames.get(target))
                    ...
                    while (n < toUpload.length) {
                      ...
                      if (n == 0) {
                        ...
                        subOp =
                            fileAppend(registryNames.get(target))
                                .fee(feeToOffer())
                                .content(thisChunk)
                                .signedBy(DEFAULT_PAYER, DEFAULT_SYSFILE_KEY)
                                .hasKnownStatusFrom(SUCCESS, FEE_SCHEDULE_FILE_PART_UPLOADED);
  ...
  private String envNodes() {
    String host = Optional.ofNullable(System.getenv("TARGET_NODE")).orElse("127.0.0.1");
    ...
    try {
      ...
      if (action == Action.REPLACE_KEY) {
        ...
        if (args.length < 5) {
          System.out.println(
              String.format(
                  "Args (%s) don't include a PEM to update at position 5!", Arrays.toString(args)));
          System.exit(1);
        }
        ...
        if (Optional.ofNullable(reKeyAccountPemPassphrase).orElse("").length() == 0) {
          System.out.println(
              String.format(
                  "Env var '%s' doesn't have a non-empty passphrase!", Arrays.toString(args)));
          System.exit(1);
        }
      } else {
        target = SystemFile.ADDRESS_BOOK;
  ...
  private static void b64EncodePem() {
    ...
    var baseName =
        startupAccountsPathOverride.substring(0, startupAccountsPathOverride.length() - 4);
  ...
  private static void dumpAvailPubKeys() throws IOException {
    ...
    System.out.println(
        "When updating this address book, use '!' in the JSON rsaPubKey field to insert "
    ...
    for (Path pubKeyLoc : allPubKeyPaths()) {
      ...
      System.out.println(
          String.format(
              "From '%s', %s",
              pubKeyLoc, BookEntryPojo.asHexEncodedDerPubKey(RSA_PUBKEYS_DIR, nodeId)));
    }
  }
  ...
  private static void dumpAvailCerts() throws IOException {
    ...
    System.out.println(
        "When updating this address book, use '!' in the JSON certHash field to insert "
    ...
    for (Path crtLoc : allCertFiles()) {
      ...
      System.out.println(
          String.format(
              "From '%s', %s", crtLoc, BookEntryPojo.asHexEncodedSha384HashFor(CRTS_DIR, nodeId)));
    }
  }
