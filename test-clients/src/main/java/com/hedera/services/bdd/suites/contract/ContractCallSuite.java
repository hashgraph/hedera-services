...
public class ContractCallSuite extends HapiApiSuite {
  ...
  protected List<HapiApiSpec> getSpecsInSuite() {
    return allOf(positiveSpecs(), negativeSpecs(), Arrays.asList(fridayThe13thSpec()));
  }

  List<HapiApiSpec> negativeSpecs() {
    return Arrays.asList(
        insufficientFee(), insufficientGas(), invalidContract(), invalidAbi(), nonPayable());
  }
  ...
  private HapiApiSpec multipleSelfDestructsAreSafe() {
    return defaultHapiSpec("MultipleSelfDestructsAreSafe")
        ...
        .when(contractCall("fuse", ContractResources.LIGHT_ABI).via("lightTxn"))
    ...
    return defaultHapiSpec("FridayThe13thSpec")
        ...
        .then(
            ...
            contractUpdate("contract")
                ...
                .hasKnownStatus(INVALID_SIGNATURE),
            contractUpdate("contract").payingWith("payer").newKey("newAdminKey"),
            contractUpdate("contract")
                .payingWith("payer")
            ...
            getContractInfo("contract")
                .payingWith("payer")
                .logged()
                .has(contractWith().solidityAddress("contract").memo(NEW_MEMO).expiry(newExpiry)),
            contractUpdate("contract").payingWith("payer").newMemo(BETTER_MEMO),
            getContractInfo("contract")
                .payingWith("payer")
                .logged()
            ...
            contractDelete("immutableContract")
                .payingWith("payer")
                .hasKnownStatus(MODIFYING_IMMUTABLE_CONTRACT),
            contractUpdate("immutableContract").payingWith("payer").newExpiryTime(betterExpiry),
            ...
            contractDelete("contract")
                ...
                .signedBy("payer")
                .hasKnownStatus(INVALID_SIGNATURE),
            contractDelete("contract").payingWith("payer").hasKnownStatus(SUCCESS));
  }
  ...
  HapiApiSpec depositSuccess() {
    return defaultHapiSpec("DepositSuccess")
        ...
        .then(
            getTxnRecord("payTxn")
                .hasPriority(recordWith().contractCallResult(resultWith().logs(inOrder()))));
  }
            ...
            withOpContext(
                (spec, opLog) -> {
                  for (int i = 0; i < 10; i++) {
                    ...
                    var subOp2 =
                        contractCall(
                                "payableContract", ContractResources.DEPOSIT_ABI, depositAmount)
                            .via("payTxn")
                            .sending(depositAmount);
                    var subOp3 =
                        getAccountBalance("payableContract")
  ...
  HapiApiSpec depositDeleteSuccess() {
    ...
    return defaultHapiSpec("DepositDeleteSuccess")
        ...
        .then(
            ...
            getAccountBalance("beneficiary").hasTinyBars(initBalance + depositAmount));
  }
  ...
  HapiApiSpec payableSuccess() {
    return defaultHapiSpec("PayableSuccess")
        .given(
            fileCreate("payableBytecode").path(ContractResources.PAYABLE_CONTRACT_BYTECODE_PATH),
            contractCreate("payableContract").bytecode("payableBytecode").adminKey(THRESHOLD))
        .when(contractCall("payableContract").via("payTxn").sending(depositAmount))
        .then(
            getTxnRecord("payTxn")
                .hasPriority(
                    recordWith()
                        .contractCallResult(
                            resultWith().logs(inOrder(logWith().longAtBytes(depositAmount, 24))))));
  }
  ...
  HapiApiSpec insufficientGas() {
    return defaultHapiSpec("InsufficientGas")
        ...
        .when()
        .then(
            contractCall("simpleStorage", ContractResources.CREATE_CHILD_ABI)
