...
public class DuplicateManagementTest extends HapiApiSuite {
  ...
  private HapiApiSpec hasExpectedDuplicates() {
    return defaultHapiSpec("HasExpectedDuplicates")
        ...
        .when(
            uncheckedSubmit(cryptoCreate("repeated").payingWith("civilian").txnId("txnId")),
            uncheckedSubmit(cryptoCreate("repeated").payingWith("civilian").txnId("txnId")),
            uncheckedSubmit(cryptoCreate("repeated").payingWith("civilian").txnId("txnId")),
            sleepFor(1_000L))
        .then(
            ...
            withOpContext(
                (spec, opLog) -> {
                  var cheapGet = getTxnRecord("cheapTxn").assertingNothingAboutHashes().logged();
                  var costlyGet = getTxnRecord("costlyTxn").assertingNothingAboutHashes().logged();
  ...
  private HapiApiSpec usesUnclassifiableIfNoClassifiableAvailable() {
    return defaultHapiSpec("UsesUnclassifiableIfNoClassifiableAvailable")
        ...
        .when(
            uncheckedSubmit(
                cryptoCreate("nope").payingWith("civilian").txnId("txnId").signedBy("wrongKey")),
            sleepFor(1_000L))
        .then(
  ...
  private HapiApiSpec classifiableTakesPriorityOverUnclassifiable() {
    return defaultHapiSpec("ClassifiableTakesPriorityOverUnclassifiable")
        ...
        .when(
            uncheckedSubmit(
                    cryptoCreate("nope").txnId("txnId").payingWith("civilian").setNode("0.0.4"))
                .logged(),
            uncheckedSubmit(
                cryptoCreate("sure").txnId("txnId").payingWith("civilian").setNode("0.0.3")),
            sleepFor(1_000L))
        .then(
            getReceipt("txnId")
                .andAnyDuplicates()
