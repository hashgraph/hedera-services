...
public class TokenCreateSpecs extends HapiApiSuite {
  ...
  public HapiApiSpec autoRenewValidationWorks() {
    return defaultHapiSpec("AutoRenewValidationWorks")
        .given(cryptoCreate("autoRenew").balance(0L), cryptoCreate("deletingAccount").balance(0L))
        .when(
        ...
        .then(
            UtilVerbs.withOpContext(
                (spec, opLog) -> {
                  ...
                  var timestamp =
                      createTxn.getResponseRecord().getConsensusTimestamp().getSeconds();
  ...
  public HapiApiSpec creationSetsCorrectExpiry() {
    return defaultHapiSpec("CreationSetsCorrectExpiry")
        .given(cryptoCreate(TOKEN_TREASURY).balance(0L), cryptoCreate("autoRenew").balance(0L))
        .when(
            tokenCreate("primary")
                .autoRenewAccount("autoRenew")
                .autoRenewPeriod(A_HUNDRED_SECONDS)
    ...
    return defaultHapiSpec("CreationValidatesName")
        .given(
            ...
            recordSystemProperty(
                "tokens.maxTokenNameUtf8Bytes", Integer::parseInt, maxUtf8Bytes::set))
        .when()
        .then(
  ...
  public HapiApiSpec numAccountsAllowedIsDynamic() {
    ...
    return defaultHapiSpec("NumAccountsAllowedIsDynamic")
        .given(newKeyNamed("admin"), cryptoCreate(TOKEN_TREASURY).balance(0L))
        .when(
            fileUpdate(APP_PROPERTIES)
                .payingWith(ADDRESS_BOOK_CONTROL)
                .overridingProps(Map.of("tokens.maxPerAccount", "" + MONOGAMOUS_NETWORK)),
            tokenCreate("primary").adminKey("admin").treasury(TOKEN_TREASURY),
        ...
        .then(
            fileUpdate(APP_PROPERTIES)
                .payingWith(ADDRESS_BOOK_CONTROL)
                .overridingProps(Map.of("tokens.maxPerAccount", "" + ADVENTUROUS_NETWORK)),
