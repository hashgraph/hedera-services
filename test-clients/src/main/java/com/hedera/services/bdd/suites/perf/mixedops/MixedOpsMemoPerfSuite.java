        ...
        () ->
            new HapiSpecOperation[] {
              cryptoCreate("testAccount" + createdSoFar.getAndIncrement())
                  ...
                  .entityMemo(
                      new String(
                          TxnUtils.randomUtf8Bytes(memoLength.getAsInt()), StandardCharsets.UTF_8))
                  .noLogging()
    ...
    return defaultHapiSpec("RunMixedMemoOps")
        ...
        .when(
            ...
            createTopic(TARGET_TOPIC + "Info")
                ...
                .topicMemo(TOPIC_MEMO)
                .logged(),
            tokenCreate(TARGET_TOKEN).entityMemo(TOKEN_MEMO).payingWith(GENESIS).logged(),
            tokenCreate(TARGET_TOKEN + "Info").entityMemo(TOKEN_MEMO).payingWith(GENESIS).logged())
