...
public class RegressionProviderFactory {
  ...
  public static Function<HapiApiSpec, OpProvider> factoryFrom(Supplier<String> resource) {
    return spec -> {
      ...
      var keys =
          new RegistrySourcedNameProvider<>(Key.class, spec.registry(), new RandomSelector());
      var files =
          new RegistrySourcedNameProvider<>(FileID.class, spec.registry(), new RandomSelector());
      var tokens =
          new RegistrySourcedNameProvider<>(TokenID.class, spec.registry(), new RandomSelector());
      var tokenRels =
      ...
      var allTopics =
          new RegistrySourcedNameProvider<>(TopicID.class, spec.registry(), new RandomSelector());
      var unstableTopics =
          new RegistrySourcedNameProvider<>(
              TopicID.class,
              spec.registry(),
      ...
      return new BiasedDelegatingProvider()
          ...
          .withOp(
              new RandomAccountInfo(allAccounts), intPropOrElse("randomAccountInfo.bias", 0, props))
          .withOp(
          ...
          .withOp(
              new RandomTokenDeletion(tokens), intPropOrElse("randomTokenDeletion.bias", 0, props))
          .withOp(
              new RandomTokenTransfer(tokenRels),
              intPropOrElse("randomTokenTransfer.bias", 0, props))
          .withOp(
              new RandomTokenFreeze(tokenRels), intPropOrElse("randomTokenFreeze.bias", 0, props))
          .withOp(
