...
public abstract class HapiSpecOperation {
  ...
  public Optional<Throwable> execFor(HapiApiSpec spec) {
    ...
    try {
      ...
      if (unavailableNode && t.getMessage().startsWith("UNAVAILABLE")) {
        log.info(
            "Node {} is unavailable as expected!", HapiPropertySource.asAccountString(node.get()));
  ...
  protected Transaction finalizedTxn(
      HapiApiSpec spec, Consumer<TransactionBody.Builder> opDef, boolean forCost) throws Throwable {
  ...
  private Transaction finalizedTxnFromTxnWithBodyBytesAndSigMap(
    ...
    SignedTransaction signedTransaction =
        SignedTransaction.newBuilder().setBodyBytes(bodyByteString).setSigMap(sigMap).build();
