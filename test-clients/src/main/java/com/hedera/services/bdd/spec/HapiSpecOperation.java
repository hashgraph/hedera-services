...
public abstract class HapiSpecOperation {
	...
	protected Transaction finalizedTxn(final HapiApiSpec spec, final Consumer<TransactionBody.Builder> opDef)
			throws Throwable {
		return finalizedTxn(spec, opDef, false);
	}

	protected Transaction finalizedTxn(
			final HapiApiSpec spec, final Consumer<TransactionBody.Builder> opDef, final boolean forCost)
			throws Throwable {
		...
		if (fee.isPresent()) {
			...
			final Key payerKey = spec.registry().getKey(payer.orElse(spec.setup().defaultPayerName()));
	...
	private Transaction getSigned(final HapiApiSpec spec, final Transaction.Builder builder, final List<Key> keys)
			throws Throwable {
