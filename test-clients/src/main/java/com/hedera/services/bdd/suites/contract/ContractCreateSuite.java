...
public class ContractCreateSuite extends HapiApiSuite {
  ...
  private List<HapiApiSpec> positiveTests() {
    return Arrays.asList(createsVanillaContract());
  }
  ...
  private HapiApiSpec createsVanillaContract() {
    return defaultHapiSpec("CreatesVanillaContract")
        .given(fileCreate("contractFile").path(ContractResources.VALID_BYTECODE_PATH))
        .when()
        .then(contractCreate("testContract").bytecode("contractFile").hasKnownStatus(SUCCESS));
  }
  ...
  private HapiApiSpec rejectsInvalidBytecode() {
    return defaultHapiSpec("RejectsInvalidBytecode")
        .given(fileCreate("contractFile").path(ContractResources.INVALID_BYTECODE_PATH))
        .when()
        .then(
            contractCreate("testContract")
                .bytecode("contractFile")
