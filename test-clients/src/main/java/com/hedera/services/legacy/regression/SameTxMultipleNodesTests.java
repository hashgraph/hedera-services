...
public class SameTxMultipleNodesTests extends BaseFeeTests {
	...
	public void smartContractCreateFeeTest_90Days() throws Throwable {
		...
		for (int i = 0; i < nummberOfNodesToTest; i++) {
			...
			sampleStorageTransaction[i] = createContractTransaction(crAccount, nodeAccounts[i], timestamp,
					"ContractCreate-" + i);
		...
		for (int i = 0; i < nummberOfNodesToTest; i++) {
			...
			if ("OK".equalsIgnoreCase(
					response[i].getNodeTransactionPrecheckCode().name()) && statusCode == ResponseCodeEnum.SUCCESS) {
				successCount++;
			}
		}
	...
	private ResponseCodeEnum printTxDetails(Transaction transaction, TransactionResponse response,
	...
	private Transaction createContractTransaction(AccountID crAccount, AccountID nodeID, Timestamp timestamp,
		...
		Transaction createContractTransaction = createContractWithTimestamp(crAccount, storageFileId, nodeID,
				autoRenewPeriod, 1000000L,
