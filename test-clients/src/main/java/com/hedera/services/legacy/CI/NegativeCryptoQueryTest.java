...
//
  ...
  accountInfoResponse = TestHelper.getCryptoGetAccountInfo(stub, newlyCreateAccountId1,
   ...
   /* log.info("Add claim api by providing wrong account id...");
    Timestamp timestamp = RequestBuilder
        .getTimestamp(Instant.now(Clock.systemUTC()).minusSeconds(13));
    TransactionBody.Builder txBodyBuilder = RequestBuilder.getTxBodyBuilder(FeeClient.getMaxFee(),
        timestamp, RequestBuilder.getDuration(30), true, "add claim",
        payerAccount, defaultNodeAccount);
    byte[] hash = MessageDigest.getInstance("SHA-384").digest("earth is round".getBytes());
    TransactionResponse transactionResponse = TestHelper
        .addClaim(txBodyBuilder, Collections.singletonList(genesisPrivateKey),
            accountIdBuild, KeyList.getDefaultInstance(), hash, stub);
    Assert.assertNotNull(transactionResponse);
    Assert.assertEquals(ResponseCodeEnum.OK, transactionResponse.getNodeTransactionPrecheckCode());
    TransactionReceipt txReceipt = TestHelper.getTxReceipt(txBodyBuilder.getTransactionID(), stub);
    Assert.assertEquals(ResponseCodeEnum.INVALID_ACCOUNT_ID, txReceipt.getStatus());
    log.info("assertion completed...");
    log.info("--------------------------------------");

    log.info("Add claim api by providing wrong Payer account id...");
    timestamp = RequestBuilder
        .getTimestamp(Instant.now(Clock.systemUTC()).minusSeconds(13));
    txBodyBuilder = RequestBuilder.getTxBodyBuilder(FeeClient.getMaxFee(),
        timestamp, RequestBuilder.getDuration(30), true, "add claim",
        accountIdBuild, defaultNodeAccount);
    transactionResponse = TestHelper
        .addClaim(txBodyBuilder, Collections.singletonList(genesisPrivateKey),
            newlyCreateAccountId1, KeyList.getDefaultInstance(), hash, stub);
    Assert.assertNotNull(transactionResponse);
    Assert.assertEquals(ResponseCodeEnum.PAYER_ACCOUNT_NOT_FOUND,
    ...
    log.info("Add claim api by providing wrong private key of payer account...");
    timestamp = RequestBuilder
        .getTimestamp(Instant.now(Clock.systemUTC()).minusSeconds(13));
    txBodyBuilder = RequestBuilder.getTxBodyBuilder(FeeClient.getMaxFee(),
        timestamp, RequestBuilder.getDuration(30), true, "add claim",
        payerAccount, defaultNodeAccount);
    transactionResponse = TestHelper
        .addClaim(txBodyBuilder, Collections.singletonList(keyPair.getPrivate()),
            newlyCreateAccountId1, KeyList.getDefaultInstance(), hash, stub);
    Assert.assertNotNull(transactionResponse);
    Assert.assertEquals(ResponseCodeEnum.INVALID_SIGNATURE,
        transactionResponse.getNodeTransactionPrecheckCode());
    ...
    log.info("Get account claim api by providing wrong account id");
    Response claimResponse = TestHelper.getAccountClaim(stub, accountIdBuild, hash,
        payerAccount, genesisPrivateKey, defaultNodeAccount);
    Assert.assertNotNull(claimResponse);
    Assert.assertTrue(claimResponse.hasCryptoGetClaim());
    Assert.assertEquals(ResponseCodeEnum.INVALID_ACCOUNT_ID,
        claimResponse.getCryptoGetClaim().getHeader().getNodeTransactionPrecheckCode());
    log.info("assertion completed...");
    log.info("--------------------------------------");

    log.info("Get account claim api by providing wrong payer account id");
    claimResponse = TestHelper.getAccountClaim(stub, newlyCreateAccountId1, hash,
        accountIdBuild, genesisPrivateKey, defaultNodeAccount);
    Assert.assertNotNull(claimResponse);
    Assert.assertTrue(claimResponse.hasCryptoGetClaim());
    Assert.assertEquals(ResponseCodeEnum.PAYER_ACCOUNT_NOT_FOUND,
        claimResponse.getCryptoGetClaim().getHeader().getNodeTransactionPrecheckCode());
    ...
    log.info("Get account claim api by providing wrong private key of payer account id");
    claimResponse = TestHelper.getAccountClaim(stub, newlyCreateAccountId1, hash,
        payerAccount, keyPair.getPrivate(), defaultNodeAccount);
    Assert.assertNotNull(claimResponse);
    Assert.assertTrue(claimResponse.hasCryptoGetClaim());
    Assert.assertEquals(ResponseCodeEnum.INVALID_SIGNATURE,
        claimResponse.getCryptoGetClaim().getHeader().getNodeTransactionPrecheckCode());
    ...
    log.info("Get account claim api by providing wrong hash of the claim ");
    claimResponse = TestHelper.getAccountClaim(stub, newlyCreateAccountId1, new byte[0],
        payerAccount, genesisPrivateKey, defaultNodeAccount);
    Assert.assertNotNull(claimResponse);
    Assert.assertTrue(claimResponse.hasCryptoGetClaim());
    Assert.assertEquals(ResponseCodeEnum.INVALID_CLAIM_HASH_SIZE,
        claimResponse.getCryptoGetClaim().getHeader().getNodeTransactionPrecheckCode());
    log.info("assertion completed...");
    log.info("--------------------------------------");
*/
