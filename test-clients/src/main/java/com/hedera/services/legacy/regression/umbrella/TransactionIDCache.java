...
public class TransactionIDCache {
  ...
  private TransactionIDCache(int receiptPeriod, int recordPeriod) {
    Comparator<TransactionID> idComparatorReceipt =
        (c1, c2) -> {
          Instant t1 =
              RequestBuilder.convertProtoTimeStamp(c1.getTransactionValidStart())
                  .plusSeconds(receiptPeriod);
          Instant t2 =
              RequestBuilder.convertProtoTimeStamp(c2.getTransactionValidStart())
                  .plusSeconds(receiptPeriod);
    ...
    Comparator<TransactionID> idComparatorRecord =
        (c1, c2) -> {
          Instant t1 =
              RequestBuilder.convertProtoTimeStamp(c1.getTransactionValidStart())
   ...
   *
   * @param txId transaction ID to be added to cache
   */
  public synchronized void addTransactionID(TransactionID txId) {
    ...
    try {
      com.hedera.services.legacy.proto.utils.CommonUtils.writeTxId2File(
          TextFormat.shortDebugString(txId));
    } catch (IOException e) {
