...
public class SmartContractPayReceivableAmount {
	...
	public void demo() throws Exception {
		...
		AccountID receiverAccount = createAccount(receiverAccountKeyPair, genesisAccount,
		...
		if (crAccount != null) {
			...
			if (simpleStorageFileId != null) {
				...
				for (int i = 0; i < 10; i++) {
					...
					AccountID contractAccountId = AccountID.newBuilder().setAccountNum(
							payTestContractId.getContractNum())
							.setRealmNum(payTestContractId.getRealmNum()).setShardNum(
									payTestContractId.getShardNum()).build();
					List<AccountAmount> expectedTransfers = new ArrayList<>(3);
					AccountAmount receiverTransfer = AccountAmount.newBuilder().setAccountID(receiverAccount).setAmount(
							receiverValue).build();
					expectedTransfers.add(receiverTransfer);
					AccountAmount contractTransfer = AccountAmount.newBuilder().setAccountID(
							contractAccountId).setAmount(remainToContract).build();
					expectedTransfers.add(contractTransfer);

					TransactionRecord sfTrRecord = sendFunds(crAccount, payTestContractId, recAccountSolidityAddress,
							currValueToDeposit, receiverValue);
