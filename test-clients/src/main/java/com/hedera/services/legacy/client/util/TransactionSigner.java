...
public class TransactionSigner {
   ...
   */
  public static Transaction signTransaction(
      final Transaction transaction, final List<PrivateKey> privKeyList) {
  ...
  public static Transaction signTransaction(
    ...
    try {
      return signTransactionComplexWithSigMap(
          transaction, keyList, pubKey2privKeyMap, appendSigMap);
   ...
   */
  public static Transaction signTransactionComplexWithSigMap(
      final TransactionOrBuilder transaction,
      final List<Key> keys,
  ...
  public static Transaction signTransactionComplexWithSigMap(
    ...
    if (appendSigMap) {
      ...
      final var sigMapToSet =
          currentSigMap.toBuilder().addAllSigPair(sigsMap.getSigPairList()).build();
   ...
   */
  public static SignatureMap signAsSignatureMap(
    ...
    for (Key key : uniqueKeys) {
      if (key.hasContractID()) {
        // according to Leemon, "for Hedera transactions, we treat this key as never having
        // signatures."
        continue;
      }

      final var sig =
          KeyExpansion.signBasicAsSignaturePair(key, len, pubKey2privKeyMap, messageBytes);
      pairs.add(sig);
    }
