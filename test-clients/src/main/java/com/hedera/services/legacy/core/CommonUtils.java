...
public class CommonUtils {
   ...
   *
   * @param timeInMillis given time in milli seconds
   */
   ...
   *
   * @param timeInSec given time in seconds
   */
   ...
   *
   * @param path the file path to write bytes to
   * @param data the byte array to be written to the file
   * @throws IOException when failing to write the byte array to the file
   */
   ...
   * @param path the file path to write bytes to
   * @param data the byte array to be written to the file
   * @param append the flag to control whether to append to or to overwrite the file current
   *     contents
   * @throws IOException when failing to write the byte array to the file
   */
   ...
   *
   * @param start starting position from which bytes need to be copied
   * @param length number of bytes to copy
   * @param bytes source byte array to be copied from
   * @return copied bytes
   */
   ...
   *
   * @param filePath path of the small binary file
   * @return the byte array of the file contents
   * @throws IOException when failing to read the file
   * @throws URISyntaxException when the given file path could not be parsed as a URI reference
   */
   ...
   *
   * @param filePath the binary file to be read
   * @param myClass the calling class
   * @param <T> calling class type
   ...
   * @param realmNum realm number
   * @param accountNum solidity contract number
   ...
   *
   * @param record transaction record whose fields need to be checked
   * @param body the originating transaction body for comparing transaction record fields
   */
  public static void checkRecord(TransactionRecord record, TransactionBody body) {
    ...
    if (body.hasCryptoTransfer()
        && ResponseCodeEnum.SUCCESS.equals(record.getReceipt().getStatus())) {
      TransferList transferList = body.getCryptoTransfer().getTransfers();
      Assertions.assertEquals(
          true, record.getTransferList().toString().contains(transferList.toString()));
    }
