...
public class ProtectedEntityTests extends BaseClient {
	...
	public void init(long[] accounts) throws Throwable {
		...
		FILE_PART_SIZE = 3000; // 3K, which forces fee schedule (8K in size) to be uploaded in 3 parts, i.e. one update
		// and two append API calls.
	}
	...
	public AccountInfo updateAccount(AccountID accountID, AccountID payerAccountID,
		...
		if (newKey != null) {
			com.hederahashgraph.api.proto.java.TransactionBody.Builder builder =
					com.hedera.services.legacy.proto.utils.CommonUtils.extractTransactionBody(
					updateTx).toBuilder();
		...
		for (int i = 0; i < payers.length; i++) {
			...
			for (int j = 0; j < files.length; j++) {
				...
				if (fid2waclMap.contains(fid)) {
					...
					wacl = acc2ComplexKeyMap.get(
							genesisAccountID).getKeyList().getKeysList(); // note: same key is used for genesis account
					// and system file wacl
				}
	 ...
	 * System Deleted & Undelete - For accounts 59 & 60, no wacl needed; accounts 2 and 50 are authorized, but need wacl
	 * to sign
	 */
	public void systemDeleteUndeleteTests() throws Throwable {
		...
		for (int i = 0; i < systemDeleteAccounts.length; i++) {
			...
			{
				systemDelete(fid, authDeletePayerID, nodeID, waclPubKeyList, ResponseCodeEnum.OK,
			...
			{
				systemUndelete(fid, undeletePayerID, nodeID, waclPubKeyList, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
			}
		}
...
//      systemUndelete(contractID, undeletePayerID, nodeID, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS);
	...
	protected TransactionResponse updateFileWithFee(FileID fid, AccountID payerID, AccountID nodeID,
			...
			ResponseCodeEnum expectedPostcheckCode) throws Throwable {
		return updateFileWithValidContent(fid, payerID, nodeID, oldWaclKeyList, newWaclKeyList, expectedPrecheckCode,
				expectedPostcheckCode, false);
	}
	 ...
	 * 		existing wacl keys for file, null allowed
	 * @param newWaclKeyList
	 * 		the new wacl keys to replace existing ones, null allowed
	 * @param expectedPrecheckCode
	 * 		expected precheck code
	 * @param expectedPostcheckCode
	 * 		expected postcheck code
	 * @param isFree
	 * 		whether the transaction is free
	 * @param isValidFileContent
	 * 		whether the fee file data for updating is valid
	...
	protected void deleteFile(FileID fid, AccountID payerID,
			AccountID nodeID, List<Key> waclKeyList, ResponseCodeEnum expectedPrecheckCode,
			ResponseCodeEnum expectedPostcheckCode) throws Throwable {
	...
	public void deleteSystemAccountTests() throws Throwable {
		...
		for (int i = 0; i < payers.length; i++) {
			...
			for (int j = 0; j < accounts.length; j++) {
				...
				accountKey = acc2ComplexKeyMap.get(
						aid); // note: same key is used for genesis account and system account wacl
	...
	private void deleteAccount(AccountID accountID, AccountID payerAccount, AccountID nodeID,
			Key accountKey, ResponseCodeEnum expectedPrecheckCode,
			ResponseCodeEnum expectedPostcheckCode) throws Throwable {
	 ...
	 *
	 * @param entityID
	 * 		file or contract ID to be deleted
	 * @param payerAccount
	 * @param nodeID
	 * @param expectedPrecheckCode
	 * @param expectedPostcheckCode
	 ...
	 * @param nodeID
	 * @param fileWacl
	 * 		the wacl of the file to be deleted, it should sign if not null
	 * @param expectedPrecheckCode
	 * @param expectedPostcheckCode
	 * @throws Throwable
	 */
	...
	private void systemUndelete(Object entityID, AccountID payerAccount, AccountID nodeID, List<Key> fileWacl,
			ResponseCodeEnum expectedPrecheckCode, ResponseCodeEnum expectedPostcheckCode) throws Throwable {
		com.hederahashgraph.api.proto.java.SystemUndeleteTransactionBody.Builder systemUndeleteTransactionBodyBuilder =
	...
	private void dynamicRestart(AccountID payerAccount, AccountID nodeID,
		...
		com.hederahashgraph.api.proto.java.FreezeTransactionBody.Builder systemDeleteTransactionBodyBuilder =
	 ...
	 *
	 * @param transactionID
	 * 		transaction whose record is to be checked.
	 * @param payerID
	 * @param nodeID
	 * @param isFree
	 * 		the transaction should be free
	 * @throws Exception
	 */
	...
	public void updateLargeFile(byte[] largeFileBytes, FileID fid, AccountID payerID,
		...
		updateLargeFile(largeFileBytes, fid, payerID,
				nodeID, waclKeyList, expectedPrecheckCodeUpdate, expectedPostcheckCodeUpdate,
				expectedPrecheckCodeAppend,
	...
	public void updateLargeFile(byte[] largeFileBytes, FileID fid, AccountID payerID,
			AccountID nodeID, List<Key> waclKeyList, ResponseCodeEnum expectedPrecheckCodeUpdate,
			ResponseCodeEnum expectedPostcheckCodeUpdate, ResponseCodeEnum expectedPrecheckCodeAppend,
			ResponseCodeEnum expectedPostcheckCodeAppend, boolean isFree,
			ResponseCodeEnum expectedPrecheckCodeIntermediateAppend,
		...
		for (; i < midParts; i++) {
			...
			appendFile(payerID, nodeID, fid, fileData, waclKeyList, expectedPrecheckCodeIntermediateAppend,
					expectedPostcheckCodeIntermediateAppend, isFree);
		...
		if (numParts >= 2 || remainder > 0) {
			...
			fileData = ByteString.copyFrom(partBytes);
			appendFile(payerID, nodeID, fid, fileData, waclKeyList, expectedPrecheckCodeAppend,
					expectedPostcheckCodeAppend, isFree);
	...
	public void updateFeeScheduleTests() throws Throwable {
		...
		List<Key> feeFileWacl = (acc2ComplexKeyMap.get(
				genesisAccountID)).getKeyList().getKeysList(); // fee schedul file share keys with genesis
		for (int i = 0; i < feeScheduleAccounts.length; i++) {
			...
			if (payer.getAccountNum() == 50 || payer.getAccountNum() == 56) {//no wacl needed, also free
				...
				updateLargeFile(feeBytes, feeSchedule111, payer, nodeID, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
				...
				updateLargeFile(invalid8KFile, feeSchedule111, payer, nodeID, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
						ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, true);
			} else if (payer.getAccountNum() == 2) {
				// success with wacl signing and fee
				updateLargeFile(feeBytes, feeSchedule111, payer, nodeID, feeFileWacl, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
				...
				if (TransactionSigner.SIGNATURE_FORMAT.equals(TransactionSigner.SIGNATURE_FORMAT_ENUM.SignatureList)) {
					updateLargeFile(feeBytes, feeSchedule111, payer, nodeID, null, ResponseCodeEnum.OK,
							ResponseCodeEnum.INVALID_SIGNATURE, ResponseCodeEnum.OK, ResponseCodeEnum.INVALID_SIGNATURE,
							false);
				} else if (TransactionSigner.SIGNATURE_FORMAT.equals(
						TransactionSigner.SIGNATURE_FORMAT_ENUM.SignatureMap)) {
					// success due to payer and exchange file share the same key, with sig map, wacl is implicitly
					// signed as payer is signing
					updateLargeFile(feeBytes, feeSchedule111, payer, nodeID, null, ResponseCodeEnum.OK,
							ResponseCodeEnum.SUCCESS, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, false);
				}

				// fails with no fee
				updateLargeFile(feeBytes, feeSchedule111, payer, nodeID, null, ResponseCodeEnum.INSUFFICIENT_TX_FEE,
						null, ResponseCodeEnum.INSUFFICIENT_TX_FEE, null, true);
			}
		}
		...
		for (int i = 0; i < feeScheduleAccountsUnauth.length; i++) {
			...
			updateLargeFile(feeBytes, feeSchedule111, payer, nodeID, feeFileWacl, ResponseCodeEnum.AUTHORIZATION_FAILED,
					null, ResponseCodeEnum.AUTHORIZATION_FAILED, null, false);
		}
		...
		updateLargeFile(serverFeeSchedule.toByteArray(), feeSchedule111, payer, nodeID, feeFileWacl,
				ResponseCodeEnum.OK,
				ResponseCodeEnum.SUCCESS,
				ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, false);
	...
	public void updateAddressFileTests() throws Throwable {
		...
		List<Key> addressFileWacl = (acc2ComplexKeyMap.get(
				genesisAccountID)).getKeyList().getKeysList(); // address files
		// share keys with genesis
		...
		for (int i = 0; i < addressAccounts.length; i++) {
			...
			if (payer.getAccountNum() == 50) {//free and no wacl needed
				...
				updateFileWithNoFee(addressBook101, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				updateFileWithNoFee(addressBook102, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);

				// update address book as a multipart file
				byte[] addressBytes = genRandomBytes(FILE_PART_SIZE * 3 + 1000);
				updateLargeFile(addressBytes, addressBook101, payer, nodeID, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
						ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, true);
				updateLargeFile(addressBytes, addressBook102, payer, nodeID, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
						ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, true);
			} else if (payer.getAccountNum() == 55) {//not free, though no wacl needed
				// success with fee but no wacl
				updateFileWithFee(addressBook101, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				updateFileWithFee(addressBook102, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);

				// fails without fee
				updateFileWithNoFee(addressBook101, payer, nodeID, addressFileWacl, null,
						ResponseCodeEnum.INSUFFICIENT_TX_FEE, null);
				updateFileWithNoFee(addressBook102, payer, nodeID, addressFileWacl, null,
						ResponseCodeEnum.INSUFFICIENT_TX_FEE, null);
				...
				updateFileWithFee(addressBook101, payer, nodeID, addressFileWacl, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				updateFileWithFee(addressBook102, payer, nodeID, addressFileWacl, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				...
				if (TransactionSigner.SIGNATURE_FORMAT.equals(TransactionSigner.SIGNATURE_FORMAT_ENUM.SignatureList)) {
					...
					updateFileWithFee(addressBook102, payer, nodeID, null, null, ResponseCodeEnum.OK,
						...
						TransactionSigner.SIGNATURE_FORMAT_ENUM.SignatureMap)) {
					// success due to payer and address files share the same key, with sig map, wacl is implicitly
					// signed as payer is signing
					updateFileWithFee(addressBook101, payer, nodeID, null, null, ResponseCodeEnum.OK,
							ResponseCodeEnum.SUCCESS);
					updateFileWithFee(addressBook102, payer, nodeID, null, null, ResponseCodeEnum.OK,
				...
				// update address book as a multipart file
				byte[] addressBytes = genRandomBytes(FILE_PART_SIZE * 3 + 1000);
				updateLargeFile(addressBytes, addressBook101, payer, nodeID, addressFileWacl, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
						ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, false);
				updateLargeFile(addressBytes, addressBook102, payer, nodeID, addressFileWacl, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS,
						ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, false);
			}
		}
		...
		AccountID payer = genAccountID(2);
		updateFile(addressBook101, payer, nodeID, addressFileWacl, null, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS,
				false, serverAddressBook.toByteString());
		updateFile(addressBook102, payer, nodeID, addressFileWacl, null, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS,
				false, serverNodeDetails.toByteString());
	...
	public void updateExchangeRateFileTests() throws Throwable {
		...
		List<Key> exchangeRateFileWacl = (acc2ComplexKeyMap.get(
				genesisAccountID)).getKeyList().getKeysList(); // exchange
		// file share keys with genesis
		...
		for (int i = 0; i < exchangeRateAccounts.length; i++) {
			...
			if (payer.getAccountNum() == 50) {//no wacl needed, also free
				...
				updateFileWithNoFee(exchangeRate112, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				...
				updateFileWithInvalidContent(exchangeRate112, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.INVALID_EXCHANGE_RATE_FILE, true);
			} else if (payer.getAccountNum() == 57) {//no wacl needed, also free
				// success with no wacl signing and no fee
				isSmallExchangeRateUpdate = true; // small change is OK. Note this works if this is the first time to
				// change the exchange rate in the same day
				updateFileWithNoFee(exchangeRate112, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);

				isSmallExchangeRateUpdate = false; // large change is NOT OK
				updateFileWithNoFee(exchangeRate112, payer, nodeID, null, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.EXCHANGE_RATE_CHANGE_LIMIT_EXCEEDED);
				isSmallExchangeRateUpdate = true; // reset flag
				...
				// success with wacl signing and fee
				isSmallExchangeRateUpdate = true; // small change is OK
				updateFileWithFee(exchangeRate112, payer, nodeID, exchangeRateFileWacl, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				isSmallExchangeRateUpdate = false; // large change is also OK
				updateFileWithFee(exchangeRate112, payer, nodeID, exchangeRateFileWacl, null, ResponseCodeEnum.OK,
						ResponseCodeEnum.SUCCESS);
				...
				// fails with no wacl signing
				if (TransactionSigner.SIGNATURE_FORMAT.equals(TransactionSigner.SIGNATURE_FORMAT_ENUM.SignatureList)) {
					updateFileWithFee(exchangeRate112, payer, nodeID, null, null, ResponseCodeEnum.OK,
							ResponseCodeEnum.INVALID_SIGNATURE);
				} else if (TransactionSigner.SIGNATURE_FORMAT.equals(
						TransactionSigner.SIGNATURE_FORMAT_ENUM.SignatureMap)) {
					// success due to payer and exchange file share the same key, with sig map, wacl is implicitly
				...
				// fails with no fee
				updateFileWithNoFee(exchangeRate112, payer, nodeID, exchangeRateFileWacl, null,
						ResponseCodeEnum.INSUFFICIENT_TX_FEE, null);
			}
		}
		...
		for (int i = 0; i < exchangeRateAccountsUnauth.length; i++) {
			...
			updateFileWithFee(exchangeRate112, payer, nodeID, exchangeRateFileWacl, null,
					ResponseCodeEnum.AUTHORIZATION_FAILED, null);
		}
		...
		AccountID payer = genAccountID(2);
		updateFile(exchangeRate112, payer, nodeID, exchangeRateFileWacl, null, ResponseCodeEnum.OK,
				ResponseCodeEnum.SUCCESS, false, serverExchangeRate.toByteString());
	 ...
	 *
	 * @param addressBookFileBytesPath
	 * 		path to the file
	 * @return byte string of the address book file read.
	 * @throws URISyntaxException
	 * @throws IOException
	 */
	public static ByteString getAddresBookByteString(
			String addressBookFileBytesPath) throws URISyntaxException, IOException {
		Path pathFeeSch =
	...
	public ByteString genExchangeFile() throws Throwable {
		...
		com.hederahashgraph.api.proto.java.ExchangeRate curr = serverRates.getCurrentRate().toBuilder().setCentEquiv(
				(int) c0n).build();
		com.hederahashgraph.api.proto.java.ExchangeRate next = serverRates.getNextRate().toBuilder().setCentEquiv(
				(int) c1n).build();
	 ...
	 *
	 * @param bound
	 * 		max increase is by a factor of (1+bound/100), decrease by 1 over that
	 * @param oldC
	 * 		the old exchange rate is for this many tinycents
	 * @param oldH
	 * 		the old exchange rate is for this many tinybars
	 * @param newC
	 * 		the new exchange rate is for this many tinycents
	 * @param newH
	 * 		the new exchange rate is for this many tinybars
	 */
	...
	/**
	 * Generates a modified version of the node details file by removing one of the elements of the source node details
	 * file.
	 *
	 * @param sourceNodeDetails
	 * 		the source node details file
	 * @param isRandom
	 * 		if true, remove a random element from the source node details file, otherwise remove the last element
	...
	/**
	 * Generates a modified version of the address book file by removing one of the elements of the source address book
	 * file.
	 *
	 * @param sourceNodeDetails
	 * 		the source address book file
	 * @param isRandom
	 * 		if true, remove a random element from the source address book file, otherwise remove the last element
	 ...
	 *
	 * @param fid
	 * 		file ID of the addressbook or node details file
	 * @param sourceAddressBook
	 * 		the source of the addressbook or node details file
	 * @param isRandom
	 * 		if true, remove a random element from the source address book file, otherwise remove the last element
	 * @return modified version of the addressbook or node details file
	 * @throws Throwable
	 */
	public ByteString modAddressBookFile(FileID fid, NodeAddressBook sourceAddressBook,
			boolean isRandom) throws Throwable {
