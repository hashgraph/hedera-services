	...
	public void updateSelfTests() throws Throwable {
		...
		for (int i = 0; i < accounts.length; i++) {
			...
			if (account.getAccountNum() <= 1000) {
				acc2ComplexKeyMap.put(account,
						acc2ComplexKeyMap.get(genesisAccountID)); // accounts under 1000 has the same keypair as genesis
			}
		}
	...
	public void updateAccountWithNewKeysTests() throws Throwable {
		...
		for (int i = 0; i < otherAccounts.length; i++) {
			...
			for (int j = 0; j < accounts.length; j++) {
				AccountID account = genAccountID(accounts[j]);
				if (payerID.equals(
						account)) {// all accounts (including genesis) can update themselves except master account
					if (payerID.getAccountNum() == 50) {
						updateAccount(account, payerID, nodeID, ResponseCodeEnum.AUTHORIZATION_FAILED, null, true,
								newKey);
						log.info("update failed: payer=" + payerID.getAccountNum() + ", target acount=" + account);
					} else {
						updateAccount(account, payerID, nodeID, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, true,
								newKey);
						log.info("update success: payer=" + payerID.getAccountNum() + ", target acount=" + account);
					}
				} else if (payerID.getAccountNum() == 2) {
					if (account.getAccountNum() <= 1000) // genesis can update all protected accounts without needing
						// sig of the existing account
					{
						updateAccount(account, payerID, nodeID, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, false,
								newKey);
		...
		for (int i = 0; i < payers.length; i++) {
			...
			for (int j = 0; j < accounts.length; j++) {
				...
				if (payerID.equals(account)) {// self update is OK
					...
					log.info("update success: payer=" + payerID.getAccountNum() + ", target acount=" + account);
				} else if (account.getAccountNum() <= 1000)  // genesis can update all protected accounts without
					// needing sig of the existing account
				{
		...
		for (int i = 0; i < payers.length; i++) {
			...
			for (int j = 0; j < accounts.length; j++) {
				...
				if (payerID.equals(account)) {// self update is NOT OK
					...
					log.info("update failed: payer=" + payerID.getAccountNum() + ", target acount=" + account);
				} else if (account.getAccountNum() >= 51 && account.getAccountNum() <= 80)  // master can update
					// protected accounts 51 through 80 without needing sig of updated account
				{
					updateAccount(account, payerID, nodeID, ResponseCodeEnum.OK, ResponseCodeEnum.SUCCESS, false, null);
				} else if (account.getAccountNum() <= 1000) { // master can NOT update protected accounts other than 51
					// through 80
