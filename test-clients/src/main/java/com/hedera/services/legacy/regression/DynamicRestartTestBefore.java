 ...
 * Perform Before Restart Action for creating State of Crypto , File and Smartcontracts refer DynamicRestartTest and
 * DynamicRestartAfter
 *
...
public class DynamicRestartTestBefore extends DynamicRestartTest {
	...
	public static void main(String[] args) throws Throwable {
		...
		for (int i = 1; i < objectCount; i++) {
			AccountID accountID = createAccount(genesisAccountID, nodeID, 1000000 + (i * 1000),
					TestHelper.getCryptoMaxFee(), true);
		...
		for (int i = 1; i < objectCount; i++) {
			FileGetInfoResponse.FileInfo fileInfo = createFileAndGetInfo(genesisAccountID, nodeID, 4 * i,
					i + fileDuration);
		...
		for (int i = 1; i < objectCount; i++) {
			...
			ContractGetInfoResponse.ContractInfo contractInfo =
					createContractResponse.getContractGetInfo().getContractInfo();
