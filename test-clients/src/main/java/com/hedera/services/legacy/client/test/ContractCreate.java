				...
				public void run() {
					...
					while (checkRunning) {
						try {
							...
							if (item != null) {
								ContractID createdContractId = Common.getContractIDfromReceipt(stub, genesisAccount,
										item);
								...
								if (createdContractId != null) {
									...
									if ((items % 100) == 0) {
										log.info(
												"receipt fetched for iteration " + ", created contract " + createdContractId);
		...
		try {
			...
			for (int iteration = 1; iteration <= ITERATIONS; iteration++) {
				...
				try {
					...
					TransactionID txId = createContractOnly(genesisAccount, contractFileId, null, accountDuration,
							null);
					if (txId != null) {
						txIdQueue.add(txId);
					}
