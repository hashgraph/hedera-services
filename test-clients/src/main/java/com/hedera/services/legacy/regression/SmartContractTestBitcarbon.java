...
public class SmartContractTestBitcarbon extends LegacySmartContractTest {
  ...
  private Transaction createQueryHeaderTransfer(AccountID payer, long transferAmt)
    ...
    Transaction transferTx =
        TestHelper.createTransferSigMap(
            payer,
            accountKeyPairs.get(payer),
            nodeAccount,
            payer,
            accountKeyPairs.get(payer),
            nodeAccount,
  ...
  private AccountID createAccount(KeyPair keyPair, AccountID payerAccount, long initialBalance)
    ...
    Transaction transaction =
        TestHelper.createAccountWithSigMap(
            payerAccount, nodeAccount, keyPair, initialBalance, accountKeyPairs.get(payerAccount));
    TransactionResponse response = stub.createAccount(transaction);
    Assertions.assertNotNull(response);
    Assertions.assertEquals(ResponseCodeEnum.OK, response.getNodeTransactionPrecheckCode());
    System.out.println(
        "Pre Check Response of Create  account :: "
            + response.getNodeTransactionPrecheckCode().name());

    TransactionBody body = TransactionBody.parseFrom(transaction.getBodyBytes());
    AccountID newlyCreateAccountId =
  ...
  private TransactionGetReceiptResponse getReceipt(TransactionID transactionId) throws Exception {
    ...
    while (attempts <= MAX_RECEIPT_RETRIES
      ...
      System.out.println(
          "waiting to getTransactionReceipts as not Unknown..."
  ...
  private ContractID createContract(
      AccountID payerAccount, FileID contractFile, byte[] constructorData) throws Exception {
    ...
    SmartContractServiceGrpc.SmartContractServiceBlockingStub stub =
        SmartContractServiceGrpc.newBlockingStub(channel);

    Timestamp timestamp =
        RequestBuilder.getTimestamp(
            Instant.now(Clock.systemUTC()).minusSeconds(-1 * TestHelper.DEFAULT_WIND_SEC));
    Duration transactionDuration = RequestBuilder.getDuration(TestHelper.TX_DURATION);
    Transaction createContractRequest =
        TestHelper.getCreateContractRequestSigMap(
    ...
    System.out.println(
        " createContract Pre Check Response :: "
            + response.getNodeTransactionPrecheckCode().name());

    TransactionBody createContractBody =
        TransactionBody.parseFrom(createContractRequest.getBodyBytes());
    TransactionGetReceiptResponse contractCreateReceipt =
        getReceipt(createContractBody.getTransactionID());
    if (contractCreateReceipt != null) {
      createdContract = contractCreateReceipt.getReceipt().getContractID();
    }
    TransactionRecord trRecord =
        getTransactionRecord(payerAccount, createContractBody.getTransactionID());
    Assertions.assertNotNull(trRecord);
    Assertions.assertTrue(trRecord.hasContractCreateResult());
    Assertions.assertEquals(
        trRecord.getContractCreateResult().getContractID(),
  ...
  public static byte[] encodeJurisdictionsAdd(
      String name, int taxRate, String inventoryAddr, String reserveAddr) {
  ...
  private TransactionRecord callContract(
      AccountID payerAccount,
      ContractID contractToCall,
      byte[] data,
    ...
    Transaction callContractRequest =
        TestHelper.getContractCallRequestSigMap(
            payerAccount.getAccountNum(),
            payerAccount.getRealmNum(),
            payerAccount.getShardNum(),
            node_account_number,
    ...
    TransactionBody callContractBody =
        TransactionBody.parseFrom(callContractRequest.getBodyBytes());
    TransactionGetReceiptResponse contractCallReceipt =
        getReceipt(callContractBody.getTransactionID());
    Assertions.assertEquals(expectedStatus, contractCallReceipt.getReceipt().getStatus());

    TransactionRecord txRecord =
        getTransactionRecord(payerAccount, callContractBody.getTransactionID());
  ...
  private TransactionRecord getTransactionRecord(
      AccountID payerAccount, TransactionID transactionId) throws Exception {
    int port = 50211;
    ManagedChannel channel = ManagedChannelBuilder.forAddress(host, port).usePlaintext().build();
    CryptoServiceGrpc.CryptoServiceBlockingStub stub = CryptoServiceGrpc.newBlockingStub(channel);
    long fee = FeeClient.getCostForGettingTxRecord();
    Response recordResp =
        executeQueryForTxRecord(payerAccount, transactionId, stub, fee, ResponseType.COST_ANSWER);
    fee = recordResp.getTransactionGetRecord().getHeader().getCost();
    recordResp =
        executeQueryForTxRecord(payerAccount, transactionId, stub, fee, ResponseType.ANSWER_ONLY);
  ...
  private Response executeQueryForTxRecord(
      AccountID payerAccount,
    ...
    Query getRecordQuery =
        RequestBuilder.getTransactionGetRecordQuery(transactionId, paymentTx, responseType);
  ...
  private byte[] callContractLocal(AccountID payerAccount, ContractID contractToCall, byte[] data)
    ...
    Response callResp =
        executeContractCall(
            payerAccount, contractToCall, stub, callData, fee, ResponseType.COST_ANSWER);
    fee = callResp.getContractCallLocal().getHeader().getCost() + localCallGas;
    callResp =
        executeContractCall(
            payerAccount, contractToCall, stub, callData, fee, ResponseType.ANSWER_ONLY);
  ...
  private Response executeContractCall(
      AccountID payerAccount,
      ContractID contractToCall,
      SmartContractServiceGrpc.SmartContractServiceBlockingStub stub,
      ByteString callData,
    ...
    Query contractCallLocal =
        RequestBuilder.getContractCallLocalQuery(
            contractToCall, 250000L, callData, 0L, 5000, paymentTx, resposeType);
  ...
  private Boolean getJurisdictionsIsValid(
      AccountID payerAccount, ContractID contractId, byte[] address) throws Exception {
    Boolean retVal = null;
  ...
  private TransactionRecord callJurisdictionsAdd(
      AccountID payerAccount,
  ...
  private TransactionRecord callMintersadd(
      AccountID payerAccount,
      ContractID contractId,
      ResponseCodeEnum expectedStatus,
      String minter,
  ...
  private TransactionRecord callMintersConfigure(
      AccountID payerAccount,
      ContractID contractId,
      ResponseCodeEnum expectedStatus,
  ...
  public void demo(String grpcHost, AccountID nodeAccount) throws Exception {
    ...
    ManagedChannel channel = ManagedChannelBuilder.forAddress(host, port).usePlaintext().build();
    TestHelper.initializeFeeClient(
        channel,
        genesisAccount,
        accountKeyPairs.get(genesisAccount),
        SmartContractTestBitcarbon.nodeAccount);
    channel.shutdown();

    KeyPair crAccountKeyPair = new KeyPairGenerator().generateKeyPair();
    AccountID crAccount =
        createAccount(crAccountKeyPair, genesisAccount, TestHelper.getCryptoMaxFee() * 10);
    ...
    FileID addressBookFileId =
        LargeFileUploadIT.uploadFile(
            crAccount,
            ADDRESS_BOOK_BIN,
            new ArrayList<>(List.of(crAccountKeyPair.getPrivate())),
            host,
            nodeAccount);
    ...
    ContractID addressBookContractId = createContract(crAccount, addressBookFileId, null);
    ...
    String addressBookSolidityAddress =
        CommonUtils.calculateSolidityAddress(0, 0, addressBookContractId.getContractNum());
    ...
    Thread.sleep(1000);
    ContractID jurisdictionsContractId =
        createContract(crAccount, jurisdictionsFileId, constructorArgs);
    ...
    String jurisdictionsSolidityAddress =
        CommonUtils.calculateSolidityAddress(0, 0, jurisdictionsContractId.getContractNum());

    // Upload Minters file, "linking" address of AddressBook into the where needed.
    FileID MintersFileId =
        LargeFileUploadIT.uploadFileWithStringSubstitution(
            crAccount,
            MINTERS_BIN,
            new ArrayList<>(List.of(crAccountKeyPair.getPrivate())),
            "_+AddressBook.sol:AddressBook_+",
            addressBookSolidityAddress,
            host,
            nodeAccount);
    ...
    byte[] minterConstructorArgs =
        encodeMintersConstructor(jurisdictionsSolidityAddress, ARBITRARY_ADDRESS);
    Thread.sleep(1000);
    ...
    txRec =
        callMintersConfigure(
            crAccount, mintersContractId, ResponseCodeEnum.SUCCESS, jurisdictionsSolidityAddress);
    log.info("Configured Jurisdictions address in Minters contract");

    // Add the Jurisdiction "ny" to the list
    txRec =
        callJurisdictionsAdd(
            crAccount,
            jurisdictionsContractId,
  ...
  private void setUp() {
    ...
    nodeAccount =
        AccountID.newBuilder()
            .setAccountNum(node_account_number)
            .setRealmNum(node_shard_number)
            .setShardNum(node_realm_number)
            .build();
