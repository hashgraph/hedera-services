...
/**
 * Test infinite loop running out of gas
 *
 * @author Peter
 */
public class StressInfiniteLoop {
  private final Logger log = LogManager.getLogger(StressInfiniteLoop.class);
  ...
  private static long gasOffered = 250_000L;
  ...
  public static void main(String args[]) throws Exception {
    ...
    if ((args.length) > 2) {
      gasOffered = Integer.parseInt(args[2]);
    }
    System.out.println("Got gas offered as " + gasOffered);

    if ((args.length) > 3) {
      msecSleep = Integer.parseInt(args[3]);
    }
    System.out.println("Got msec to sleep between iterations as " + msecSleep);

    StressInfiniteLoop scSs = new StressInfiniteLoop();
    scSs.demo();
  }
  ...
  private Response executeContractCall(AccountID payerAccount, ContractID contractToCall,
    ...
    public void demo() throws Exception {
     loadGenesisAndNodeAcccounts();
      ...
      // Invoke an infinite loop with the passed amount of gas
      long iterations = 0;
      while (true) {
        long before = System.currentTimeMillis();
        setForever(crAccount, newContractId, gasOffered, ResponseCodeEnum.INSUFFICIENT_GAS);
        long after = System.currentTimeMillis();
        iterations += 1;
        log.info("Completed iteration " + iterations + " at " + new Date() + " in about " +
            (after - before) +  " msec");
