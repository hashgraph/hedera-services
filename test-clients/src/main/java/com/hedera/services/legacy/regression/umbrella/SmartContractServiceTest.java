...
public class SmartContractServiceTest extends FileServiceTest {
  ...
  private static final String CONTRACT_STORAGE_NAME =
   ...
   *
   * @param contractFile file id of the contract
   * @param contractFileName contract file name
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param nodeAccount ndoe account id, default listening account
   * @param addAdminKey if admin key needs to be added
   * @return created contract id
   * @throws Throwable indicates failure while creating contract
   */
  public ContractID createContract(
    ...
    Transaction createContractRequest =
        RequestBuilder.getCreateContractRequest(
            ...
            TestHelper.getContractMaxFee(),
            timestamp,
            transactionDuration,
            true,
            "createContract",
            DEFAULT_CREATE_CONTRACT_OP_GAS,
            contractFile,
            ByteString.EMPTY,
            0,
            contractAutoRenew,
    ...
    createContractRequest =
        TransactionSigner.signTransactionComplexWithSigMap(
            createContractRequest, keys, pubKey2privKeyMap);
    ...
    TransactionBody createContractBody =
    ...
    if (getReceipt) {
      TransactionReceipt contractCreateReceipt =
          TestHelperComplex.getTxReceipt(transactionID, cstub, log, host);
      ...
      if (!contractCreateReceipt.getStatus().name().equals(ResponseCodeEnum.SUCCESS.name())) {
        throw new Exception(
            "Problem creating contract: receipt="
   ...
   * @param contractToUpdate contract id of the contract to be updated
   * @return transaction Receipt for updating contract
   * @throws Exception indicates failure while updating contract
   */
  public TransactionReceipt updateContract(
      AccountID payerAccount, ContractID contractToUpdate, AccountID nodeAccount) throws Exception {
    Timestamp timestamp = TestHelperComplex.getDefaultCurrentTimestampUTC();
    ...
    Timestamp expirationTime =
        Timestamp.newBuilder()
            .setSeconds(cInfo1.getExpirationTime().getSeconds() + DAY_SEC * 30)
            .build();
    Key newAdminKey = genAdminKeyComplex();
    Transaction updateContractRequest =
    ...
    updateContractRequest =
        TransactionSigner.signTransactionComplexWithSigMap(
            updateContractRequest, keys, pubKey2privKeyMap);
    ...
    Assertions.assertNotNull(response);
    TransactionBody updateContractBody =
        com.hedera.services.legacy.proto.utils.CommonUtils.extractTransactionBody(
            updateContractRequest);
    log.info(
    ...
    if (getReceipt) {
      contractUpdateReceipt = TestHelperComplex.getTxReceipt(transactionID, cstub, log, host);
      ...
      if (!contractUpdateReceipt.getStatus().name().equals(ResponseCodeEnum.SUCCESS.name())) {
        ...
        throw new Exception(
            "Problem updating contract: receipt="
   ...
   *
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param contractId contract id whose info needs to be queried
   * @param nodeAccount node account id, default listening account
   * @return ContractInfo object
   * @throws Exception indicates failure while getting contract info
   */
  public ContractInfo getContractInfo(
      AccountID payerAccount, ContractID contractId, AccountID nodeAccount) throws Exception {
    long contractInfoCost = getContractInfoCost(payerAccount, contractId, nodeAccount);
    Transaction paymentTx =
        getQueryPaymentSignedWithFee(
            payerAccount, nodeAccount, "getContractInfo", contractInfoCost);
    Query getContractInfoQuery =
        RequestBuilder.getContractGetInfoQuery(contractId, paymentTx, ResponseType.ANSWER_ONLY);
  ...
  private long getContractInfoCost(
      AccountID payerAccount, ContractID contractId, AccountID nodeAccount) throws Exception {
    Transaction paymentTxCost =
        getQueryPaymentSigned(payerAccount, nodeAccount, "getContractInfoCost");
    Query getContractInfoQueryCost =
        RequestBuilder.getContractGetInfoQuery(contractId, paymentTxCost, ResponseType.COST_ANSWER);
  ...
  public byte[] callContract(
      AccountID payerAccount, AccountID nodeAccount, ContractID contractID, int value)
      throws Throwable {
  ...
  private byte[] callContractBase(
    ...
    callContractRequest =
        TransactionSigner.signTransactionComplexWithSigMap(
            callContractRequest, keys, pubKey2privKeyMap);

    log.debug("callContract: request=" + callContractRequest);
    TransactionBody callContractBody =
        com.hedera.services.legacy.proto.utils.CommonUtils.extractTransactionBody(
            callContractRequest);
    ...
    cache.addTransactionID(transactionID);
    TransactionReceipt contractCallReceipt =
        TestHelperComplex.getTxReceipt(transactionID, cstub, log, host);
    ...
    if (contractCallReceipt.getStatus().name().equalsIgnoreCase(ResponseCodeEnum.SUCCESS.name())) {
      TransactionRecord trRecord =
          getTransactionRecord(callContractBody.getTransactionID(), payerAccount, nodeAccount);
  ...
  public TransactionRecord getTransactionRecord(
      TransactionID transactionId, AccountID payerAccount, AccountID nodeAccount) throws Exception {

    long transactionRecordCost = getTransactionRecordCost(transactionId, payerAccount, nodeAccount);
    Transaction paymentTx =
        getQueryPaymentSignedWithFee(
            payerAccount, nodeAccount, "getTransactionRecord", transactionRecordCost);
    Query getRecordQuery =
        RequestBuilder.getTransactionGetRecordQuery(
            transactionId, paymentTx, ResponseType.ANSWER_ONLY);
  ...
  private long getTransactionRecordCost(
      TransactionID transactionId, AccountID payerAccount, AccountID nodeAccount) throws Exception {
    Transaction paymentTxCost =
        getQueryPaymentSigned(payerAccount, nodeAccount, "getTransactionRecordCost");
    Query getRecordQueryCost =
        RequestBuilder.getTransactionGetRecordQuery(
            transactionId, paymentTxCost, ResponseType.COST_ANSWER);
  ...
  private byte[] callContractLocalBase(
      AccountID payerAccount, AccountID nodeAccount, ContractID contractToCall, ByteString callData)
      throws Throwable {
    ...
    ByteString functionResults =
        callResp.getContractCallLocal().getFunctionResult().getContractCallResult();
   ...
   * @throws Exception indicates failure while getting contract byte code
   */
  public String getContractByteCode(
    ...
    Transaction paymentTx =
        getQueryPaymentSignedWithFee(
            payerAccount, nodeAccount, "getContractByteCode", getContractByteCodeCost);
    Query getContractBytecodeQuery =
        RequestBuilder.getContractGetBytecodeQuery(contractId, paymentTx, ResponseType.ANSWER_ONLY);
  ...
  private long getContractBytecodeCost(
      AccountID payerAccount, ContractID contractId, AccountID nodeAccount) throws Exception {
    Transaction paymentTxCost =
        getQueryPaymentSigned(payerAccount, nodeAccount, "getContractByteCodeCost");
    Query getContractBytecodeQueryCost =
        RequestBuilder.getContractGetBytecodeQuery(
            contractId, paymentTxCost, ResponseType.COST_ANSWER);
   ...
   *
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param solidityId solidity id given for getting contract Id
   * @param nodeAccount node account id, default listening account
   * @return contract ID fetched from the solidity Id
   * @throws Exception indicates failure while getting contractID based on solidity Id
   */
  public ContractID getBySolidityID(
      AccountID payerAccount, String solidityId, AccountID nodeAccount) throws Exception {
    Transaction paymentTx = getQueryPaymentSigned(payerAccount, nodeAccount, "getBySolidityID");
    Query getBySolidityIdQuery =
        RequestBuilder.getBySolidityIDQuery(solidityId, paymentTx, ResponseType.ANSWER_ONLY);
   ...
   * @param contractId contract id for which transaction records needs to be queried
   * @param nodeID node account id, default listening account
   * @return list of transaction records for the contract ID
   * @throws Exception indicates failure while fetching transaction records
   */
  public List<TransactionRecord> getTxRecordByContractID(
      AccountID payerID, ContractID contractId, AccountID nodeID) throws Exception {
    ...
    Transaction paymentTxSigned =
        getSignedTransferTx(
            ...
            payerID,
            nodeID,
            getTxRecordByContractIDCost,
            "getTxRecordByContractId");
    Query query =
        RequestBuilder.getContractRecordsQuery(
            contractId, paymentTxSigned, ResponseType.ANSWER_ONLY);
  ...
  private long getTxRecordByContractIDCost(
      AccountID payerID, ContractID contractId, AccountID nodeID) throws Exception {
    Transaction paymentTxSignedCost =
        getSignedTransferTx(
            payerID, nodeID, payerID, nodeID, QUERY_PAYMENT_AMOUNT, "getTxRecordByContractIdCost");
    Query queryCost =
        RequestBuilder.getContractRecordsQuery(
            contractId, paymentTxSignedCost, ResponseType.COST_ANSWER);
  ...
  public int callContractLocal(AccountID payerAccount, AccountID nodeAccount, ContractID contractID)
    ...
    if (contractFile.equals(CONTRACT_STORAGE_NAME)) {
      ...
      if (result != null && result.length > 0) {
        ...
        throw new Exception(
            "callContractLocal error: result is null or empty! "
                + result
                + "; contractID="
                + contractID
                + "; "
