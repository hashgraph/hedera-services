...
public class SmartContractNegativeCalls {
	...
	public void demo() throws Exception {
		...
		if (simpleStorageFileId != null) {
			...
			setValueToContract(crAccount, sampleStorageContractId, ResponseCodeEnum.OK,
					ResponseCodeEnum.INSUFFICIENT_GAS,
					CONTRACT_CALL_FAIL_GAS, 0L);
			log.info("Call failed with insufficient gas");


			setValueToContract(crAccount, sampleStorageContractId, ResponseCodeEnum.INSUFFICIENT_PAYER_BALANCE,
					ResponseCodeEnum.SUCCESS,
					CONTRACT_CALL_SUCCESS_GAS, INITIAL_BALANCE);
			log.info("Call failed precheck with unpayable value");

			setValueToContract(crAccount, sampleStorageContractId, ResponseCodeEnum.CONTRACT_NEGATIVE_GAS,
					ResponseCodeEnum.SUCCESS,
					NEGATIVE_NUMBER, 0L);
			log.info("Call failed precheck with negative gas");

			setValueToContract(crAccount, sampleStorageContractId, ResponseCodeEnum.CONTRACT_NEGATIVE_VALUE,
					ResponseCodeEnum.SUCCESS,
