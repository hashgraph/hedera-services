...
public class FilePerformanceThread extends BaseClient implements Runnable {
			...
			for (int i = 0; i < txCounts; i++) {
				...
				try {
					...
					do {
						...
						transferRes = createFile(payerID, nodeID, fileData, waclPubKeyList, CryptoServiceTest.stub,
								txHolder);
	...
	public TransactionResponse createFile(AccountID payerID, AccountID nodeID, ByteString fileData,
		...
		Transaction FileCreateRequest = RequestBuilder.getFileCreateBuilder(payerID.getAccountNum(),
				...
				nodeID.getShardNum(), TestHelper.getFileMaxFee(), timestamp, CryptoServiceTest.transactionDuration,
				true,
	 ...
	 *
	 * @param markerWindow
	 * 		number to transactions executed before receipt is retrieved
	 */
	 ...
	 *
	 * @param millis
	 * 		number of millisecs for the retry frequency
	 */
