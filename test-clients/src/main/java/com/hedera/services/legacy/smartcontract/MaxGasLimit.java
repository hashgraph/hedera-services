...
/**
 * Test running out of gas when offering under / over / exactly the server gas limit.
 *
 * @author Peter
 */
public class MaxGasLimit {
  private final Logger log = LogManager.getLogger(MaxGasLimit.class);

  private static final int MAX_RECEIPT_RETRIES = 600;
  public static final String INFINITE_LOOP_BIN = "testfiles/Loops.bin";

  private static final String FOREVER_ABI = "{\"constant\":true,\"inputs\":[],\"name\":\"forever\",\"outputs\":[],\"payable\":false,\"stateMutability\":\"pure\",\"type\":\"function\"}";
  ...
  private static long gasOffered = 150_000L;
  private static int msecSleep = 500;

  public static void main(String args[]) throws Exception {
    Properties properties = TestHelper.getApplicationProperties();
    host = properties.getProperty("host");
    port = Integer.parseInt(properties.getProperty("port"));
    contractDuration = Integer.parseInt(properties.getProperty("CONTRACT_DURATION"));
    ...
    nodeAccount = RequestBuilder
        .getAccountIdBuild(node_account_number, 0l, 0l);

    MaxGasLimit scSs = new MaxGasLimit();
    scSs.demo();
  }

  private void loadGenesisAndNodeAcccounts() throws Exception {
    Map<String, List<AccountKeyListObj>> hederaAccounts = null;
   Map<String, List<AccountKeyListObj>> keyFromFile = TestHelper.getKeyFromFile(TestHelper.getStartUpFile());
  ...
  /*
  Methods to run the "Forever" method
   */
  private void setForever(AccountID payerAccount, ContractID contractId, long gas,
      ResponseCodeEnum expectedStatus)
      throws Exception {
    byte[] dataToGet = encodeForever();
    callContract(payerAccount, contractId, dataToGet, expectedStatus, gas);
  }

  public static byte[] encodeForever() {
    CallTransaction.Function function = getForeverFunction();
    byte[] encodedFunc = function.encode();
    return encodedFunc;
  }

  public static CallTransaction.Function getForeverFunction() {
    String funcJson = FOREVER_ABI.replaceAll("'", "\"");
  ...
  private ContractCallLocalResponse callContractLocal(AccountID payerAccount,
    ...
    fee = callResp.getContractCallLocal().getHeader().getCost() + gas;
  ...
  public void demo() throws Exception {
    ...
    long serverMaxGasLimit = serverConfig.getMaxGasLimit();

    // Upload contract file
    FileID newContractFileId = LargeFileUploadIT
        .uploadFile(crAccount, INFINITE_LOOP_BIN,
            Collections.singletonList(crAccountKeyPair.getPrivate()),
            host, nodeAccount);
    ...
    // Invoke an infinite call loop with the passed amount of gas
    setForever(crAccount, newContractId, serverMaxGasLimit - 100, ResponseCodeEnum.INSUFFICIENT_GAS);
    setForever(crAccount, newContractId, serverMaxGasLimit + 100, ResponseCodeEnum.MAX_GAS_LIMIT_EXCEEDED);
    setForever(crAccount, newContractId, serverMaxGasLimit, ResponseCodeEnum.MAX_GAS_LIMIT_EXCEEDED);

    byte[] getValueEncodedFunction = encodeForever();
    // Invoke an infinite local-call loop with the passed amount of gas
    invokeCallContractLocal(crAccount, newContractId, serverMaxGasLimit - 100,
        ResponseCodeEnum.INSUFFICIENT_GAS, getValueEncodedFunction);
    invokeCallContractLocal(crAccount, newContractId, serverMaxGasLimit + 100,
        ResponseCodeEnum.MAX_GAS_LIMIT_EXCEEDED, getValueEncodedFunction);
    invokeCallContractLocal(crAccount, newContractId, serverMaxGasLimit,
        ResponseCodeEnum.MAX_GAS_LIMIT_EXCEEDED, getValueEncodedFunction);
  }
}
