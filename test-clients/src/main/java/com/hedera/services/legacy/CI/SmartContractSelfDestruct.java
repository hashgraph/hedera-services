...
public class SmartContractSelfDestruct {
	...
	private static final String SC_KILL_ME = "{\"constant\":false,\"inputs\":[{\"name\":\"beneficiary\"," +
	...
	public void demo() throws Exception {
		...
		if (crAccount != null) {
			...
			if (simpleStorageFileId != null) {
				...
				killMe(crAccount, payTestContractId, bogusSolidityAddress.toString(),
						ResponseCodeEnum.INVALID_SOLIDITY_ADDRESS);

				//passing contract address as an obtainer should return exception
				ContractInfo cntInf = getContractInfo(crAccount, payTestContractId, ResponseCodeEnum.OK,
						ResponseCodeEnum.OK);
				killMe(crAccount, payTestContractId, cntInf.getContractAccountID(),
				...
				cntInf = getContractInfo(crAccount, payTestContractId, ResponseCodeEnum.CONTRACT_DELETED,
	...
	private ContractInfo getContractInfo(AccountID payerAccount,
			ContractID contractId, ResponseCodeEnum expectedConstResponse,
			ResponseCodeEnum expecedAnswerResponse) throws Exception {
		ManagedChannel channel = ManagedChannelBuilder.forAddress(host, port)
				.usePlaintext(true)
				.build();
		SmartContractServiceGrpc.SmartContractServiceBlockingStub stub = SmartContractServiceGrpc
		...
		Response respToReturn = executeGetContractInfo(payerAccount, contractId, stub, fee,
				ResponseType.COST_ANSWER);
		Assert.assertEquals(respToReturn.getContractGetInfo().getHeader().getNodeTransactionPrecheckCode(),
				expectedConstResponse);
	...
	private void killMe(AccountID payerAccount, ContractID contractId, String beneficiaryAccount,
			ResponseCodeEnum expectedResponse) throws Exception {
