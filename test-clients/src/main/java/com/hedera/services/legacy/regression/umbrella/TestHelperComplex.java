...
public class TestHelperComplex extends TestHelper {
   ...
   *
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param nodeAccount node account id, default listening account
   * @param key key for the account to be created
   * @param initialBalance initial balance on account to be created
   * @param receiverSigRequired if receiver signature is required
   * @param accountDuration auto renew period for the account
   * @return Transaction for creating account
   */
  public static Transaction createAccountComplex(
      AccountID payerAccount,
      AccountID nodeAccount,
      Key key,
      long initialBalance,
      boolean receiverSigRequired,
      long accountDuration) {
    long transactionFee = 100000000L;
    Duration duration = RequestBuilder.getDuration(accountDuration);
    return createAccountComplex(
        payerAccount,
        nodeAccount,
        key,
        initialBalance,
   ...
   * @param nodeAccount node account id, default listening account
   * @param key key for the account to be created
   * @param initialBalance initial balance on account
   * @param transactionFee transaction fees for creating account
   * @param receiverSigRequired if receiver signature is required
   * @param autoRenewPeriod auto renew period for the account
   * @return Transaction for creating account
   */
  public static Transaction createAccountComplex(
      AccountID payerAccount,
    ...
    Transaction createAccountRequest =
    ...
    try {
      transaction =
          TransactionSigner.signTransactionComplexWithSigMap(
              createAccountRequest, keys, pubKey2privKeyMap);
    } catch (Exception e) {
      e.printStackTrace();
    }
    return transaction;
   ...
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param nodeAccount node account id, default listening account
   * @param getTxRecordFee fees to get transaction record
   * @param responsetype response type for the query
   * @return Query for getting transaction record
   * @throws Exception indicates failure while getting transaction record
   */
  public static Query getTxRecordByAccountIdComplex(
      AccountID accountID,
      AccountID payerAccount,
      AccountID nodeAccount,
      long getTxRecordFee,
    ...
    Transaction transferTransaction =
        CryptoServiceTest.getSignedTransferTx(
            payerAccount,
            nodeAccount,
            payerAccount,
            nodeAccount,
            getTxRecordFee,
            "getTxRecordByAccountId");
    return RequestBuilder.getAccountRecordsQuery(accountID, transferTransaction, responsetype);
  }

  public static Transaction updateAccount(
      AccountID accountID, AccountID payerAccount, AccountID nodeAccount, Duration autoRenew) {
    ...
    return RequestBuilder.getAccountUpdateRequest(
        accountID,
        payerAccountNum,
        0l,
        0l,
