	...
	void demo() throws Exception {
		...
		try {
			...
			for (int i = 0; i < transferTimes; i++) {
					...
					if (pemFileName == null) {
						...
						if (isCheckTransferList) {
							preBalance = Common.createBalanceMap(stub,
									new ArrayList<>(
											List.of(genesisAccount, nodeAccount, DEFAULT_FEE_COLLECTION_ACCOUNT)),
				...
				try {
					...
					if (isCheckTransferList) {
						if (!testingUpdate) {
							preBalance = Common.createBalanceMap(stub,
									new ArrayList<>(List.of(fromAccount, toAccount, nodeAccount,
					...
					Transaction submittedTran = Common.tranSubmit(() -> {
						...
						try {
							if (testingUpdate) {
								...
								singleTranFee = FeeClient.getCreateUpdateFee(request, 2, expirationTimeStamp,
										existingKey);
			...
			if (isCheckTransferList && !testingUpdate) {
				preBalance = Common.createBalanceMap(stub,
						new ArrayList<>(List.of(fromAccount, toAccount, nodeAccount, genesisAccount,
								DEFAULT_FEE_COLLECTION_ACCOUNT)),
						genesisAccount, genesisKeyPair,
						nodeAccount);
