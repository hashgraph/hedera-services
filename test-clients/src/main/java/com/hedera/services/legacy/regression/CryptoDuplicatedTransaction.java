...
public class CryptoDuplicatedTransaction extends Thread {
	...
	private void demo()
		...
		for (int i = 0; i < transferTimes; i++) {
			...
			Pair<Transaction, Transaction> pair = CreateTransferToDiffNodes(nodeAccount2, keyPair2.getPrivate(),
					nodeAccount3,
					nodeAccount4,
		...
		if (fetchReceipt == 1) { // ask for receipt
			...
			for (int i = 0; i < transferTimes; i++) {
				...
				log.info(
						this.getName() + " Tx Record is successfully retrieve and asserted: " + i + " record = " + record);
				...
				log.info(
						this.getName() + " Tx Record is successfully retrieve and asserted: " + i + " record = " + record2);
