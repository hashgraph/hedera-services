...
public class LargeFileUploadIT extends FileServiceIT {
   ...
   *
   * @param payerAccount payer account Id, as the payer of transaction
   * @param filePath upload file path
   * @param payerAccountKeyPair the payer account's single keypair
   * @return file ID of the file that is uploaded
   * @throws Exception indicates there is any failure during upload
   */
  public static FileID uploadFile(
      AccountID payerAccount, String filePath, KeyPair payerAccountKeyPair) throws Exception {
   ...
   * @param payerAccount payer account Id, as the payer of transaction
   * @param filePath upload file path
   * @param payerAccountPrivateKeys list of private keys for payer account
   * @return file ID of the file that is uploaded
   * @throws Exception indicates there is any failure during upload
   */
  public static FileID uploadFile(
      AccountID payerAccount, String filePath, List<PrivateKey> payerAccountPrivateKeys)
      throws Exception {
   ...
   *
   * @param payerAccount payer account Id, as the payer of transaction
   * @param filePath path of the file that needs to be uploaded
   * @param payerAccountPrivateKeys private keys for payer account
   * @param overrideHost override host
   * @param overrideNodeAccountID default listening node account id
   * @return file Id of the file that is uploaded
   * @throws Exception indicates there is any failure during upload
   */
  public static FileID uploadFile(
      ...
      String filePath,
      List<PrivateKey> payerAccountPrivateKeys,
      String overrideHost,
      AccountID overrideNodeAccountID)
      throws Exception {
    LargeFileUploadIT tester = new LargeFileUploadIT();
   ...
   * @param payerAccountPrivateKeys list of private keys for payer account
   * @param regex regex used to compare and replace the data in the file
   * @param replacement replacing string that is used to replace content in the file based on regex
   *     provided
   * @param overridehost overriding host
   * @param overrideAccount default listening node account id
   * @return file Id of the file that is uploaded
   * @throws Exception indicates there is any failure during upload
   */
  public static FileID uploadFileWithStringSubstitution(
      AccountID payerAccount,
      String filePath,
      List<PrivateKey> payerAccountPrivateKeys,
      String regex,
  ...
  public ByteString getFileContent() throws Exception {
    long fileContentFee = FeeClient.getFeeByID(HederaFunctionality.FileGetContents);

    Transaction paymentTxSigned =
        getPaymentSigned(payerSeq, recvSeq, "FileGetContentCost", fileContentFee);
    Query fileGetContentCostQuery =
        RequestBuilder.getFileGetContentBuilder(paymentTxSigned, fid, ResponseType.COST_ANSWER);
    Response getFileCostInfoResponse = stub.getFileContent(fileGetContentCostQuery);
    long queryGetFileContentCostFee =
        getFileCostInfoResponse.getFileGetContents().getHeader().getCost();

    paymentTxSigned =
        getPaymentSigned(payerSeq, recvSeq, "FileGetContent", queryGetFileContentCostFee);
    Query fileGetContentQuery =
