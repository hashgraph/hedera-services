...
public class CryptoServiceTest extends TestHelperComplex {
  ...
  protected static AccountID[] payerAccounts =
      null; // payer accounts, size determined by numCryptoAccounts
  protected static boolean isRandomTransferAccount = true;
  protected static int numTransferAccounts = 2;
  protected static AccountID[] transferAccounts =
      null; // accounts for transfer's from and to parties, size
  ...
  protected static boolean getReceipt =
      true; // flag whether or not to get receipts after a transfer transaction
  protected static boolean useSystemAccountAsPayer =
      false; // flag whether or not to use system accounts (account
  ...
  protected static long DEFAULT_INITIAL_ACCOUNT_BALANCE =
      getUmbrellaProperties().getLong("initialAccountBalance", 2000_00_000_000L);
  protected static long SMALL_ACCOUNT_BALANCE_FACTOR =
      getUmbrellaProperties().getLong("smallAccountBalanceFactor", 2000L);
  ...
  protected static int transactionMaxBytes =
      Integer.parseInt(
          getApplicationProperties().getProperty("transactionMaxBytes", "8192")); // 8192;
  protected static long specialAccountNum =
      Long.parseLong(getApplicationProperties().getProperty("specialAccountNum", "50"));
  protected static long genesisAccountNum =
      Long.parseLong(getApplicationProperties().getProperty("genesisAccountNum", "2"));
  ...
  protected static Duration transactionDuration =
      Duration.newBuilder().setSeconds(TX_DURATION_SEC).build();
  ...
  public void setUp(
      String host, int port, long nodeAccount, long retryFreq, boolean isExponentialBackoff)
      throws Throwable {
    ...
    createStubs();
    cache =
        TransactionIDCache.getInstance(
            TransactionIDCache.txReceiptTTL, TransactionIDCache.txRecordTTL);
  ...
  protected void getTestConfig() {
    ...
    isRandomTransferAccount =
        Boolean.parseBoolean(testProps.getString("isRandomTransferAccount", "false"));
    ...
    String[] fileSizesKStr =
        testProps.getString("fileSizesK", "1,10,100,1000").split(CONFIG_LIST_SEPARATOR);
    ...
    accountKeyTypes = testProps.getString("accountKeyType", "single").split(CONFIG_LIST_SEPARATOR);
    ...
    useSystemAccountAsPayer =
        Boolean.parseBoolean(testProps.getString("useSystemAccountAsPayer", "false"));
    ...
    specialAccountNum =
        Long.parseLong(applicationProperties.getProperty("specialAccountNum", "55"));
    genesisAccountNum = Long.parseLong(applicationProperties.getProperty("genesisAccountNum", "2"));
    accountDuration =
        Long.parseLong(applicationProperties.getProperty("ACCOUNT_DURATION", "7890000"));
    fileDuration = Long.parseLong(applicationProperties.getProperty("FILE_DURATION", "7890000"));
    contractDuration =
        Long.parseLong(applicationProperties.getProperty("CONTRACT_DURATION", "7890000"));
  }
   ...
   *
   * @param accountID account id to get transaction records for
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param nodeAccountID node account id, default listening account id
   * @return list of transaction records of the given account Id
   * @throws Throwable indicates failure while getting transaction record
   */
  public List<TransactionRecord> getTransactionRecordsByAccountId(
      AccountID accountID, AccountID payerAccount, AccountID nodeAccountID) throws Throwable {
    log.info("Get Tx records by account Id...");
    long fee =
        getAccountRecordsCostFee(
            accountID, payerAccount.getAccountNum(), nodeAccountID.getAccountNum());
    Query query =
        TestHelperComplex.getTxRecordByAccountIdComplex(
            accountID, payerAccount, nodeAccountID, fee, ResponseType.ANSWER_ONLY);
    ...
    Assertions.assertEquals(
        accountID, transactionRecord.getCryptoGetAccountRecords().getAccountID());
    List<TransactionRecord> recordList =
        transactionRecord.getCryptoGetAccountRecords().getRecordsList();
    log.info(
   ...
   *
   * @param accountID account id of the account to be updated
   * @param payerAccountID payer account id, acting as payer for the transaction
   * @param nodeAccountID node account id, default listening account id
   * @return account info after update
   * @throws Throwable indicates failure while signing transaction with sigMap
   */
  public AccountInfo updateAccount(
      AccountID accountID, AccountID payerAccountID, AccountID nodeAccountID) throws Throwable {
    Duration autoRenew =
        RequestBuilder.getDuration(
    ...
    Transaction updateTx =
        TestHelperComplex.updateAccount(accountID, payerAccountID, nodeAccountID, autoRenew);
    Transaction signUpdate =
        TransactionSigner.signTransactionComplexWithSigMap(updateTx, keys, pubKey2privKeyMap);
    ...
    TransactionBody body =
        com.hedera.services.legacy.proto.utils.CommonUtils.extractTransactionBody(signUpdate);
   ...
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param nodeAccountID node account id, default listening account id
   * @param cacheTxID if transaction id need to be cached
   * @return the account ID of the newly created account
   * @throws Throwable indicates failure while creating complex account
   */
   ...
   *
   * @param stub CryptoServiceBlockingStub
   * @param payerAccount payer account id, acting as payer for the transaction
   * @param nodeAccountID node account id, default listening account id
   ...
   * @throws Throwable indicates failure while creating complex account
   */
  public AccountID createAccountComplex(
      ...
      boolean cacheTxID)
      throws Throwable {
    return createAccountComplex(payerAccount, nodeAccountID, accountKeyType, getReceipt, cacheTxID);
  }
   ...
   * @param nodeAccountID node account id, default listening account id
   * @param accountKeyType type of complex key
   * @param needReceipt if receipt needs to be provided
   * @param cacheTxID if transaction id need to be cached
   * @return the account ID of the created account
   * @throws Throwable indicates failure while creating complex account
   */
  public AccountID createAccountComplex(
    ...
    return createAccountComplex(
        payerAccount, nodeAccountID, accountKeyType, balance, needReceipt, cacheTxID);
  }
  ...
  public AccountID createAccountComplex(
    ...
    Transaction createAccountRequest =
        TestHelperComplex.createAccountComplex(
            payerAccount, nodeAccountID, key, initBalance, receiverSigRequired, accountDuration);
    ...
    if (needReceipt) {
      Query query =
          Query.newBuilder()
              .setTransactionGetReceipt(
                  RequestBuilder.getTransactionGetReceiptQuery(
                      transactionID, ResponseType.ANSWER_ONLY))
              .build();
      Response transactionReceipts = fetchReceipts(query, cstub);
      if (!ResponseCodeEnum.SUCCESS
          .name()
      ...
      Assertions.assertEquals(
          body.getCryptoCreateAccount().getInitialBalance(), accInfo.getBalance());
    }
   ...
   *
   * @throws Exception indicates failure while reading genesis information
   */
   ...
   *
   * @param hostOverWrite used to overwrite default host
   * @param nodeAccountOverWrite used to overwrite default listening node account ID
   */
  protected void readAppConfig(String hostOverWrite, Long nodeAccountOverWrite) {
    ...
    if (nodeAccountOverWrite == null) {
      String nodeAccIDStr =
          properties.getString("defaultListeningNodeAccount", DEFAULT_NODE_ACCOUNT_ID_STR);
      defaultListeningNodeAccountID = extractAccountID(nodeAccIDStr);
    } else {
      defaultListeningNodeAccountID =
          AccountID.newBuilder().setAccountNum(nodeAccountOverWrite).build();
    }
   ...
   *
   * @param arrayOfStr string array which need to be parsed
   ...
   *
   * @param txId given transaction's TransactionID
   * @return the transaction receipt
   * @throws Throwable indicates failure while getting transaction receipt
   */
   ...
   *
   * @param txId transaction id of transaction
   ...
   * @throws Throwable indicates failure while getting account info of given account
   */
  public AccountInfo getAccountInfo(AccountID accountID) throws Throwable {
    return getAccountInfo(
        accountID, genesisAccountID.getAccountNum(), defaultListeningNodeAccountID.getAccountNum());
  }
   ...
   *
   * @param accountID the account to get info for
   * @param fromAccountNum payer and sender account id for query payment transaction
   * @param toAccountNum receiver account id for query payment transaction
   * @return account info of the given account
   * @throws Throwable indicates failure while getting account info of given account
   */
  public AccountInfo getAccountInfo(AccountID accountID, long fromAccountNum, long toAccountNum)
    ...
    AccountID payerAccountID =
        AccountID.newBuilder().setAccountNum(fromAccountNum).setRealmNum(0).setShardNum(0).build();
    AccountID toID =
        AccountID.newBuilder().setAccountNum(toAccountNum).setRealmNum(0).setShardNum(0).build();
    Transaction paymentTxSigned =
        getQueryPaymentSignedWithFee(
            payerAccountID, toID, "getCryptoGetAccountInfo", getAccountInfoFee);

    Query cryptoGetInfoQuery =
        RequestBuilder.getCryptoGetInfoQuery(accountID, paymentTxSigned, ResponseType.ANSWER_ONLY);
    log.info(
        "\n-----------------------------------\ngetAccountInfo: request = " + cryptoGetInfoQuery);
    Response getInfoResponse = retryLoopQuery(cryptoGetInfoQuery, "getAccountInfo");
    log.info(
        "Pre Check Response of getAccountInfo:: "
            + getInfoResponse
                .getCryptoGetInfo()
                .getHeader()
                .getNodeTransactionPrecheckCode()
  ...
  private long getGetAccountInfoFee(AccountID accountID, long fromAccountNum, long toAccountNum)
      throws Exception {
    Transaction paymentTxSignedCost =
        getPaymentSigned(fromAccountNum, toAccountNum, "getCryptoGetAccountInfoCost");
    Query cryptoGetInfoQueryCost =
        RequestBuilder.getCryptoGetInfoQuery(
            accountID, paymentTxSignedCost, ResponseType.COST_ANSWER);
  ...
  private long getAccountRecordsCostFee(AccountID accountID, long fromAccountNum, long toAccountNum)
      throws Exception {
    Transaction paymentTxSignedCost =
        getPaymentSigned(fromAccountNum, toAccountNum, "getAccountRecordsCost");
    Query cryptoGetInfoQueryCost =
        RequestBuilder.getAccountRecordsQuery(
            accountID, paymentTxSignedCost, ResponseType.COST_ANSWER);
  ...
  protected Key genComplexKey(String accountKeyType) {
    ...
    if (accountKeyType.equals(SUPPORTED_KEY_TYPES.thresholdKey.name())) {
      key =
          KeyExpansion.genThresholdKeyInstance(
              COMPLEX_KEY_SIZE, COMPLEX_KEY_THRESHOLD, pubKey2privKeyMap);
   ...
   *
   * @param memo memo from which account ID to be extracted from
   ...
   *
   * @param fid file id to get content from
   * @param payerID payer account id, acting as payer for the transaction
   * @param nodeID node account id, default listening account id
   * @return byte string of the contents of the given file ID
   * @throws Throwable indicates failure while getting file content
   */
  public ByteString getFileContent(FileID fid, AccountID payerID, AccountID nodeID)
    ...
    Transaction paymentTxSigned =
        getQueryPaymentSignedWithFee(payerID, nodeID, "FileGetContent", fileGetContentCost);
  ...
  private long getFileGetContentCost(
      FileID fid, AccountID payerID, AccountID nodeID, FileServiceBlockingStub stub)
      throws Exception {
    Transaction paymentTxSignedCost = getQueryPaymentSigned(payerID, nodeID, "FileGetContent");
    Query fileGetContentQueryCost =
        RequestBuilder.getFileGetContentBuilder(paymentTxSignedCost, fid, ResponseType.COST_ANSWER);
   ...
   * exists.
   *
   * @param nodeID the node to connect to as default listening account id
   * @return FileServiceBlockingStub, channel for file service
   * @throws Exception indicates failure while getting FileServiceBlockingStub
   */
  public synchronized FileServiceBlockingStub getStub(AccountID nodeID) throws Exception {
   ...
   * @throws Throwable indicates failure while getting account balance
   */
   ...
   * @throws Throwable indicates failure while getting account info
   */
   ...
   * @param payerAccountID payer account id, acting as payer for the transaction
   * @param nodeAccountID node account id, default listening account id
   * @param fromAccountID sender account id for the transfer
   * @param toAccountID receiver account id of the transfer
   * @param amount amount to be transferred
   * @return transaction receipt for the transfer transaction
   * @throws Throwable indicates failure caused while doing the transfer transaction
   */
  public TransactionReceipt transfer(
      ...
      long amount)
      throws Throwable {
    Transaction transferTxSigned =
        getSignedTransferTx(
            payerAccountID, nodeAccountID, fromAccountID, toAccountID, amount, "Transfer");
   ...
   * @throws Exception indicates failure caused if target level is less than 2 for keyList and
   *     thresholdKey
   */
   ...
   * @param memo memo in the payment transaction
   * @return the signed query payment transaction
   * @throws Exception indicates failure while getting query payment signed transaction
   */
  protected static Transaction getPaymentSigned(long payerSeq, long toSeq, String memo)
      throws Exception {
    AccountID payerAccountID =
        AccountID.newBuilder().setAccountNum(payerSeq).setRealmNum(0).setShardNum(0).build();
    AccountID toID =
        AccountID.newBuilder().setAccountNum(toSeq).setRealmNum(0).setShardNum(0).build();
   ...
   *
   * @param query query to get the receipt for
   * @param cstub2 CryptoServiceBlockingStub, blocking-style stub that supports unary and streaming
   *     output calls on the crypto service
   * @return response for the query
   * @throws Exception indicates failure while fetching receipts
   */
   ...
   * @throws Exception indicates failure while getting signed transfer transaction
   */
  public static Transaction getQueryPaymentSigned(AccountID payerID, AccountID nodeID, String memo)
      throws Exception {
    Transaction paymentTxSigned =
        getSignedTransferTx(payerID, nodeID, payerID, nodeID, TestHelper.getCryptoMaxFee(), memo);
   ...
   * @param nodeID node account id, default listening account id
   * @param memo memo for the transaction
   * @param queryFee fees to be paid for the query
   * @return query payment signed transaction with fee
   * @throws Exception indicates failure while getting signed transfer transaction
   */
  public static Transaction getQueryPaymentSignedWithFee(
      AccountID payerID, AccountID nodeID, String memo, long queryFee) throws Exception {
    Transaction paymentTxSigned =
        getSignedTransferTx(payerID, nodeID, payerID, nodeID, queryFee, memo);
    return paymentTxSigned;
  }
   ...
   * @param payerAccountID payer account id, acting as payer for the transaction
   * @param nodeAccountID node account id, default listening account id
   * @param fromAccountID sender account id for transfer transaction
   * @param toAccountID receiver account id of transfer transaction
   * @param amount amount to be transferred
   * @param memo memo for the transaction
   * @return signed transfer transaction
   * @throws Exception indicates failure while getting signed transaction with complex sigMap
   */
  protected static Transaction getSignedTransferTx(
      AccountID payerAccountID,
      AccountID nodeAccountID,
      AccountID fromAccountID,
      AccountID toAccountID,
    ...
    Transaction paymentTx =
        getUnSignedTransferTx(
            payerAccountID, nodeAccountID, fromAccountID, toAccountID, amount, memo);
    Key payerKey = acc2ComplexKeyMap.get(payerAccountID);
    ...
    Transaction paymentTxSigned =
        TransactionSigner.signTransactionComplexWithSigMap(paymentTx, keys, pubKey2privKeyMap);
    return paymentTxSigned;
  }
   ...
   *
   * @param txSigned given signed transaction
   * @throws Exception indicates the max size of transaction is exceeded.
   */
  protected void checkTxSize(Transaction txSigned) throws Exception {
    ...
    if (requestSize > transactionMaxBytes) {
      String msg =
          "transactionMaxBytes ("
              + transactionMaxBytes
              + ") exceeded! requestSize="
              + requestSize
              + ", txShortInfo="
              + com.hedera.services.legacy.proto.utils.CommonUtils.toReadableTransactionID(
                  txSigned);
      log.warn(msg);
   ...
   * Makes a transfer.
   *
   * @param transferTxSigned signed transfer transaction
   * @return transaction receipt for the transfer transaction
   * @throws Throwable indicates failure while extracting transaction body
   */
  public TransactionReceipt transfer(Transaction transferTxSigned) throws Throwable {
    log.info(
        "\n-----------------------------------\ntransfer: request = "
            + com.hedera.services.legacy.proto.utils.CommonUtils.toReadableString(
                transferTxSigned));
    ...
    Assertions.assertNotNull(response);
    TransactionBody body =
        com.hedera.services.legacy.proto.utils.CommonUtils.extractTransactionBody(transferTxSigned);
   ...
   * @param toAccountID receiver account id of the transfer
   * @param amount amount to be transferred
   * @param memo memo for the transfer transaction
   * @return unsigned transfer transaction
   */
  public static Transaction getUnSignedTransferTx(
      AccountID payerAccountID,
      AccountID nodeAccountID,
      AccountID fromAccountID,
      AccountID toAccountID,
    ...
    Transaction transferTx =
        RequestBuilder.getCryptoTransferRequest(
            ...
            true,
            memo,
            fromAccountID.getAccountNum(),
            -amount,
            toAccountID.getAccountNum(),
            amount);
    return transferTx;
  }
   ...
   *
   * @param stub CryptoServiceBlockingStub, blocking-style stub that supports unary and streaming
   *     output calls on the crypto service
   * @param accountID account id to get balance
   * @param payerID payer account id, acting as payer for the transaction
   * @param nodeID node account id, default listening account id
   * @return account balance of given account id
   * @throws Throwable indicates failure while getting signed payment transaction
   */
  public static long getAccountBalance(
      ...
      throws Throwable {
    Transaction paymentTxSigned =
        getPaymentSigned(payerID.getAccountNum(), nodeID.getAccountNum(), "getAccountBalance");
    Query cryptoGetBalanceQuery =
        RequestBuilder.getCryptoGetBalanceQuery(
            accountID, paymentTxSigned, ResponseType.ANSWER_ONLY);
    log.info(
    ...
    Response getBalanceResponse =
        cryptoServiceTest.retryLoopQuery(cryptoGetBalanceQuery, "cryptoGetBalance");
    log.info(
        "Pre Check Response of getAccountBalance:: "
            + getBalanceResponse
                .getCryptoGetInfo()
                .getHeader()
                .getNodeTransactionPrecheckCode()
  ...
  private Response retryLoopQuery(Query query, String apiName) throws InterruptedException {
      ...
      try {
        switch (apiName) {
          ...
          case "getAccountInfo":
            ...
            precheckCode = response.getCryptoGetInfo().getHeader().getNodeTransactionPrecheckCode();
            break;
          ...
          case "getTxRecordByTxID":
            ...
            precheckCode =
                response.getTransactionGetRecord().getHeader().getNodeTransactionPrecheckCode();
            break;
