 ...
 * Perform After Restart Tests by fetching state of Crypto , File and Smartcontracts refer DynamicRestartTest and DynamicRestartBefore
...
public class DynamicRestartTestAfter extends DynamicRestartTest {
  ...
  public static void main(String[] args) throws Throwable {
    ...
    accountInfoMap.keySet().stream().forEach(a->{
      ...
      CryptoGetInfoResponse.AccountInfo beforeFreezeAccountInfo = afterFreezeGetInfoResponse.getCryptoGetInfo().getAccountInfo();
      ...
      Assert.assertEquals(beforeFreezeAccountInfo.getExpirationTime(), afterFreezeAccountInfo.getExpirationTime());
    });
    ...
    contractInfoMap.keySet().stream().forEach(a->{
      ...
      ContractGetInfoResponse.ContractInfo beforeFreezeContractInfo = contractResponse.getContractGetInfo().getContractInfo();
      ...
      Assert.assertEquals(beforeFreezeContractInfo.getExpirationTime(), afterFreezeContractInfo.getExpirationTime());
      Assert.assertEquals(beforeFreezeContractInfo.getMemo(), afterFreezeContractInfo.getMemo());
      Assert.assertEquals(beforeFreezeContractInfo.getAutoRenewPeriod(), afterFreezeContractInfo.getAutoRenewPeriod());
