...
public class FeeClient {
  ...
  public static void initialize(int hbarEquiv, int centEquiv, byte[] feeSchBytes) {
    ...
    exchangeRate =
        ExchangeRate.newBuilder().setHbarEquiv(hbarEquiv).setCentEquiv(centEquiv).build();
  ...
  public static long getCreateAccountFee(Transaction transaction, int payerAcctSigCount)
    ...
    SigValueObj sigValueObj =
        new SigValueObj(totalSignatureCount, payerAcctSigCount, signatureSize);
  ...
  public static long getMaxFee() {
    ...
    return ((defaultFeeData.getNodedata().getMax()
            + defaultFeeData.getNetworkdata().getMax()
            + defaultFeeData.getServicedata().getMax()))
        / FEE_DIVISOR_TOTINYBARS;
  }
  ...
  public static long getSystemDeleteFee(Transaction transaction, int payerAcctSigCount)
    ...
    return (fileFeeBuilder.getTotalFeeforRequest(
        feeData.get(SubType.DEFAULT), feeMatrices, exchangeRate));
  }
  ...
  public static long getFileInfoQueryFee(KeyList keys) {
    ...
    FeeData feeMatrices =
        fileFeeBuilder.getFileInfoQueryFeeMatrices(keys, ResponseType.ANSWER_ONLY);
    return fileFeeBuilder.getTotalFeeforRequest(
        feeData.get(SubType.DEFAULT), feeMatrices, exchangeRate);
  }
  ...
  public static long getFileContentQueryFee(int contentSize) {
    ...
    FeeData feeMatrices =
        fileFeeBuilder.getFileContentQueryFeeMatrices(contentSize, ResponseType.ANSWER_ONLY);
    return fileFeeBuilder.getTotalFeeforRequest(
        feeData.get(SubType.DEFAULT), feeMatrices, exchangeRate);
  }
}
