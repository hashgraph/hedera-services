...
public class FileServiceTest extends CryptoServiceTest {
   ...
   * @param payerID the fee payer ID, acting as payer for transaction
   * @param nodeID the node ID, default listening account
   * @param oldWaclKeyList existing wacl keys
   * @param newWaclKeyList the new wacl keys to replace existing ones
   * @throws Throwable indicates failure to update the file
   */
  protected void updateFile(
      ...
      AccountID payerID,
      AccountID nodeID,
      List<Key> oldWaclKeyList,
      List<Key> newWaclKeyList)
      throws Throwable {
    Timestamp fileExp =
        ProtoCommonUtils.getCurrentTimestampUTC(
            CustomPropertiesSingleton.getInstance().getUpdateDurationValue());
    ...
    Transaction FileUpdateRequest =
        RequestBuilder.getFileUpdateBuilder(
            payerID.getAccountNum(),
            payerID.getRealmNum(),
            payerID.getShardNum(),
            nodeID.getAccountNum(),
            nodeID.getRealmNum(),
            nodeID.getShardNum(),
            TestHelper.getFileMaxFee(),
            timestamp,
            fileExp,
            transactionDuration,
            true,
            "FileUpdate",
            ByteString.copyFrom(fileData),
            fid,
            wacl);
    ...
    Transaction txSigned =
        TransactionSigner.signTransactionComplexWithSigMap(
            FileUpdateRequest, keys, pubKey2privKeyMap);

    log.info("\n-----------------------------------");
    log.info(
        "FileUpdate: input data = "
            + fileData
            + "\nexpirationTime = "
            + fileExp
            + "\nWACL keys = "
    ...
    log.info(
        "FileUpdate with data, exp, and wacl respectively, Response :: "
            + response.getNodeTransactionPrecheckCodeValue());
    Assertions.assertNotNull(response);
    Assertions.assertEquals(
        ResponseCodeEnum.OK_VALUE, response.getNodeTransactionPrecheckCodeValue());
  ...
  public Transaction updateFile(
    ...
    if (updateBody.getTransactionID() == null
        || !updateBody.getTransactionID().hasTransactionValidStart()) {
      return updateFile(
          fid, payerID, nodeID, oldWaclKeyList, newWaclKeyList, fileData, memo, fileExp);
    }
    return txSigned;
  }
   ...
   * @param payerID the fee payer ID, acting as payer for the transaction
   * @param nodeID the node ID, default listening account
   * @param waclKeyList the file creation WACL public key list
   * @throws Throwable indicates failure to delete the file
   */
  protected void deleteFile(FileID fid, AccountID payerID, AccountID nodeID, List<Key> waclKeyList)
      throws Throwable {
    Timestamp timestamp = TestHelperComplex.getDefaultCurrentTimestampUTC();
    Transaction FileDeleteRequest =
        RequestBuilder.getFileDeleteBuilder(
            payerID.getAccountNum(),
            payerID.getRealmNum(),
            payerID.getShardNum(),
            nodeID.getAccountNum(),
    ...
    Transaction txSigned =
        TransactionSigner.signTransactionComplexWithSigMap(
            FileDeleteRequest, keys, pubKey2privKeyMap);
   ...
   *
   * @param payerID payer account id, acting as payer for the transaction
   * @param nodeID node account id, default listening account
   * @param fileData contents of the file to be used while creating file
   * @param waclKeyList the file creation WACL public key list
   * @param cacheTxID whether to cache transaction ID
   ...
   * @throws Throwable indicates failure while creating file
   */
  public FileID createFile(
    ...
    Transaction FileCreateRequest =
        RequestBuilder.getFileCreateBuilder(
            payerID.getAccountNum(),
            payerID.getRealmNum(),
            payerID.getShardNum(),
            nodeID.getAccountNum(),
    ...
    Transaction filesigned =
        TransactionSigner.signTransactionComplexWithSigMap(
            FileCreateRequest, keys, pubKey2privKeyMap);
  ...
  private TransactionResponse retryLoopTransaction(
      AccountID nodeID, Transaction transaction, String Action) throws Exception {
  ...
  private Response retryLoopQuery(AccountID nodeID, Query query, String Action) throws Exception {
    FileServiceBlockingStub stub = null;
    ManagedChannel[] createdChannels = new ManagedChannel[1];
    if (isOneUseChannel) {
      stub = createFileServiceStub(createdChannels);
      ...
      try {
        switch (Action) {
          case "getFileInfo":
            ...
            precheckCode = response.getFileGetInfo().getHeader().getNodeTransactionPrecheckCode();
            break;
          default:
            throw new IllegalArgumentException();
        }
   ...
   * @param fileData contents of the file to be used while creating file
   * @param waclKeyList the file creation WACL public key list
   * @param fileExp expiration time for the file
   * @param memo memo of the file create transaction
   * @return file create transaction
   * @throws Throwable indicates failure while creating file
   */
  public Transaction createFile(
      AccountID payerID,
      AccountID nodeID,
      ByteString fileData,
      List<Key> waclKeyList,
    ...
    if (txBody.getTransactionID() == null
        || !txBody.getTransactionID().hasTransactionValidStart()) {
   ...
   * @throws Throwable indicates failure while uploading file
   */
  public FileID uploadFile(
      String savePath, byte[] bytes, AccountID payerID, AccountID nodeID, List<Key> waclPubKeyList)
      throws Throwable {
   ...
   * @param payerID the fee payer ID, acting as payer for the transaction
   * @param nodeID the node ID, default listening account
   * @param fid file id of the file whose content needs to be appended
   * @param fileData contents of the file to be appended
   * @param waclKeys WACL public key list
   * @throws Throwable indicates failure while appending file
   */
  public void appendFile(
      AccountID payerID, AccountID nodeID, FileID fid, ByteString fileData, List<Key> waclKeys)
      throws Throwable {
    Timestamp timestamp = TestHelperComplex.getDefaultCurrentTimestampUTC();

    Transaction fileAppendRequest =
        RequestBuilder.getFileAppendBuilder(
            payerID.getAccountNum(),
            payerID.getRealmNum(),
            payerID.getShardNum(),
            nodeID.getAccountNum(),
    ...
    Transaction txSigned =
        TransactionSigner.signTransactionComplexWithSigMap(
            fileAppendRequest, keys, pubKey2privKeyMap);
   ...
   * @throws IOException exception caused if file doesn't exist
   * @throws URISyntaxException exception caused if the URI for the path can't be parsed
   */
   ...
   * @throws IOException indicates failure reading file from the given path
   */
   ...
   *
   * @param fid target file ID to get file info for
   * @param payerID payer account ID, acting as payer for the transaction
   * @param nodeID node account id, default listening account
   * @return queried file info
   * @throws Exception indicates failure while getting file info for a file
   */
  public FileInfo getFileInfo(FileID fid, AccountID payerID, AccountID nodeID) throws Exception {
    long fileGetInfoCost = getFileGetInfoCost(fid, payerID, nodeID);

    Transaction paymentTxSigned =
        getQueryPaymentSignedWithFee(payerID, nodeID, "fileGetInfoQuery", fileGetInfoCost);
    ...
    if (code != ResponseCodeEnum.OK) {
      throw new Exception(
          "Precheck error geting file info! Precheck code = "
              + code.name()
              + "\nfileGetInfoQuery="
              + fileGetInfoQuery);
    }
  ...
  private long getFileGetInfoCost(FileID fid, AccountID payerID, AccountID nodeID)
      throws Exception {
    Transaction paymentTxSignedCost =
        getQueryPaymentSigned(payerID, nodeID, "fileGetInfoQueryCost");
    Query fileGetInfoQueryCost =
        RequestBuilder.getFileGetInfoBuilder(paymentTxSignedCost, fid, ResponseType.COST_ANSWER);
  ...
  protected void saveFile(byte[] content, String filePath) throws IOException {
    ...
    log.info(
        "File downloaded and saved at project root as: "
            + workDir
