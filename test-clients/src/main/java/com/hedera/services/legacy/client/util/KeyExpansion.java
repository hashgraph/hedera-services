...
public class KeyExpansion {
   ...
   */
  public static List<Key> genEd25519Keys(
   ...
   */
  private static Signature signBasic(
      final Key key, final Map<String, PrivateKey> pubKey2privKeyMap, final byte[] msgBytes)
      throws Exception {
    if (key.hasContractID()) {
      ...
      return Signature.newBuilder().setEd25519(sigBytes).build();
    } else if (!key.getECDSA384().isEmpty()) {
      final var pubKeyBytes = key.getECDSA384().toByteArray();
      final var sigBytes = signatureBytes(pubKeyBytes, pubKey2privKeyMap, msgBytes);
      return Signature.newBuilder().setECDSA384(sigBytes).build();
    }

    throw new Exception("Key type not implemented: key=" + key);
  }

  private static ByteString signatureBytes(
  ...
  public static byte[] addKeyMap(
      final KeyPair pair, final Map<String, PrivateKey> pubKey2privKeyMap) {
   ...
   */
  public static Key genKeyListInstance(
      final int numKeys, final Map<String, PrivateKey> pubKey2privKeyMap) {
   ...
   * @return generated threshold key
   */
  public static Key genThresholdKeyInstance(
   ...
   */
  public static void expandKeyMinimum4Signing(
      final Key key, int depth, final List<Key> expandedKeys) {
    if (!(key.hasThresholdKey() || key.hasKeyList())) {
   ...
   */
  public static SignaturePair signBasicAsSignaturePair(
      final Key key,
      final int prefixLen,
      final Map<String, PrivateKey> pubKey2privKeyMap,
      final byte[] msgBytes)
    ...
    final var pubKeyBytes = key.getEd25519().toByteArray();
    final var pubKeyHex = com.swirlds.common.CommonUtils.hex(pubKeyBytes);
    final var prefixBytes =
        prefixLen == -1 ? pubKeyBytes : CommonUtils.copyBytes(0, prefixLen, pubKeyBytes);
