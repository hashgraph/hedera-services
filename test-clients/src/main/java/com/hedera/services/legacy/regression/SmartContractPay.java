...
/**
 * Testing contract with a payable method
 *
 * @author Constantin
 */
public class SmartContractPay {
	...
	public static long ACCOUNT_FOR_CONTRACT_BALANCE = TestHelper.getContractMaxFee() * 100000000;
	...
	private ContractID createContract(AccountID payerAccount, FileID contractFile,
		...
		Transaction createContractRequest = TestHelper
				.getCreateContractRequestSigMap(payerAccount.getAccountNum(), payerAccount.getRealmNum(),
						...
						nodeAccount.getShardNum(), TestHelper.getContractMaxFee() * 10000000, timestamp,
						transactionDuration, true, "", 250000L, contractFile, ByteString.EMPTY, 0,
		...
		if (trRecord.hasContractCreateResult()) {
	...
	private byte[] callContract(AccountID payerAccount, ContractID contractToCall, byte[] data)
		...
		Transaction callContractRequest = TestHelper
				.getContractCallRequestSigMap(payerAccount.getAccountNum(), payerAccount.getRealmNum(),
						payerAccount.getShardNum(), node_account_number, 0l, 0l, TestHelper.getContractMaxFee() * 100,
						timestamp,
	...
	private AccountInfo getCryptoGetAccountInfo(AccountID payerAccount,
		...
		long fee = TestHelper.getCryptoMaxFee() * 100;
	...
	public void demo() throws Exception {
		...
		AccountID crAccount = createAccount(crAccountKeyPair, genesisAccount, ACCOUNT_FOR_CONTRACT_BALANCE);
		...
		if (crAccount != null) {
			...
			if (simpleStorageFileId != null) {
				...
				ContractID payTestContractId = createContract(crAccount, simpleStorageFileId, DAY_SEC * 1);
				...
				for (int i = 0; i < 10; i++) {
					...
					int sendFundsAmount = currValueToDeposit / 2;
					sendFunds(crAccount, payTestContractId, receiverSolitiyAcc, sendFundsAmount);
					long receiverBalance = getBalanceOf(crAccount, payTestContractId, receiverSolitiyAcc);
