...
public class ValidationCommand implements Callable<Integer> {
  ...
  @CommandLine.Parameters(
      ...
      paramLabel = "<services>",
      description =
          "one or more from { crypto, file, contract, consensus, token, scheduling }; or 'all'")
  ...
  private void ensureTemplatesExistFor(String[] entityLocs) {
    for (var entityLoc : entityLocs) {
      ...
      if (!f.exists()) {
        ...
        System.out.println(
            ". Creating " + entityLoc + " from resource template '" + tplResource + "'...");
  ...
  private boolean isRequested(String prefix) {
    return (services.length == 1 && "all".equals(services[0]))
        || Arrays.stream(services).anyMatch(prefix::equals);
  }
