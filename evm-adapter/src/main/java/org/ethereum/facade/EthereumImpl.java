...
public class EthereumImpl implements Ethereum, SmartLifecycle {
    ...
    private static final Logger logger = LogManager.getLogger("facade");
    private static final Logger gLogger = LogManager.getLogger("general");
    ...
    public EthereumImpl(final SystemProperties config, final CompositeEthereumListener compositeEthereumListener) {
        ...
        gLogger.debug("EthereumJ node started: enode://" + Hex.toHexString(config.nodeId()) + "@" + config.externalIp() + ":" + config.listenPort());
    }
    ...
    public BlockSummary replayBlock(Block block) {
        ...
        if (parent == null) {
            logger.debug("Failed to replay block #{}, its ancestor is not presented in the db", block.getNumber());
    ...
    public void stop(Runnable callback) {
        logger.debug("Shutting down Ethereum instance...");
        worldManager.close();
        callback.run();
    }
