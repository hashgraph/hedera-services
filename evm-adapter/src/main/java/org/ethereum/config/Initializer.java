...
class Initializer implements BeanPostProcessor {
    ...
    private void initConfig(SystemProperties config) {
        logger.debug("Running {},  core version: {}-{}", config.genesisInfo(), config.projectVersion(), config.projectVersionModifier());
        BuildInfo.printInfo();
        ...
        if (logger.isInfoEnabled()) {
            ...
            logger.debug("capability eth version: [{}]", versions);
        }
        logger.debug("capability shh version: [{}]", ShhHandler.VERSION);
        logger.debug("capability bzz version: [{}]", BzzHandler.VERSION);
     ...
     * Database version is stored in ${database}/ethjversion.properties
    ...
    public static class DatabaseVersionHandler {
        ...
        public void process(SystemProperties config) {
            ...
            if (isDatabaseDirectoryExists(config)) {
                ...
                if (actualVersion.equals(expectedVersion) || (isVersionFileNotFound && expectedVersion.equals(1))) {
                    logger.debug("Database directory location: '{}', version: {}", config.databaseDir(), actualVersion);
                } else {
                    ...
                    if (behavior == Behavior.EXIT) {
                        ...
                        // IGNORE
                        logger.debug("Continue working according to flag");
                    }
                }
            } else {
                putDatabaseVersion(config, config.databaseVersion());
                logger.debug("Created database version file");
            }
        }
        ...
        private File getDatabaseVersionFile(SystemProperties config) {
            return new File(config.databaseDir() + "/ethjversion.properties");
        }
    }
}
