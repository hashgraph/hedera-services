...
public class RepositoryImpl implements Repository, org.ethereum.facade.Repository {
    ...
    public synchronized void saveCode(byte[] addr, byte[] code) {
        byte[] codeHash = addr ; //HashUtil.sha3(code);
    ...
    public synchronized byte[] getCode(byte[] addr) {
        byte[] codeHash = addr ;//getCodeHash(addr);
        byte[] retVal = FastByteComparisons.equal(codeHash, HashUtil.EMPTY_DATA_HASH) ? (ByteUtil.EMPTY_BYTE_ARRAY) : codeCache.get(codeHash);
        if(retVal == null) {
          retVal = ByteUtil.EMPTY_BYTE_ARRAY;
        }
        return retVal;
    }
