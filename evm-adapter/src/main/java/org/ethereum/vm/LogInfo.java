...
public class LogInfo {
    ...
    @JsonProperty("topics")
    public List<String> getTopicsHs() {
    	List<String> listToreturn = null;
    	if(topics != null) {
    		listToreturn = new ArrayList<String>(topics.size());
    		for(DataWord currtopic : topics) {
    			listToreturn.add( Hex.toHexString(currtopic.getData()));
    		}
    	}
    ...
    @JsonProperty("bloom")
    public String getBloomHs() {
    	Bloom  currBloom = this.getBloom();
   	    return  Hex.toHexString(currBloom.getData());
    }
