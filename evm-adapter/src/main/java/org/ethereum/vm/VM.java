...
public class VM {
    ...
    private static final Logger dumpLogger = LogManager.getLogger("dump");
    private static BigInteger _32_ = BigInteger.valueOf(32);
    private static final String logString = "{}    Op: [{}]  Gas: [{}] Deep: [{}]  Hint: [{}]";
    // Disable log level checking for performance reasons
    private static final boolean LOGGING_ENABLED = logger.isDebugEnabled();
    ...
    private long sstore_word_cost = 0L;
			...
			*/
            ...
            switch (op) {
                ...
                case SSTORE:
                    ...
                    if (oldValue == null && !newValue.isZero()) {
                    	if(sstore_word_cost==0) {
                    		long durationInSeconds = program.getOwnerRemainingDuration();
                            int bytesize = DataWord.DATA_SIZE *2; //1 word for key storage and second  word for data
                            long gasPrice = ByteUtil.byteArrayToLong(program.getGasPrice().getData());
                            sstore_word_cost = Program.calculateStorageGasNeeded(bytesize, durationInSeconds,program.getSbh(), gasPrice);
                    	}
                        gasCost = sstore_word_cost;
                        storageGasUsed = gasCost;
                    
                    }else if (oldValue != null && !newValue.isZero()) {
                    	//in case of value update just charge the standart gas cost for reset
                ...
                case LOG4: 
                    ...
                    long gasPrice = ByteUtil.byteArrayToLong(program.getGasPrice().getData());
                    gasCost = Program.calculateStorageGasNeeded(logStorageTotalSize,program.getLogStorageDuration(),program.getRbh(), gasPrice);
                    storageGasUsed = gasCost;
                   break;
                case EXP:
            ...
            switch (op) {
                ...
                case ADD: {
                    ...
                    if (LOGGING_ENABLED && logger.isInfoEnabled())
                ...
                case CREATE: {
                    ...
                    if (LOGGING_ENABLED && logger.isDebugEnabled())
                ...
                break;
                case CREATE2: {
                    // Local convention: salt is discarded; behaves the same as CREATE
                    if (program.isStaticCall()) throw new Program.StaticCallModificationException();
            ...
            if (LOGGING_ENABLED && logger.isDebugEnabled() && !op.isCall())
                logger.debug(logString, String.format("%5s", "[" + program.getPC() + "]"),
                        String.format("%-12s",
