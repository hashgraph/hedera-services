...
public class Program {
	...
	private static final Logger logger = LogManager.getLogger("VM");

	private static final int LOG_CONTRACT_ID_SIZE = 24; // Contract ID could be represented by 8 bytes x 3 for realm ,
														// shar contract num
	private static final int LOG_TOPIC_SIZE = 32; // each topic consumes 32 bytes
	private static final int LOG_BLOOM_SIZE = 256;// bloom filter is 256 byted long
	/**
	...
	private Map<byte[], Long> remainingDurations = new HashMap<byte[], Long>();
	private NewAccountCreateAdapter newAcccountCreateAdapter;
	private byte[] systemFeeAccount;
	private long rbh; // tinybar per byte per hour
	private long sbh; // in tinybars
	private long defaultContractDuration; // duration in seconds
	private long logStorageDuration; // duration of transaction record to calculate log rbh
	...
	public Program(byte[] ops, ProgramInvoke programInvoke, Transaction transaction) {
		this(ops, programInvoke, transaction, SystemProperties.getDefault(),null, ByteUtil.EMPTY_BYTE_ARRAY, 1L, 1L,
				THIRTY_DAYS_IN_SECONDS, TWENTY_FIVE_HOURS_IN_SECONDS);
	}

	public Program(byte[] ops, ProgramInvoke programInvoke, Transaction transaction, SystemProperties config,NewAccountCreateAdapter accCreateAdapter, byte[] systemFeeAccount, long rbh,
			long sbh, long defaultContractDuration, long logStorageDuration) {
		this(null, ops, programInvoke, transaction, config, accCreateAdapter, systemFeeAccount, rbh,
				 sbh,  defaultContractDuration,  logStorageDuration);
	}

	public Program(byte[] codeHash, byte[] ops, ProgramInvoke programInvoke, Transaction transaction,
			SystemProperties config) {
		this(codeHash, ops, programInvoke, transaction, config, null, ByteUtil.EMPTY_BYTE_ARRAY, 1L, 1L,
				THIRTY_DAYS_IN_SECONDS, TWENTY_FIVE_HOURS_IN_SECONDS);
	}

	public Program(byte[] codeHash, byte[] ops, ProgramInvoke programInvoke, Transaction transaction,
			SystemProperties config, NewAccountCreateAdapter accCreateAdapter, byte[] systemFeeAccount, long rbh,
			long sbh, long defaultContractDuration, long logStorageDuration) {
		this.config = config;
		this.invoke = programInvoke;
		this.transaction = transaction;

		this.codeHash = codeHash == null || FastByteComparisons.equal(HashUtil.EMPTY_DATA_HASH, codeHash) ? null
				: codeHash;
		...
		this.newAcccountCreateAdapter = accCreateAdapter;
		this.systemFeeAccount = systemFeeAccount;
		this.rbh = rbh;
		this.sbh = sbh;
	...
	private InternalTransaction addInternalTx(byte[] nonce, DataWord gasLimit, byte[] senderAddress,
			byte[] receiveAddress, BigInteger value, byte[] data, String note) {
	...
	public void suicide(DataWord obtainerAddress) {
		...
		// if obtainer is the same as owner and there is some balance throw an exception
		if (FastByteComparisons.compareTo(owner, 0, 20, obtainer, 0, 20) == 0
				&& balance.compareTo(BigInteger.ZERO) != 0) {
			throw Program.Exception.sameOwnerObtainerException(this.systemFeeAccount);
		} else {
			...
			if (getStorage().isExist(obtainer) && !getStorage().getAccountState(obtainer).isDeleted()) {
				...
				throw Program.Exception.accountDoesNotExist(obtainer);
			}
		}
	...
	public void createContract(DataWord value, DataWord memStart, DataWord memSize) {
		...
		byte[] newAddress;
		if (newAcccountCreateAdapter == null) {
			throw Program.Exception.unableToGenerateContractIdException();
		} else {
			newAddress = newAcccountCreateAdapter.calculateNewAddress(getOwnerAddress().getLast20Bytes(), getStorage());
		}
		...
		if (byTestingSuite()) {
			...
			getResult().addCallCreate(programCode, EMPTY_BYTE_ARRAY, gasLimit.getNoLeadZeroesData(),
		...
		AccountState newAddr = track.createAccount(newAddress);
		newAddr.setSmartContract(true);
		DataWord currTimeStamp = getTimestamp();
		long timeStampInSeconds = ByteUtil.byteArrayToLong(currTimeStamp.getData());
		long expirationTime = timeStampInSeconds + this.defaultContractDuration;
		newAddr.setExpirationTime(expirationTime);

		if (newAcccountCreateAdapter != null) {
			newAcccountCreateAdapter.addCreatedContract(newAddress, getOwnerAddress().getLast20Bytes(), track);
		}
		...
		ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(this, new DataWord(newAddress),
				getOwnerAddress(), value, gasLimit, newBalance, null, track, this.invoke.getBlockStore(), false,
				byTestingSuite());

		ProgramResult result = ProgramResult.createEmpty();

		Program program = new Program(programCode, programInvoke, internalTx, config, this.newAcccountCreateAdapter, this.systemFeeAccount, this.rbh,
				this.sbh, this.defaultContractDuration, this.logStorageDuration).withCommonConfig(commonConfig);
		...
		if (result.getException() == null && !result.isRevert()) {
			...
			int codeLengthBytes = getLength(code);
			long gasPrice = ByteUtil.byteArrayToLong(getGasPrice().getData());
			long storageCost = Program.calculateStorageGasNeeded(codeLengthBytes, durationInSeconds, this.sbh, gasPrice);
			// long storageCost = getLength(code) *
			// getBlockchainConfig().getGasCost().getCREATE_DATA();
			...
			if (afterSpend < 0) {
				...
				result.setException(Program.Exception.notEnoughSpendingGas(
						"Contract size too large: " + getLength(result.getHReturn()), storageCost, this));
		...
		if (refundGas > 0) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("The remaining gas is refunded, account: [{}], gas: [{}] ",
	...
	public void callToAddress(MessageCall msg) {
		...
		// C.Z. 14-MARCH-2019 Check if address exists
		// P.L. 19-SEPT-2019  ...or is deleted.
		if (!getStorage().isExist(codeAddress) || getStorage().getAccountState(codeAddress).isDeleted()) {
			throw Program.Exception.accountDoesNotExist(codeAddress);
		}
		Repository track = getStorage().startTracking();
		...
		// throw an exception if it is a call to smart contract function (data is not
		// empty ) but the code is empty
		if (!isEmpty(data) && isEmpty(programCode)) {
			throw Program.Exception.emptyByteCodeException(codeAddress);
		}

		BigInteger contextBalance = ZERO;
		if (byTestingSuite()) {
			// This keeps track of the calls created for a test
			getResult().addCallCreate(data, contextAddress, msg.getGas().getNoLeadZeroesData(),
		...
		if (isNotEmpty(programCode)) {
			ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(this, new DataWord(contextAddress),
					msg.getType().callIsDelegate() ? getCallerAddress() : getOwnerAddress(),
					msg.getType().callIsDelegate() ? getCallValue() : msg.getEndowment(), msg.getGas(), contextBalance,
					data, track, this.invoke.getBlockStore(), msg.getType().callIsStatic() || isStaticCall(),
					byTestingSuite());

			VM vm = new VM(config);
			Program program = new Program(getStorage().getCodeHash(codeAddress), programCode, programInvoke, internalTx,
					config, this.newAcccountCreateAdapter, this.systemFeeAccount, this.rbh, this.sbh,
					this.defaultContractDuration, this.logStorageDuration).withCommonConfig(commonConfig);
			vm.play(program);
			result = program.getResult();
	...
	public void refundGas(long gasValue, String cause) {
		logger.debug("[{}] Refund for cause: [{}], gas: [{}]", invoke.hashCode(), cause, gasValue);
	...
	public byte[] getCodeHashAt(DataWord address) {
		...
		// Change to ethvm behavior. Return empty if no such contract, else ultimately
		// the address
	...
	public DataWord getBlockHash(int index) {
		 ...
		 * this.getNumber().intValue()) - 256 ? new
		 * DataWord(this.invoke.getBlockStore().getBlockHashByNumber(index,
		 * getPrevHash().getData())).clone() : DataWord.ZERO.clone()
		 */;
	...
	public void fullTrace() {
		if (logger.isTraceEnabled() || listener != null) {
			...
			/* getStorage() is not implemented in RepositoryImpl hence commented below */
			// if (contractDetails != null) {
			// List<DataWord> storageKeys = new
			// ArrayList<>(contractDetails.getStorage().keySet());
			...
			if (memory.size() > 320)
				...
				for (int i = 0; i < memory.size(); ++i) {
					...
					if ((i + 1) % 16 == 0) {
						String tmp = format("[%4s]-[%4s]", Integer.toString(i - 15, 16), Integer.toString(i, 16))
								.replace(" ", "0");
			...
			logger.trace("\n  Spent Gas: [{}]/[{}]\n  Left Gas:  [{}]\n", getResult().getGasUsed(),
					invoke.getGas().longValue(), getGas().longValue());
	...
	public void callToPrecompiledAddress(MessageCall msg, PrecompiledContract contract) {
		...
		if (byTestingSuite()) {
			...
			this.getResult().addCallCreate(data, msg.getCodeAddress().getLast20Bytes(),
					msg.getGas().getNoLeadZeroesData(), msg.getEndowment().getNoLeadZeroesData());
	...
	public static class InvalidAccountAddressException extends BytecodeExecutionException {
		public InvalidAccountAddressException(byte[] accountAddress) {
			super(String.format("Account  (%s) does not exist", ByteUtil.toHexString(accountAddress)));
		}
	}

	@SuppressWarnings("serial")
	public static class SameOwnerObtainerOnSelfSestructException extends BytecodeExecutionException {
		public SameOwnerObtainerOnSelfSestructException(byte[] accountAddress) {
			super(String.format("Contract adress (%s) can not be used as a balance transfer target for self destruct",
					ByteUtil.toHexString(accountAddress)));
		}
	}
	...
	public static class EmptyByteCodeException extends BytecodeExecutionException {
		public EmptyByteCodeException(byte[] accountAddress) {
			super(String.format("Bytecode for ontract adress (%s) is empty", ByteUtil.toHexString(accountAddress)));
		}
	}
	...
	public static class Exception {
		...
		public static InvalidAccountAddressException accountDoesNotExist(byte[] accountAddress) {
			return new InvalidAccountAddressException(accountAddress);
		}

		public static SameOwnerObtainerOnSelfSestructException sameOwnerObtainerException(byte[] accountAddress) {
			return new SameOwnerObtainerOnSelfSestructException(accountAddress);
		}

		public static EmptyByteCodeException emptyByteCodeException(byte[] accountAddress) {
			return new EmptyByteCodeException(accountAddress);
		}
		
		public static UnableToGenerateContractIdException unableToGenerateContractIdException() {
			return new UnableToGenerateContractIdException();
		}
	...
	public long geRemainingDuration(byte[] addressToCheck, Storage storage) {
		long durationToReturn = 0;
		// byte[] senderAddress = this.getOwnerAddress().getLast20Bytes();
		if (remainingDurations.containsKey(addressToCheck)) {
			durationToReturn = remainingDurations.get(addressToCheck);
		} else {
			AccountState accState = storage.getAccountState(addressToCheck);
			if (accState != null) {
				long contractExpirationSec = accState.getExpirationTime();
				DataWord currTimeStamp = getTimestamp();
				long timeStampInSeconds = ByteUtil.byteArrayToLong(currTimeStamp.getData());
				durationToReturn = contractExpirationSec - timeStampInSeconds;
				remainingDurations.put(addressToCheck, durationToReturn);
			}
		}
	...
	public long getOwnerRemainingDuration(Storage storage) {
		...
		return geRemainingDuration(ownerAddress, storage);
	}
	...
	public static long calculateStorageGasNeeded(long numberOfBytes, long durationInSeconds,
			long byteHourCostIntinybars, long gasPrice) {
