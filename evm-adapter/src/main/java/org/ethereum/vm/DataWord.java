 ...
 * @since 01.06.2014
 */
public class DataWord implements Comparable<DataWord> , Serializable {
    ...
    public static final DataWord ZERO = new DataWord(new byte[DATA_SIZE]);      // don't push it in to the stack
    ...
    private DataWord(ByteBuffer buffer) {
        final ByteBuffer data = ByteBuffer.allocate(DATA_SIZE);
        final byte[] array = buffer.array();
        System.arraycopy(array, 0, data.array(), DATA_SIZE - array.length, array.length);
    ...
    public DataWord(byte[] data) {
        if (data == null)
            ...
            System.arraycopy(data, 0, this.data, DATA_SIZE - data.length, data.length);
        else
     ...
     * @param arg
     */
    public void shiftLeft(DataWord arg) {
     ...
     * @param arg
     */
    public void shiftRight(DataWord arg) {
        if (arg.value().compareTo(BigInteger.valueOf(MAX_POW)) >= 0) {
            this.data = new byte[DATA_SIZE];
            return;
        }
     ...
     * @param arg
     */
    public void shiftRightSigned(DataWord arg) {
        if (arg.value().compareTo(BigInteger.valueOf(MAX_POW)) >= 0) {
            if (this.isNegative()) {
