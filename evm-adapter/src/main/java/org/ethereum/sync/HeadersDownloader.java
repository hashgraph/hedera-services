...
public class HeadersDownloader extends BlockDownloader {
    ...
    public HeadersDownloader(BlockHeaderValidator headerValidator) {
        ...
        logger.debug("HeaderDownloader created.");
    }

    public void init(byte[] startFromBlockHash) {
        if(logger.isDebugEnabled())
            logger.debug("HeaderDownloader init: startHash = " + Hex.toHexString(startFromBlockHash));
    ...
    protected void pushHeaders(List<BlockHeaderWrapper> headers) {
        ...
        if(logger.isDebugEnabled())
            logger.debug(headers.size() + " headers loaded: " + headers.get(0).getNumber() + " - " + headers.get(headers.size() - 1).getNumber());
