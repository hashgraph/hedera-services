...
public abstract class BlockDownloader {
    ...
    private final static Logger logger = LogManager.getLogger("sync");
    ...
    public void init(SyncQueueIfc syncQueue, final SyncPool pool) {
        ...
        logger.debug("Initializing BlockDownloader.");
            ...
            try {
                    if (hReq.isEmpty()) {
                        synchronized (this) {
                            ...
                            if (hReq == null) {
                                logger.debug("Headers download complete.");
                    ...
                    for (Iterator<SyncQueueIfc.HeadersRequest> it = hReq.iterator(); it.hasNext();) {
                        ...
                        if (any == null) {
                            ...
                            if(logger.isDebugEnabled()) {
                                logger.debug(
                                    "headerRetrieveLoop: request headers (" + headersRequest
                                        .getStart() + ") from " + any.getNode());
                            }
                            ...
                            if (futureHeaders != null) {
                                Futures.addCallback(futureHeaders, new FutureCallback<List<BlockHeader>>() {
                                    ...
                                    public void onFailure(Throwable t) {
                                        if(logger.isDebugEnabled()) {
                                            logger.debug(
                                                "Error receiving headers. Dropping the peer.", t);
                                        }
        ...
        while(!Thread.currentThread().isInterrupted()) {
            try {
                ...
                if (bReqs.isEmpty() && headersDownloadComplete) {
                    logger.debug("Block download complete.");
                    finishDownload();
                    downloadComplete = true;
                    return;
                }
                        ...
                        for (BlockHeaderWrapper blockHeaderWrapper : bReqs.get(0).getBlockHeaders()) {
                            ...
                            if (channel != null) {
                                ...
                                if (futureBlocks != null) {
                                    Futures.addCallback(futureBlocks, new BlocksCallback(channel),MoreExecutors.directExecutor());
                                }
                            }
                        }
                    }
...
//                    for (SyncQueueIfc.BlocksRequest blocksRequest : bReq.split(MAX_IN_REQUEST)) {
                        ...
                        if (any == null) {
                            ...
                            if(logger.isDebugEnabled()) {
                                logger.debug("blockRetrieveLoop: Requesting " + blocksRequest
                                    .getBlockHeaders().size() + " blocks from " + any.getNode());
                            }
                            ListenableFuture<List<Block>> futureBlocks =
                                    any.getEthHandler().sendGetBlockBodies(blocksRequest.getBlockHeaders());
                            blocksRequested += blocksRequest.getBlockHeaders().size();
                            if (futureBlocks != null) {
                                Futures.addCallback(futureBlocks, new BlocksCallback(any),MoreExecutors.directExecutor());
                                reqBlocksCounter++;
                                it.remove();
                            }
                        }
        ...
        synchronized (this) {
            if(logger.isDebugEnabled()) {
                logger.debug("Adding new " + blocks.size() + " blocks to sync queue: " +
                    blocks.get(0).getShortDescr() + " ... " + blocks.get(blocks.size() - 1)
                    .getShortDescr());
            }
            ...
            if(logger.isDebugEnabled()) {
                logger.debug(
                    "Pushing " + wrappers.size() + " blocks to import queue: " + (wrappers.isEmpty()
                        ? "" :
                        wrappers.get(0).getBlock().getShortDescr() + " ... " + wrappers
                            .get(wrappers.size() - 1).getBlock().getShortDescr()));
            }
            pushBlocks(wrappers);
        }
