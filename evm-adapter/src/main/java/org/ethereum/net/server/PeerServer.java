...
public class PeerServer {
    ...
    public void start(int port) {
        ...
        try {
            ...
            // Start the client.
            logger.debug("Listening for incoming connections, port: [{}] ", port);
            logger.debug("NodeId: [{}] ", Hex.toHexString(config.nodeId()));
    ...
    public void close() {
        if (listening && channelFuture != null && channelFuture.channel().isOpen()) {
            try {
                logger.debug("Closing PeerServer...");
                channelFuture.channel().close().sync();
                logger.debug("PeerServer closed.");
            } catch (Exception e) {
