...
public class Eth62 extends EthHandler {
    ...
    protected final static Logger logger = LogManager.getLogger("sync");
    protected final static Logger loggerNet = LogManager.getLogger("net");
    ...
    protected synchronized void processBlockBodies(BlockBodiesMessage msg) {
        ...
        try {
            ...
            logger.debug("Fatal validation error while processing block bodies from peer {}", channel.getPeerIdShort());
        }
        ...
        if (ethState == EthState.STATUS_SENT) {
            ...
            if (validator != null) {
                ...
                if (result.success) {
                    ...
                    if(logger.isDebugEnabled()) {
                        logger
                            .debug("Peer {}: wrong fork ({}). Drop the peer and reduce reputation.",
                                channel.getPeerIdShort(), result.error);
                    }
    ...
    private List<Block> validateAndMerge(BlockBodiesMessage response) {
        ...
        if (bodies.hasNext()) {
            logger.debug("Peer {}: invalid BLOCK_BODIES response: at least one block body doesn't correspond to any of requested headers: ",
    ...
    public synchronized void dropConnection() {
        logger.debug("Peer {}: is a bad one, drop", channel.getPeerIdShort());
