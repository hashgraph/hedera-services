...
public class BloomedSource extends AbstractChainedSource<byte[], byte[], byte[], byte[]> {
    private final static Logger logger = LogManager.getLogger("db");
        ...
        if (filter != null) {
            ...
            if (filter.getAllocatedBytes() > maxBloomSize) {
                if(logger.isDebugEnabled()) {
                    logger.debug("Bloom filter became too large (" + filter.getAllocatedBytes()
                        + " exceeds max threshold " + maxBloomSize
                        + ") and is now disabled forever.");
                }
                getSource().put(filterKey, new byte[0]);
                filter = null;
                dirty = false;
