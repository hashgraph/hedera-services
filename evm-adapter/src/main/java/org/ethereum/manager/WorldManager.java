...
public class WorldManager {
    ...
    public void addListener(EthereumListener listener) {
        logger.debug("Ethereum listener added");
    ...
    public void loadBlockchain() {
        ...
        if (blockStore.getBestBlock() == null) {
            logger.debug("DB is empty - adding Genesis");
...
//            repository.dumpState(Genesis.getInstance(config), 0, 0, null);
            ...
            logger.debug("*** Loaded up to block [{}] totalDifficulty [{}] with stateRoot [{}]",
        ...
        if (config.rootHashStart() != null) {
            ...
            logger.debug("Loading root hash from property file: [{}]", config.rootHashStart());
