 ...
 */
public class Ethash {
    private static final Logger logger = LogManager.getLogger("mine");
            ...
            if (fileCacheEnabled && file.canRead()) {
                try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                    if(logger.isDebugEnabled())
                        logger.debug("Loading light dataset from " + file.getAbsolutePath());
                    long bNum = ois.readLong();
                    if (bNum == blockNumber) {
                        cacheLight = (int[]) ois.readObject();
                        logger.debug("Dataset loaded.");
                    } else {
                        logger.debug("Dataset block number miss: " + bNum + " != " + blockNumber);
                    }
    ...
    public synchronized int[] getFullDataset() {
        if (fullData == null) {
            ...
            if (fileCacheEnabled && file.canRead()) {
                try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                    logger.debug("Loading dataset from " + file.getAbsolutePath());
                    long bNum = ois.readLong();
                    if (bNum == blockNumber) {
                        ...
                        if(logger.isDebugEnabled()) {
                            logger
                                .debug("Dataset block number miss: " + bNum + " != " + blockNumber);
                        }
                    }
                } catch (IOException | ClassNotFoundException e) {
                    throw new RuntimeException(e);
            ...
            if (fullData == null){
                ...
                logger.debug("Calculating full dataset...");
                fullData = getEthashAlgo().calcDataset(getFullSize(), getCacheLight());
                logger.debug("Full dataset calculated.");

                if (fileCacheEnabled) {
                    file.getParentFile().mkdirs();
                    try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(file))) {
                        if(logger.isDebugEnabled()) {
                            logger.debug("Writing dataset to " + file.getAbsolutePath());
                        }
                        oos.writeLong(blockNumber);
