...
public class BlockchainImpl implements Blockchain, org.ethereum.facade.Blockchain {
    ...
    private static final Logger logger = LogManager.getLogger("blockchain");
    private static final Logger stateLogger = LogManager.getLogger("state");
    ...
    private synchronized BlockSummary tryConnectAndFork(final Block block) {
        ...
        if (isMoreThan(this.totalDifficulty, savedState.savedTD)) {
            if(logger.isDebugEnabled()) {
                logger.debug("Rebranching: {} ~> {}", savedState.savedBest.getShortHash(),
                    block.getShortHash());
            }
...
//                }
    ...
    private BlockSummary applyBlock(Repository track, Block block) {
        if(logger.isDebugEnabled()) {
            logger.debug("applyBlock: block: [{}] tx.list: [{}]", block.getNumber(),
        ...
        for (Transaction tx : block.getTransactionsList()) {
            ...
            stateLogger.debug("block: [{}] executed tx: [{}] \n  state: [{}]", block.getNumber(), i,
                    Hex.toHexString(track.getRoot()));

            stateLogger.debug("[{}] ", receipt.toString());

            if (stateLogger.isInfoEnabled())
                stateLogger.debug("tx[{}].receipt: [{}] ", i, Hex.toHexString(receipt.getEncoded()));
        ...
        stateLogger.debug("applied reward for block: [{}]  \n  state: [{}]",
                block.getNumber(),
                Hex.toHexString(track.getRoot()));
        ...
        if(logger.isDebugEnabled()) {
            logger.debug("block: num: [{}] hash: [{}], executed after: [{}]nano", block.getNumber(),
                block.getShortHash(), totalTime);
        }
    ...
    public synchronized void storeBlock(Block block, List<TransactionReceipt> receipts) {
        ...
        if (block.getNumber() % 100 == 0)
            logger.debug("*** Last block added [ #{} ]", block.getNumber());
