...
public class TransactionExecutor {
    ...
    public void execute() {
        ...
        if (!localCall) {
            ...
            if (logger.isDebugEnabled())
                logger.debug("Paying: txGasCost: [{}], gasPrice: [{}], gasLimit: [{}]", txGasCost, toBI(tx.getGasPrice()), txGasLimit);
        }
    ...
    private void create() {
        ...
        if (isEmpty(tx.getData())) {
            ...
            this.vm = new VM(config);
            this.program = new Program(tx.getData(), programInvoke, tx, config,null, ByteUtil.EMPTY_BYTE_ARRAY, 1L, 1L,
    				1L, 1L).withCommonConfig(commonConfig);
        }
    ...
    public TransactionExecutionSummary finalization() {
        ...
        logger.debug("Pay total refund to sender: [{}], refund val: [{}]", Hex.toHexString(tx.getSender()), summary.getRefund());
