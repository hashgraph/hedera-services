...
public final class VirtualHasher<K extends VirtualKey, V extends VirtualValue> {
    ...
    /**
     * This atomic reference is used to serialize (non-deterministically) all notification tasks.
     */
    private final AtomicReference<NotificationTask> nextTaskPlaceHolder = new AtomicReference<>();
    ...
    class NotificationTask extends AbstractTask {
        ...
        NotificationTask(final ForkJoinPool pool, final int dependencies) {
            super(pool, dependencies);
        }

        void setData(VirtualLeafRecord<K, V> leaf, long path, Hash hash, AbstractTask nextTask) {
            this.leaf = leaf;
            this.path = path;
            this.hash = hash;
    ...
    class ChunkHashTask extends HashHoldingTask {
        ...
        void scheduleNotificationTask(VirtualLeafRecord<K, V> leaf, long path, Hash hash) {
            NotificationTask currentTask = nextTaskPlaceHolder.get();
            if (path == ROOT_PATH) {
                currentTask.setData(leaf, path, hash, resultTask);
            } else {
                final NotificationTask nextTask = new NotificationTask(HASHING_POOL, 2);
                while (!nextTaskPlaceHolder.compareAndSet(currentTask, nextTask)) {
                    currentTask = nextTaskPlaceHolder.get();
                }
                currentTask.setData(leaf, path, hash, nextTask);
            }
        }
        ...
        protected boolean exec() {
            try {
                ...
                if (leaf != null) {
                    ...
                    scheduleNotificationTask(leaf, path, hash);
                } else {
                    int len = 1 << height;
                    ...
                    while (len > 1) {
                        for (int i = 0; i < len / 2; i++) {
                            ...
                            if ((left == null) && (right == null)) {
                                ...
                                scheduleNotificationTask(null, hashedPath, ins[i]);
                            }
                        }
    ...
    public Hash hash(
        ...
        // Initialize the notification task series
        NotificationTask notificationTask = new NotificationTask(HASHING_POOL, 1);
        nextTaskPlaceHolder.set(notificationTask);

        // This map contains all tasks created, but not scheduled for execution yet
        final HashMap<Long, ChunkHashTask> map = new HashMap<>();
        // The result task. When the root task is done executing, that is it produced
        // a root hash, this hash is set as an input dependency for this result task, where
        // it's read and returned in the end of this method. The other dependency is the
        // root notification task.
        resultTask = new HashHoldingTask(HASHING_POOL, 2, 1);
