 ...
 * and {@link #dirtyHashes(long, long)} methods and sending them to the code responsible for flushing. The cache
...
public final class VirtualNodeCache<K extends VirtualKey<? super K>, V extends VirtualValue>
    ...
    public Stream<VirtualLeafRecord<K, V>> deletedLeaves() {
        ...
        final StandardFuture<Void> result = dirtyLeaves.parallelTraverse(CLEANING_POOL, element -> {
            if (element.deleted) {
                K key = element.key;
                Mutation<K, VirtualLeafRecord<K, V>> mutation = lookup(keyToDirtyLeafIndex.get(key));
    ...
    public void putHash(final VirtualInternalRecord node) {
        Objects.requireNonNull(node);
        putHash(node.getPath(), node.getHash());
    }
     ...
     *
     * @param lastLeafPath
     * 		The last leaf path at and above which no internal node results should be returned. It is possible,
     ...
     */
    public Stream<VirtualInternalRecord> dirtyHashes(long firstLeafPath, long lastLeafPath) {
        ...
        return dirtyHashes
                ...
                .filter(mutation -> !mutation.deleted)
                .map(mutation ->
                        new VirtualInternalRecord(mutation.key, mutation.value != NULL_HASH ? mutation.value : null));
    }
    ...
    private Mutation<K, VirtualLeafRecord<K, V>> mutate(
            VirtualLeafRecord<K, V> leaf, Mutation<K, VirtualLeafRecord<K, V>> mutation) {
     ...
     */
    private static <K, V> void purge(ConcurrentArray<Mutation<K, V>> array, Map<K, Mutation<K, V>> index) {
