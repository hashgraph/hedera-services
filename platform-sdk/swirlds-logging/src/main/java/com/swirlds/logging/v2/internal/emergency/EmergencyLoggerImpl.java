...
package com.swirlds.logging.v2.internal.emergency;
...
import com.swirlds.logging.v2.internal.format.LineBasedFormat;
...
public class EmergencyLoggerImpl implements EmergencyLogger {
    ...
    @Override
    public void log(@NonNull final Level level, @NonNull final String message, @Nullable final Throwable thrown) {
    ...
    @Override
    public void log(@NonNull final LogEvent event) {
        if (event == null) {
            logNPE("event");
            return;
        }
     ...
     */
    private boolean isLoggable(@NonNull final Level level) {
        if (level == null) {
            logNPE("level");
            return true;
        }
    ...
    private <T> T callGuarded(
            @Nullable final LogEvent fallbackLogEvent,
        ...
        if (guard != null && guard) {
            ...
            try {
                ...
                final LogEvent logEvent =
                        new LogEvent(Level.ERROR, EMERGENCY_LOGGER_NAME, "Error in Emergency logger", t);
                handle(logEvent);
                if (fallbackLogEvent != null) {
                    handle(fallbackLogEvent);
                }
    ...
    public List<LogEvent> publishLoggedEvents() {
        final List<LogEvent> result = List.copyOf(logEvents);
        logEvents.clear();
        return result;
    }
