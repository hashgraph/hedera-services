...
import com.swirlds.baseapi.service.Service;
...
public class AdapterHandler<T> implements HttpHandler {
    ...
    protected AdapterHandler(final @NonNull PlatformContext context, Service<T> delegatedService, final String path) {
        ...
        this.tps = new CountPerSecond(
                context.getMetrics(), ApplicationMetrics.TRANSACTION_PER_SECOND);

    }

    /**
     * Handle the given request and generate an appropriate response. See {@link HttpHandler} for a description of the
    ...
    public void handleRequest(HttpServerExchange exchange) {
        ...
        try {
            switch (requestMethod) {
                case "GET":
                    result = (isId && !id.contains("?")) ? retrieve(id)
        ...
        log.trace("Received Request to:{}{} took:{} answered status:{} body:{}", requestMethod,
                exchange.getRequestURL(),
                duration, statusCode, response);
    }
