...
class StateUtilsTest extends MerkleTestBase {
    ...
    void nullServiceNameThrows() {
        ...
        assertThatThrownBy(() -> StateMetadata.validateServiceName(null)).isInstanceOf(NullPointerException.class);
    }

    @Test
    @DisplayName("Validating a service name with no characters throws an exception")
    void emptyServiceNameThrows() {
        assertThatThrownBy(() -> StateMetadata.validateServiceName(""))
    ...
    void invalidServiceNameThrows(final String serviceName) {
        assertThatThrownBy(() -> StateMetadata.validateServiceName(serviceName))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void validServiceNameWorks(final String serviceName) {
        assertThat(StateMetadata.validateServiceName(serviceName)).isEqualTo(serviceName);
    }
    ...
    void validStateKeyWorks(final String stateKey) {
        assertThat(StateMetadata.validateServiceName(stateKey)).isEqualTo(stateKey);
    }
    ...
    void nullIdentifierThrows() {
        //noinspection DataFlowIssue
        assertThatThrownBy(() -> StateMetadata.validateIdentifier(null)).isInstanceOf(NullPointerException.class);
    }

    @Test
    @DisplayName("Validating an identifier with no characters throws an exception")
    void emptyIdentifierThrows() {
        assertThatThrownBy(() -> StateMetadata.validateIdentifier("")).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void invalidIdentifierThrows(final String identifier) {
        assertThatThrownBy(() -> StateMetadata.validateIdentifier(identifier))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void validIdentifierWorks(final String identifier) {
        assertThat(StateMetadata.validateIdentifier(identifier)).isEqualTo(identifier);
    }
    ...
    void computeClassId() {
        final var classId = StateMetadata.hashString("A:B:v1.0.0:C");
        assertThat(StateMetadata.computeClassId("A", "B", version(1, 0, 0), "C"))
                .isEqualTo(classId);
    }
    ...
    void computeClassId_withMetadata() {
        setupFruitMerkleMap();
        final var classId = StateMetadata.hashString(FIRST_SERVICE
                + ":"
                + FRUIT_STATE_KEY
                + ":v"
                + TEST_VERSION.major()
                ...
                + TEST_VERSION.patch()
                + ":C");
        assertThat(StateMetadata.computeClassId(FIRST_SERVICE, FRUIT_STATE_KEY, TEST_VERSION, "C"))
                .isEqualTo(classId);
    }
    ...
    void uniqueHashing() {
        ...
        for (final var serviceName : fakeServiceNames) {
            for (final var stateKey : fakeStateKeys) {
                final var hash = StateMetadata.computeClassId(serviceName, stateKey, TEST_VERSION, "extra string");
                hashes.add(hash);
            }
        }
