...
public abstract class LongList implements CASable, Closeable {
    ...
    /**
     * The current maximum index that can be stored. This is determined dynamically based on the actual indexes used. It
     * will grow, but never shrinks. Ultimately it would be nice to have some way to shrink this based on knowledge
     * other parts of the system have about how many hashes need to be stored, but in the real system, this isn't
     * important because we don't shrink the state size that dramatically, and on a reboot, this would be reset anyway.
     */
    ...
    /**
     * Get the maximum capacity of this LongList; that is, one greater than the maximum legal value of an {@code index}
     * parameter used in a {@code put()} call.
     */
     ...
     * <p><b>
     * It is not guaranteed what version of data will be written if the LongList is changed via put methods while this
     * LongList is being written to a file. If you need consistency while calling put concurrently then use a
