...
import static com.swirlds.jasperdb.utilities.HashTools.HASH_SIZE_BYTES;
import static com.swirlds.jasperdb.utilities.HashTools.byteBufferToHash;
import static com.swirlds.jasperdb.utilities.HashTools.hashToByteBuffer;
...
public final class HashListByteBuffer implements HashList {
    ...
    public HashListByteBuffer(Path file) throws IOException {
        try (FileChannel fc = FileChannel.open(file, StandardOpenOption.READ)) {
            ...
            JasperDBFileUtils.completelyRead(fc, headerBuffer);
            headerBuffer.rewind();
            ...
            for (int i = 0; i < numOfBuffers; i++) {
                ...
                JasperDBFileUtils.completelyRead(fc, buffer);
                buffer.position(0);
                data.add(buffer);
            }
        }
    ...
    public void writeToFile(Path file) throws IOException {
        ...
        try (final FileChannel fc = FileChannel.open(file, StandardOpenOption.CREATE, StandardOpenOption.WRITE)) {
            ...
            headerBuffer.flip();
            JasperDBFileUtils.completelyWrite(fc, headerBuffer);
            // write data
            for (int i = 0; i < numOfBuffers; i++) {
    ...
    public String toString() {
        return "HashListByteBuffer{" + "num of buffers="
                + data.size() + ", maxIndexThatCanBeStored="
                + maxIndexThatCanBeStored + '}';
    }
}
