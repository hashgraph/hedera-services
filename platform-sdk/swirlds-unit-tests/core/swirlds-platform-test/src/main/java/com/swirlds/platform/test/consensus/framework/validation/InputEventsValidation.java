...
public class InputEventsValidation {
    public static void validateInputsAreTheSame(
            final ConsensusOutput output1, final ConsensusOutput output2) {
        ...
        assertBaseEventLists(
                "Verifying input events", output1.sortedAddedEvents(), output2.sortedAddedEvents());
    }
    ...
    public static void assertBaseEvents(
            final String description, final EventImpl e1, final EventImpl e2) {
        if (!Objects.equals(e1.getBaseEvent(), e2.getBaseEvent())) {
            final String sb =
                    description
                            + "\n"
