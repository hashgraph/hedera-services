...
public final class ConsensusTestDefinitions {
	...
	public static void removeNode(final TestInput input) {
		...
		final ConsensusTestOrchestrator orchestrator2 =
				OrchestratorBuilder.builder().setTestInput(input.setNumberOfNodes(input.numberOfNodes() - 1)).build();
		for (int i = 0; i < 2; i++) {
			orchestrator2.getNodes().get(i).getIntake().loadSnapshot(
					orchestrator1.getNodes().get(i).getIntake().getLatestRound().getSnapshot()
			);
			final int fi = i;
