...
class ActiveStatusLogicTests {
    ...
    @Test
    @DisplayName("Go from ACTIVE to FREEZING")
    void activeToFreezing() {
        triggerActionAndAssertTransition(logic, PlatformStatusAction.FREEZE_PERIOD_ENTERED, PlatformStatus.FREEZING);
    }

    @Test
    @DisplayName("Go from ACTIVE to CHECKING")
    void activeToChecking() {
    ...
    //    DONE_REPLAYING_EVENTS         X
    //    OWN_EVENT_REACHED_CONSENSUS   X
    //    FREEZE_PERIOD_ENTERED         X
    //    FALLEN_BEHIND                 X
    //    RECONNECT_COMPLETE            X
    //    STATE_WRITTEN_TO_DISK         X
    //    CATASTROPHIC_FAILURE          X
    //    TIME_ELAPSED                  X
