 ...
 *                                              working directory, unless the provided path begins with "/", in which
 *                                              case it will be interpreted as an absolute path.
 * @param mainClassNameOverride                 Typically, the fully qualified name of the application class
 *                                              implementing {@link com.swirlds.common.system.SwirldMain SwirldMain} is
 *                                              used as a directory name when saving signed states. If this property is
 *                                              not the empty string then it overrides the main class name for signed
 *                                              states.
 * @param cleanSavedStateDirectory              If true, clean out all data in the {@link #savedStateDirectory} except
 *                                              for the previously saved state.
 * @param stateSavingQueueSize                  The number of states permitted to sit in the signed state file manager's
 *                                              queue of states being written. If this queue backs up then some states
 ...
 *                                              affect the performance of the node where the ISS occurs. This feature is
 ...
 *                                              Halt this node whenever any ISS in the network is detected. A halt
 *                                              causes the node to stop doing work, but does not shut down the JVM.
 *                                              </p>
 *
 *                                              <p>
 ...
 *                                              detected.
 * @param haltOnCatastrophicIss                 If true, then halt this node if a catastrophic ISS is detected. A halt
 *                                              causes the node to stop doing work, but does not shut down the JVM.
 * @param secondsBetweenISSDumps                If one ISS is detected, it is likely that others will be detected
 *                                              shortly afterward. Specify the minimum time, in seconds, that must
 *                                              transpire after dumping a state before another state dump is permitted.
 ...
 * @param maxAgeOfFutureStateSignatures         It's possible to receive state signatures before it's time to process
 *                                              the round signed by the signature. This is the maximum number of rounds,
 *                                              in the future, for which a node will accept a state signature.
 * @param roundsToKeepForSigning                The maximum number of rounds that a state will be kept in memory while
