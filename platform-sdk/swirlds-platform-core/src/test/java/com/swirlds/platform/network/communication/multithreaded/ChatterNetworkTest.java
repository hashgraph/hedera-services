...
import com.swirlds.platform.sync.ConnectionFactory;
...
class ChatterNetworkTest {
    ...
    void transmitMessages()
        ...
        Awaitility.await().atMost(WAIT_TIME, WAIT_TIME_UNIT).until(() -> peer1.isDone() && peer2.isDone());
        ...
        Awaitility.await().atMost(WAIT_TIME, WAIT_TIME_UNIT).until(future1::isDone);
        Awaitility.await().atMost(WAIT_TIME, WAIT_TIME_UNIT).until(future2::isDone);
