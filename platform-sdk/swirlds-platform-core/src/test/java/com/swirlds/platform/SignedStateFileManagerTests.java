...
class SignedStateFileManagerTests {
    ...
    //     * Make sure the signed state was properly saved.
    //     */
    //    private void validateSavingOfState(final SignedState originalState, final Path stateDirectory) throws
    // IOException {
    //
    ...
    //                "hash should match");
    //        assertEquals(originalState.getState().getHash(), deserializedSignedState.originalHash(), "hash should
    // match");
    //    }
    //
    //    @ParameterizedTest
    ...
    //
    //        if (!successExpected) {
    //            // To make the save fail, create a file with the name of the directory the state will try to be saved
    // to
    //            final Path savedDir =
    ...
    //        }
    //
    //        assertEquals(successExpected, saveSucceeded.get(), "Invalid 'success' value passed to
    // StateToDiskConsumer");
    //
    //        // cleanup
    //        manager.stop();
    ...
    //        thread.join(1000);
    //
    //        final Path stateDirectory = testDirectory.resolve("fatal").resolve("node1234_round" +
    ...
    //
    //        final Path stateDirectory = testDirectory.resolve("iss").resolve("node1234_round" +
    // signedState.getRound());
    //        validateSavingOfState(signedState, stateDirectory);
    //
    //        // cleanup
    ...
    //
    //    /**
    //     * Helper method for {@link #maxCapacityTest()}. Add states to the queue, which will eventually become
    // blocked.
    //     */
    //    private void addToQueue(
    //            final SignedStateFileManager manager, final int stateIndex, final SignedState state, final int
    // queueSize) {
    //
    ...
    //            assertEventuallyEquals(
    //                    stateIndex + 1, statesWritten::get, Duration.ofSeconds(1), "state should eventually be
    // saved");
    //            assertEventuallyEquals(
    ...
    //
    //    /**
    //     * Simulate a sequence of states where a state is saved periodically. Ensure that the proper states are saved,
    // and
    ...
    //                    Math.max(1, random.nextGaussian() * standardDeviationTimeBetweenStates +
    ...
    //                                    SignedStateFileReader.getSavedStateFiles(MAIN_CLASS_NAME, SELF_ID,
    // SWIRLD_NAME);
    //
    ...
    //                                assertEquals(originalState.getRound(), stateFromDisk.getRound(), "round should
    // match");
    //                                assertEquals(
