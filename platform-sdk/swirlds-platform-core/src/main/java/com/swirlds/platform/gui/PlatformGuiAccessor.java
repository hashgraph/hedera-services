 ...
 */
public class PlatformGuiAccessor {
    ...
    private static final PlatformGuiAccessor INSTANCE = new PlatformGuiAccessor();
     ...
     */
    public static PlatformGuiAccessor getInstance() {
        return INSTANCE;
    }

    private PlatformGuiAccessor() {}
    ...
    private record EventOrderInfo(long consensusOrder, @NonNull Instant timeReceived)
            implements Comparable<PlatformGuiAccessor.EventOrderInfo> {

        /**
         * Create a new EventOrderInfo from an event.
         */
        public static PlatformGuiAccessor.EventOrderInfo of(@NonNull final EventImpl event) {
            return new PlatformGuiAccessor.EventOrderInfo(event.getConsensusOrder(), event.getTimeReceived());
        }
         ...
         */
        @Override
        public int compareTo(@NonNull final PlatformGuiAccessor.EventOrderInfo that) {
    ...
    public PlatformEvent[] getAllEvents(@NonNull NodeId nodeId) {
        ...
        final EventImpl[] allEvents =
                PlatformGuiAccessor.getInstance().getShadowGraph(nodeId).getAllEvents();
        ...
        final Map<Hash, PlatformGuiAccessor.EventOrderInfo> orderInfo = new HashMap<>();
        for (final EventImpl event : allEvents) {
            orderInfo.put(event.getHashedData().getHash(), PlatformGuiAccessor.EventOrderInfo.of(event));
        }

        Arrays.sort(allEvents, (eventA, eventB) -> {
            final PlatformGuiAccessor.EventOrderInfo orderA =
                    orderInfo.get(eventA.getHashedData().getHash());
            final PlatformGuiAccessor.EventOrderInfo orderB =
                    orderInfo.get(eventB.getHashedData().getHash());
