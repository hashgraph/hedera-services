...
public class SignatureSchema {
    ...
    private SignatureSchema(BilinearPairing pairing) {
        ...
        this.publicKeyGroupSupplier =
                pairing::getGroup2; // receive this with an enum and make the decission based on thatsa
    ...
    public boolean verifySignature(byte[] message, PublicKey publicKey, Signature signature) {
        ...
        PairingResult lhs = pairing.pairingBetween(
                signature.element(), publicKeyGroupSupplier.get().getGenerator());
    ...
    public static Signature deserializeSignature(byte[] bytes) {
        ...
        return new Signature(
                SignatureSchema.forPairing(curveType).signatureGroup().elementFromBytes(bytes));
    }
    ...
    public static PrivateKey deserializePrivateKey(byte[] bytes) {
        ...
        return new PrivateKey(
                SignatureSchema.forPairing(curveType).pairing.getField().elementFromBytes(bytes));
    }
    ...
    public static PublicKey deserializePublicKey(byte[] bytes) {
        ...
        return new PublicKey(
                SignatureSchema.forPairing(curveType).publicKeyGroup().elementFromBytes(bytes));
    }
