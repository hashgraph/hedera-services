...
public record PrivateKey(FieldElement element) {
    ...
    static PrivateKey deserialize(final byte[] bytes) {
        return BlsSchema.deserializePrivateKey(bytes);
    }
    ...
    public Signature sign(@NonNull final byte[] message) {
        return BlsSchema.forPairing(element.curveType()).sign(message, this);
    }

    static PrivateKey create(CurveType type) {
        return BlsSchema.forPairing(type).createKeyPair().key();
    }

    public PublicKey createPublicKey() {
        return BlsSchema.forPairing(element.curveType()).createPublicKey(element);
    }
}
