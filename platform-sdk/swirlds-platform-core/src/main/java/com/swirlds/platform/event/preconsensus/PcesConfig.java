 ...
 *
 * @param writeQueueCapacity                      the queue capacity for preconsensus events waiting to be written to
 *                                                disk
 * @param minimumRetentionPeriod                  the minimum amount of time that preconsensus events should be stored
 *                                                on disk. At a minimum, should exceed the length of time between state
 *                                                saving.
 ...
 *                                                average for the span utilization will not have any values in it. Use
 *                                                this value until the running average represents real data. Once the
 *                                                running average for the span utilization becomes available, then this
 *                                                value is ignored.
 * @param spanUtilizationSpanRunningAverageLength the preconsensus event stream tracks the running average of the span
 ...
 *                                                span if the node crashes. Choosing a value too small may case event
 *                                                files to be smaller than the preferred size. Should be larger than the
 ...
 *                                                previous file span utilization by this factor. Choosing a value too
 *                                                large will cause unnecessary un-utilized span if the node crashes.
 *                                                Choosing a value too small may case event files to be smaller than the
 *                                                preferred size.
 * @param minimumSpan                             when creating a new file, make sure it at least has this much
 *                                                available span. This is puts a sane "floor" on the span heuristic, so
 *                                                that we never attempt to open a file that doesn't have capacity for
 *                                                events, regardless of the required span to do so. If properly
 *                                                configured and under steady state operation, this capacity is unlikely
 *                                                to be a limiting factor on the span of files.
 ...
 *                                                possible if a preconsensus event file has been deleted out of band.
 *                                                This setting is present only to allow emergency manual action. In
 *                                                general, allowing gaps is likely to either lead to ISSes or, more
 *                                                likely, cause events to be added to the hashgraph without their
 *                                                parents being added. Or Both. Use this with caution.
 ...
 *                                                guaranteed to contain all non-ancient events w.r.t. the state
 *                                                snapshot.
 * @param compactLastFileOnStartup                if true, then compact the last file's span on startup.
 * @param forceIgnorePcesSignatures               if true, then ignore the signatures on preconsensus events. Note: This
 *                                                is a TEST ONLY setting. It must never be enabled in production.
 */
...
public record PcesConfig(
        ...
        @ConfigProperty(defaultValue = "5") int spanUtilizationSpanRunningAverageLength,
