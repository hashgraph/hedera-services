...
public class TurboSyncRunner {
    ...
    private boolean areNodesOutOfSync() {
        ...
        switch (status) {
            ...
            case SELF_FALLEN_BEHIND -> {
                ...
                logger.info(
                        STARTUP.getMarker(),
                        "self fallen behind, peer = " + peerId + ", cycle = " + cycleNumber); // TODO
                return true;
            }
            case OTHER_FALLEN_BEHIND -> {
                ...
                logger.info(
                        STARTUP.getMarker(),
                        "peer fallen behind, peer = " + peerId + ", cycle = " + cycleNumber); // TODO
                return true;
            }
    ...
    private void checkIfProtocolShouldBeAborted() {
        if (fallenBehindManager.hasFallenBehind()
            ...
            logger.info(STARTUP.getMarker(), "aborting with peer " + peerId + ", cycle = " + cycleNumber);
    ...
    private void receiveData() throws IOException, InterruptedException {
        ...
        if (dataInputStream.readBoolean()) {
            ...
            logger.info(
                    STARTUP.getMarker(), "peer requested abort, peer = " + peerId + ", cycle = " + cycleNumber); // TODO
            return;
        }
    ...
    private List<Hash> sendTips() throws IOException {
        ...
        final List<ShadowEvent> shadowgraphTips = shadowgraph.getTips();
        final List<Hash> myTips =
                shadowgraphTips.stream().map(e -> e.getEvent().getBaseHash()).toList();
