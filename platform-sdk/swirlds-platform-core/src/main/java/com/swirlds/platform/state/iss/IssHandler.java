...
public class IssHandler {
     ...
     * 		builds dispatchers
     * @param stateConfig
     * 		settings for the state
     * @param selfId
     * 		the self ID of this node
     * @param haltRequestedConsumer
     * 		consumer to invoke when a system halt is desired
     * @param fatalErrorConsumer
     * 		consumer to invoke if a fatal error occurs
     * @param issConsumer
     * 		consumer to invoke if an ISS is detected
     */
    public IssHandler(
            final Time time,
     ...
     * @param nodeId
     * 		the ID of the node that had an ISS
     * @param nodeHash
     * 		the incorrect hash computed by the node
     * @param consensusHash
     * 		the correct hash computed by the network
     */
     ...
     * 		the round of the ISS
     * @param selfStateHash
     * 		the incorrect hash computed by this node
     * @param consensusHash
     * 		the correct hash computed by the network
     */
    @Observer(SelfIssTrigger.class)
    public void selfIssObserver(final Long round, final Hash selfStateHash, final Hash consensusHash) {
    ...
    public void catastrophicIssObserver(final Long round, final Hash selfStateHash) {
        ...
        if (stateConfig.haltOnAnyIss() || stateConfig.haltOnCatastrophicIss()) {
            ...
            stateDumpRequestedDispatcher.dispatch(ISS_DUMP_CATEGORY, stateConfig.automatedSelfIssRecovery());
        }
    }
}
