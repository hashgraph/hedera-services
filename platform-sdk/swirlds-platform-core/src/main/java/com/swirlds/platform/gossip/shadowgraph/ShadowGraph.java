    ...
    private InsertableStatus insertable(final EventImpl e) {
        ...
        if (hasOP) {
            ...
            if (!knownOP && !expiredOP) {
                logger.warn(EXCEPTION.getMarker(), "Missing non-expired other parent for {}", e::toMediumString);
            }
        }
        ...
        if (hasSP) {
            ...
            if (!knownSP && !expiredSP) {
                logger.warn(EXCEPTION.getMarker(), "Missing non-expired self parent for {}", e::toMediumString);
            }
        }
