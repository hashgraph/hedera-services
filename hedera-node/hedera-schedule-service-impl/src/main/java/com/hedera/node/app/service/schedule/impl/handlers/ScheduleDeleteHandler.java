...
//        // check for SCHEDULE_ALREADY_DELETED, SCHEDULE_ALREADY_EXECUTED errors
        ...
        // check whether payer key for this tx matches the admin key for underlying scheduled transaction
        final KeyOrLookupFailureReason payerKeyOrLookupFailure = keyLookup.getKey(payer);
        if (payerKeyOrLookupFailure.failed()) {
            return new InvalidTransactionMetadata(txn, payer, payerKeyOrLookupFailure.failureReason());
        } else {
            var payerKey = payerKeyOrLookupFailure.key();
            if (!adminKey.get().equals(payerKey))
                return new InvalidTransactionMetadata(txn, payer, INVALID_SIGNATURE);   // I'm not sure whether this is the correct code to return here
        }
