...
class FreezeHandlerTest {
    ...
    void rejectIfUnknownFreezeType() {
        TransactionBody txn = TransactionBody.newBuilder()
                ...
                .freeze(FreezeTransactionBody.newBuilder().freezeType(UNKNOWN_FREEZE_TYPE).build())
                .build();
        final var context = new PreHandleContext(keyLookup, txn);
        PreCheckException e = assertThrows(
                PreCheckException.class,
                () -> subject.preHandle(context, specialFileStore));

        assertTrue(e.responseCode() == INVALID_FREEZE_TRANSACTION_BODY);
    }
    ...
    void rejectIfStartTimeNotSetForCertainFreezeTypes() {
        ...
        for (FreezeType freezeType : freezeTypes) {
            TransactionBody txn = TransactionBody.newBuilder()
                    .transactionID(TransactionID.newBuilder().accountID(nonAdminAccount))
                    .freeze(FreezeTransactionBody.newBuilder().freezeType(freezeType).build())
                    .build();
            final var context = new PreHandleContext(keyLookup, txn);
    ...
    void rejectIfStartTimeInPastForCertainFreezeTypes() {
        ...
        for (FreezeType freezeType : freezeTypes) {
            ...
            assertTrue(e.responseCode() == FREEZE_START_TIME_MUST_BE_FUTURE);
        }
    }
    ...
    void rejectIfUpdateFileNotSetForCertainFreezeTypes() {
        ...
        for (FreezeType freezeType : freezeTypes) {
            ...
            assertTrue(e.responseCode() == FREEZE_UPDATE_FILE_DOES_NOT_EXIST);
        }
    }
    ...
    void rejectIfFileHashNotSetForCertainFreezeTypes() {
        ...
        for (FreezeType freezeType : freezeTypes) {
            ...
            assertTrue(e.responseCode() == FREEZE_UPDATE_FILE_HASH_DOES_NOT_MATCH);
        }
    }
    ...
    void happyPathFreezeAbort() throws PreCheckException {
        ...
        TransactionBody txn = TransactionBody.newBuilder()
                .transactionID(TransactionID.newBuilder().accountID(nonAdminAccount))
                .freeze(FreezeTransactionBody.newBuilder().freezeType(FREEZE_ABORT).build())
                .build();
        final var context = new PreHandleContext(keyLookup, txn);
