...
abstract class EvmTxProcessor {
  ...
  protected TransactionProcessingResult execute(
    ...
    if (isStatic) {
      ...
      try {
        ...
        final var feesOnlyUpdater = (HederaEvmWorldState) worldState.updater();
        chargeForGas(
            gasCost,
            upfrontCost,
            value,
  ...
  private ChargingResult chargeForGas(
      ...
      final HederaEvmWorldState updater) {
  ...
  private void validateTrue(final boolean flag, final ResponseCodeEnum code) {
    if (!flag) {
