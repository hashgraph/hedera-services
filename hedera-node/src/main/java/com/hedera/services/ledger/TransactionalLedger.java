 ...
 * per-account changesets, which are then either saved to a backing store when the transaction is
    ...
    changes
        ...
        .forEach(
            change -> {
              ...
              var accountNotInDeadAccounts =
                  deadEntities.contains(id) ? "*NEW -> DEAD* " : "*NEW* ";
              var prefix = entities.contains(id) ? accountInDeadAccounts : accountNotInDeadAccounts;
              desc.append(prefix)
              ...
              desc.append(
                  change.getValue().entrySet().stream()
                      .map(
                          entry ->
                              String.format(
                                  "%s -> %s", entry.getKey(), readableProperty(entry.getValue())))
                      .collect(joining(", ")));
              desc.append("]");
