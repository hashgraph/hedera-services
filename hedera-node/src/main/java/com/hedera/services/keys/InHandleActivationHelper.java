...
/**
 * Provides information about the Ed25519 keys that compose the Hedera keys linked to the active
 * transaction (and the schedule it references, if applicable).
 *
 * <p>In particular, lets a visitor traverse these Ed25519 keys along with their expanded {@code
...
public class InHandleActivationHelper {
  ...
  public InHandleActivationHelper(
      CharacteristicsFactory characteristics, Supplier<TxnAccessor> accessorSource) {
    this.characteristics = characteristics;
    this.accessorSource = accessorSource;
  }

  /**
   * Returns true if the set of Ed25519 signing keys for the active transaction suffice to meet the
   * signing requirements of all Hedera keys prerequisite to the active transaction.
   *
  ...
  /**
   * Returns true if the set of Ed25519 signing keys for the active transaction suffice to meet the
   * signing requirements of all Hedera keys prerequisite to the schedule referenced by the active
   * transaction.
   *
   * @param scheduledTxn the scheduled transaction
   * @param tests the predicate(s) to use for testing if an Ed25519 key has signed
   * @return whether or not the given set of keys are sufficient for signing the schedule referenced
  ...
  /**
   * Permits a visitor to traverse the Ed25519 keys, and their expanded signatures, that constitute
   * the Hedera keys prerequisite to the schedule referenced by the active transaction.
   *
  ...
  /**
   * Returns the canonical mapping between Ed25519 public keys and expanded signatures for the
   * active transaction.
   *
   * @return the canonical mapping between Ed25519 public keys and expanded signatures for the
   *     active transaction.
   */
