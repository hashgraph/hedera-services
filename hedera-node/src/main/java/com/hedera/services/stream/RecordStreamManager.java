...
public class RecordStreamManager {
  ...
  /**
   * receives {@link RecordStreamObject}s from {@link
   * com.hedera.services.legacy.services.state.AwareProcessLogic} * .addForStreaming, then passes to
  ...
  /**
   * receives {@link RecordStreamObject}s from hashQueueThread, calculates this object's Hash, then
   * passes to runningHashQueueThread
   */
   ...
   * RecordStreamObject}s to record stream files
   */
   ...
   *     record stream enabled?, (2) how many seconds should elapse before creating the next record
   *     file, and (3) how large a capacity the record stream blocking queue should have.
   * @param nodeScopedRecordLogDir the direct file folder for writing record stream files
  ...
  public RecordStreamManager(
    ...
    // receives {@link RecordStreamObject}s from hashCalculator, calculates and set runningHash for
    // this object
    ...
    log.info(
        "Finish initializing RecordStreamManager with: enableRecordStreaming: {}, recordStreamDir: {}, "
  ...
  /**
   * receives a consensus record from {@link
   * com.hedera.services.legacy.services.state.AwareProcessLogic} each time, sends it to multiStream
  ...
  /**
   * sets startWriteAtCompleteWindow: it should be set to be true after reconnect; it should be set
   * to be false at restart
   *
   * @param startWriteAtCompleteWindow whether the writer should not write until the first complete
   *     window
   */
  public void setStartWriteAtCompleteWindow(boolean startWriteAtCompleteWindow) {
    if (streamFileWriter != null) {
      ...
      log.info(
          "RecordStreamManager::setStartWriteAtCompleteWindow: {}", startWriteAtCompleteWindow);
    }
  }
