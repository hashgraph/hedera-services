...
public class DomainUtils {
  ...
  public static ContractFunctionResult asHapiResult(
      TransactionReceipt receipt, Optional<List<ContractID>> created) {
    ...
    if (!isFailed(receipt)) {
      if (isCreation(receipt)) {
        result.setContractID(
            contractParsedFromSolidityAddress(receipt.getTransaction().getContractAddress()));
      } else {
        Optional.ofNullable(receipt.getExecutionResult())
            .map(ByteString::copyFrom)
            .ifPresent(result::setContractCallResult);
      }
      Optional.ofNullable(receipt.getLogInfoList())
          .ifPresent(logs -> logs.stream().map(DomainUtils::asHapiLog).forEach(result::addLogInfo));
    }
  ...
  public static ContractLoginfo asHapiLog(LogInfo logInfo) {
    ...
    Optional.ofNullable(logInfo.getData()).map(ByteString::copyFrom).ifPresent(log::setData);
    Optional.ofNullable(logInfo.getTopics()).stream()
        .flatMap(List::stream)
