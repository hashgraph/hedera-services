...
abstract class EvmTxProcessor {
  ...
  protected EvmTxProcessor(
    ...
    this.messageCallProcessor =
        new HederaMessageCallProcessor(evm, precompileContractRegistry, precompiledContractMap);
   ...
   * @param receiver the priority form of the receiving {@link Address} (i.e., EIP-1014 if present);
   *     or the newly created address
    ...
    if (!isStatic) {
      ...
      if (relayer == null) {
        ...
        if (userOfferedGasPrice.equals(BigInteger.ZERO)) {
          ...
          var senderFee =
              Wei.of(
                  userOfferedGasPrice
                      ...
                      .divide(WEIBARS_TO_TINYBARS));
          validateTrue(
              mutableSender.getBalance().compareTo(senderFee) >= 0, INSUFFICIENT_PAYER_BALANCE);
          ...
          validateTrue(
              mutableRelayer.getBalance().compareTo(remainingFee) >= 0, INSUFFICIENT_PAYER_BALANCE);
