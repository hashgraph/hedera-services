...
public class SolidityLifecycle {
  ...
  public Map.Entry<ContractFunctionResult, ResponseCodeEnum> run(
      SolidityExecutor executor, ServicesRepositoryRoot root) {
    cycle(executor);
  ...
  public Map.Entry<ContractFunctionResult, ResponseCodeEnum> runPure(
      long maxResultSize, SolidityExecutor executor) {
    cycle(executor);
        ...
        if (resultSize > maxResultSize) {
          ...
          result =
              result.toBuilder()
                  ...
                  .setErrorMessage(
                      String.format(OVERSIZE_RESULT_ERROR_MSG_TPL, resultSize, maxResultSize))
                  .build();
        }
      }
    }
