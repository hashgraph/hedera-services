...
/**
 * Gathers the contrtact action meta-data
 */
public class HederaTracer implements OperationTracer {
	...
	static EntityId toEntiryId(Address address) {
		return EntityIdUtils.entityIdFromSolidityAddress(address.toArrayUnsafe());
	}
	...
	public void traceExecution(MessageFrame frame, ExecuteOperation executeOperation) {
		...
		if (currentHeight == 0) {
			var action = new SolidityAction(
					...
					toEntiryId(frame.getOriginatorAddress()),
					null,
					frame.getRemainingGas().toLong(),
					frame.getInputData().toArray(),
					null,
					toEntiryId(frame.getContractAddress()),
					null,
		...
		if (frameState == MessageFrame.State.CODE_SUSPENDED) {
			...
			if (nextFrame != frame) {
				...
				var action = new SolidityAction(
						...
						null,
						toEntiryId(nextFrame.getSenderAddress()),
						nextFrame.getRemainingGas().toLong(),
						nextFrame.getInputData().toArray(),
						null,
						toEntiryId(nextFrame.getContractAddress()),
						null,
			...
			lastAction.setGasUsed(lastAction.getGas() - frame.getRemainingGas().toLong());
