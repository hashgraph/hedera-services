...
/**
 * Loads token-related entities from the Swirlds state, hiding the details
 * of Merkle types from client code by providing an interface in
 * terms of model objects whose methods can perform validated business logic.
 * It does not permit mutations of Merkle types.
 * <p>
...
public class ReadOnlyTokenStore {
	...
	private void initModelFields(UniqueToken uniqueToken, UniqueTokenValue immutableUniqueTokenValue) {
		uniqueToken.setCreationTime(immutableUniqueTokenValue.getCreationTime());
		uniqueToken.setMetadata(immutableUniqueTokenValue.getMetadata());
		uniqueToken.setOwner(immutableUniqueTokenValue.getOwner().asId());
		uniqueToken.setSpender(immutableUniqueTokenValue.getSpender().asId());
	}
