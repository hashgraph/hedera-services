...
/**
 * Defines the lifecycle for a <b>single execution</b> of a precompiled contract executed via
 * {@link HTSPrecompiledContract}. Implementations may change the world state; unless reverted,
 * their execution will be externalized in the record stream via a "synthetic"
 * {@link TransactionBody} and "child" {@link TransactionRecord}.
 *
 * <p>
 * Just as with {@link HTSPrecompiledContract} itself, implementations will generally be
 * <b>stateful</b> and <b>not thread-safe</b>; their correctness will depend on the integrity of
 * the internal state they keep in-between calls to the various lifecycle methods.
 *
 * <p>
 * The precompile lifecycle methods are used by the {@link HTSPrecompiledContract}
 * executor as below.
 * <ol>
 *     <li>First, the executor calls {@link Precompile#body(Bytes, UnaryOperator)} to get a
 *     synthetic transaction body that captures the input to the precompile.</li>
 *     <li>Second, the executor computes the effective gas for the precompile by consulting
 ...
 *     <li>Third, the executor invokes {@link Precompile#run(PrecompileInfoProvider)}, which is expected
 ...
 *     {@link Precompile#getSuccessResultFor(ExpirableTxnRecord.Builder)} to get the {@link Bytes}
 *     to return to the EVM; otherwise, {@link Precompile#getFailureResultFor(ResponseCodeEnum)}
 *     is called for the same purpose. The executor customizes the final record using some
 *     additional bits of information from {@link Precompile#getCustomFees()} and
