...
public class HederaScheduleStore extends HederaStore implements ScheduleStore {
  ...
  public boolean exists(ScheduleID id) {
    return (isCreationPending() && pendingId.equals(id))
        || schedules.get().containsKey(fromScheduleId(id));
  }
  ...
  public CreationResult<ScheduleID> createProvisionally(
      MerkleSchedule schedule, RichInstant consensusTime) {
    ...
    if (validity == OK) {
      validity =
          usableOrElse(schedule.schedulingAccount().toGrpcAccountId(), INVALID_SCHEDULE_ACCOUNT_ID);
    }
    if (validity != OK) {
  ...
  private void buildContentAddressableViewOfExtantSchedules() {
    ...
    forEach(
        curSchedules, (key, value) -> extantSchedules.put(value.toContentAddressableView(), key));
  }
  ...
  private ResponseCodeEnum usabilityCheck(ScheduleID id, boolean requiresMutability) {
  ...
  private void throwIfMissing(ScheduleID id) {
    if (!exists(id)) {
      throw new IllegalArgumentException(
          String.format(
              "Argument 'id=%s' does not refer to an extant scheduled entity!", readableId(id)));
    }
  }
}
