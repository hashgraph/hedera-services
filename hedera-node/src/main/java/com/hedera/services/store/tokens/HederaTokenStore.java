...
public class HederaTokenStore extends HederaStore implements TokenStore {
  ...
  public ResponseCodeEnum associate(AccountID aId, List<TokenID> tokens) {
    return fullySanityChecked(
        ...
        (account, tokenIds) -> {
          ...
          var validity = OK;
          if ((accountTokens.numAssociations() + tokenIds.size())
              > properties.maxTokensPerAccount()) {
  ...
  public boolean exists(TokenID id) {
    return (isCreationPending() && pendingId.equals(id))
        || tokens.get().containsKey(fromTokenId(id));
  }
          ...
          if (owner.equals(fromGrpcAccountId(AccountID.getDefaultInstance()))) {
            ...
            owner =
                this.tokens
                    .get()
                    .get(
                        new MerkleEntityId(tid.getShardNum(), tid.getRealmNum(), tid.getTokenNum()))
                    .treasury();
          }
  ...
  private void updateLedgers(
    ...
    final var merkleNftId =
        new MerkleUniqueTokenId(fromGrpcTokenId(nftId.tokenId()), nftId.serialNo());
  ...
  private ResponseCodeEnum validateFeeSchedule(
    ...
    for (var customFee : feeSchedule) {
      ...
      if (customFee.hasFixedFee()) {
        ...
        responseCode =
            validateFractional(customFee, isUpdate, targetToken, targetTokenId, feeCollector);
        if (responseCode != OK) {
          return responseCode;
        }
      } else if (customFee.hasRoyaltyFee()) {
        responseCode =
            validateRoyalty(customFee, isUpdate, targetToken, targetTokenId, feeCollector);
        if (responseCode != OK) {
          return responseCode;
        }
      } else {
  ...
  private ResponseCodeEnum validateFixed(
      FixedFee fixedFee, AccountID feeCollector, TokenID targetTokenId) {
  ...
  private void throwIfKnownTreasuryIsMissing(AccountID aId) {
    if (!knownTreasuries.containsKey(aId)) {
      throw new IllegalArgumentException(
          String.format("Argument 'aId=%s' does not refer to a known treasury!", readableId(aId)));
    }
  }
  ...
  public ResponseCodeEnum update(TokenUpdateTransactionBody changes, long now) {
    ...
    apply(
        ...
        token -> {
          ...
          checkKeyOfType(
              appliedValidity, token.hasKycKey(), newKycKey.isPresent(), TOKEN_HAS_NO_KYC_KEY);
          checkKeyOfType(
              appliedValidity,
              token.hasFreezeKey(),
              ...
              TOKEN_HAS_NO_FREEZE_KEY);
          checkKeyOfType(
              appliedValidity, token.hasWipeKey(), newWipeKey.isPresent(), TOKEN_HAS_NO_WIPE_KEY);
          checkKeyOfType(
              appliedValidity,
              token.hasSupplyKey(),
  ...
  private void processExpiry(
      AtomicReference<ResponseCodeEnum> appliedValidity,
  ...
  private void updateTreasuryIfAppropriate(
      final MerkleToken token, final TokenUpdateTransactionBody changes, final TokenID tId) {
    if (changes.hasTreasury()
  ...
  private void updateKeyOfTypeIfAppropriate(
      boolean check, Consumer<JKey> consumer, Supplier<Key> supplier) {
    if (check) {
  ...
  private ResponseCodeEnum sanityCheckedFungibleCommon(
      AccountID aId, TokenID tId, Function<MerkleToken, ResponseCodeEnum> action) {
