...
public class TypedTokenStore {
   ...
   * the {@code uniqueTokens} map---NFTs are not marked deleted but actually removed from state when
   * they are burned.)
   *
   ...
   * TypedTokenStore#loadToken(Id)} and {@link AccountStore#loadAccount(Id)} methods, respectively,
   * since it will very rarely (or never) be correct to do business logic on a relationship whose
   * token or account have not been validated as usable.
   *
   * <p><b>IMPORTANT:</b> Changes to the returned model are not automatically persisted to state!
   * The altered model must be passed to {@link TypedTokenStore#persistTokenRelationships(List)} in
  ...
  private void persistNonDestroyed(
    ...
    final var mutableTokenRel =
        isNewRel ? new MerkleTokenRelStatus() : currentTokenRels.getForModify(key);
   ...
   *
   * <p><b>IMPORTANT:</b> Changes to the returned model are not automatically persisted to state!
   * The altered model must be passed to {@link TypedTokenStore#persistToken(Token)} in order for
   ...
   * @throws InvalidTransactionException if the requested token is missing, deleted, or expired and
   *     pending removal
   */
  ...
  /**
   * Returns a {@link UniqueToken} model of the requested unique token, with operations that can be
   * used to implement business logic in a transaction.
   *
  ...
  private void destroyRemoved(List<UniqueToken> nfts, EntityId treasury) {
    ...
    for (var nft : nfts) {
      final var merkleNftId =
          new MerkleUniqueTokenId(new EntityId(nft.getTokenId()), nft.getSerialNumber());
  ...
  private void initModelAccounts(
      Token token, EntityId _treasuryId, @Nullable EntityId _autoRenewId) {
    if (_autoRenewId != null) {
      final var autoRenewId =
          new Id(_autoRenewId.shard(), _autoRenewId.realm(), _autoRenewId.num());
