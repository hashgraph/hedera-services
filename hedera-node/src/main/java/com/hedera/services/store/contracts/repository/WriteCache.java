...
public class WriteCache<Key, Value> extends AbstractCachedSource<Key, Value> {
    ...
    private static final class SimpleCacheEntry<V> extends WriteCache.CacheEntry<V> {
    ...
    private static final class CountCacheEntry<V> extends WriteCache.CacheEntry<V> {
    ...
    protected volatile Map<Key, WriteCache.CacheEntry<Value>> cache = new HashMap<>();
    ...
    public WriteCache(Source<Key, Value> src, WriteCache.CacheType cacheType) {
        super(src);
        this.isCounting = cacheType == WriteCache.CacheType.COUNTING;
    }

    public WriteCache<Key, Value> withCache(Map<Key, WriteCache.CacheEntry<Value>> cache) {
        this.cache = cache;
        return this;
    }
    ...
    private WriteCache.CacheEntry<Value> createCacheEntry(Value val) {
        if (isCounting) {
            return new WriteCache.CountCacheEntry<>(val);
        } else {
            return new WriteCache.SimpleCacheEntry<>(val);
        }
    }
    ...
    public void put(Key key, Value val) {
        ...
        try (ALock l = writeLock.lock()){
            WriteCache.CacheEntry<Value> curVal = cache.get(key);
            if (curVal == null) {
                curVal = createCacheEntry(val);
                WriteCache.CacheEntry<Value> oldVal = cache.put(key, curVal);
                if (oldVal != null) {
    ...
    public boolean flush() {
        ...
        try (ALock l = updateLock.lock()){
            if (modified) {
                for (Map.Entry<Key, WriteCache.CacheEntry<Value>> entry : cache.entrySet()) {
    ...
    private Value unknownValue() {
        return (Value) WriteCache.CacheEntry.UNKNOWN_VALUE;
    }

    public Entry<Value> getCached(Key key) {
        try (ALock l = readLock.lock()){
            WriteCache.CacheEntry<Value> entry = cache.get(key);
    ...
    public Map<Key, WriteCache.CacheEntry<Value>> getCache() {
        return this.cache;
    }
