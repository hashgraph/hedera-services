 ...
 * fully supported. For example, a {@link Token#mint(TokenRelationship, long)} signature only makes
 * sense for a token of type {@code FUNGIBLE_COMMON}; the signature for a {@code
 * NON_FUNGIBLE_UNIQUE} will likely be {@code mint(TokenRelationship, byte[])}.
 */
public class Token {
  private final Id id;
  ...
  public void mint(
    ...
    validateTrue(
        (lastUsedSerialNumber + metadataCount) <= MAX_NUM_ALLOWED, SERIAL_NUMBER_LIMIT_REACHED);
   ...
   */
  public void wipe(
      OwnershipTracker ownershipTracker, TokenRelationship accountRel, List<Long> serialNumbers) {
    validateTrue(type == TokenType.NON_FUNGIBLE_UNIQUE, FAIL_INVALID);
  ...
  private void changeSupply(
      TokenRelationship treasuryRel, long amount, ResponseCodeEnum negSupplyCode) {
  ...
  private void amountWipeValidations(final TokenRelationship accountRel, final long amount) {
    ...
    validateTrue(
        newAccountBalance >= 0, INVALID_WIPING_AMOUNT, "Wiping would negate account balance");
  }
