 ...
 *
 * <p>Note that writing {@link UInt256#ZERO} to a key removes it from the map; so it is possible
 * for a change to decrease the number of key/value pairs used.
 */
@Singleton
public class SizeLimitedStorage {
  ...
  /**
   * Records the new mapping counts and/or first storage keys of any contracts whose storage changed
   * in this session.
   *
  ...
  public void recordNewKvUsageTo(
    ...
    usageChanges.forEach(
        (contractNum, kvUsageInfo) -> {
          ...
          accountsLedger.set(
              id, FIRST_CONTRACT_STORAGE_KEY, newFirstKey == null ? null : newFirstKey.getKey());
        });
  }
  ...
  public void putStorage(final AccountID id, final UInt256 key, final UInt256 value) {
    ...
    if (kvCountImpact != 0) {
      usageChanges.computeIfAbsent(id.getAccountNum(), usageInfoLookup)
  ...
  private void validatePendingSizeChanges() {
    ...
    usageChanges.forEach(
        (id, kvUsageInfo) -> usageLimits.assertUsableContractSlots(kvUsageInfo.pendingUsage()));
  }
