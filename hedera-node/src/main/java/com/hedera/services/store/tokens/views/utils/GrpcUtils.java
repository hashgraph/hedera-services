...
/**
 * Small helper class to provide a query-friendly representation of a Merkle unique token whose
 * owner id may be the sentinel {@code 0.0.0} representing treasury ownership.
 */
public class GrpcUtils {
  ...
  /**
   * Build a query-friendly representation of the given unique token, using the given treasury
   * account id to substitute for the sentinel {@code 0.0.0} id if necessary.
   *
  ...
  public static TokenNftInfo reprOf(
    ...
    final var nftId = NftID.newBuilder().setTokenID(type).setSerialNumber(serialNo);

    AccountID effectiveOwner;
    if (nft.isTreasuryOwned()) {
      if (treasury == null) {
        throw new IllegalArgumentException(
            EntityIdUtils.readableId(type)
                + "."
                + serialNo
