...
public final class TokenConversion {
	 ...
	 * @param id
	 * 			Id of the token.
	 * @param treasuryAccount
	 * 			treasury Account {@link Account} of the merkleToken
	 * @param autoRenewAccount
	 * 			autoRenew Account {@link Account} of the merkleToken.
	 * 			Can be null if the merkleToken has no AutoRenewAccount
	 * @return
	 * 			Token Model based on the passed MerkleToken.
	 */
	public static Token fromMerkle(final MerkleToken merkleToken, final Id id,
			final Account treasuryAccount, @Nullable final Account autoRenewAccount) {
		final var token = new Token(id);
		initModelAccounts(token, treasuryAccount, autoRenewAccount);
