...
public class AccountStore {
	...
	private Account loadEntityOrFailWith(
		...
		final var accountId = EntityNum.fromModel(id);
		final var merkleAccount = accounts.getImmutableRef(accountId);
	...
	public void commitAccount(Account account) {
		final var id = EntityNum.fromModel(account.getId());

		final var mutableAccount = accounts.getRef(id);
		mapModelToMutable(account, mutableAccount);
		mutableAccount.tokens().updateAssociationsFrom(account.getAssociatedTokens());
		accounts.put(id, mutableAccount);
	}
