 ...
 *
 * This class provides the indices to use when selecting from the sub-lists.
 * <pre>
 *     {@code
...
public class MultiSourceRange {
	...
	/**
	 * Indicates if the original requested range has been exhausted, based on the sub-list
	 * indices returned from {@link MultiSourceRange#rangeForCurrentSource()} so far.
	 *
	...
	/**
	 * Returns the indices to use with the current source to get the next
	 * items for the original requested range. Note that these indices may
	 * be {@code (0, 0)} and indicate that the current source has no items
	 * that belong in the requested range.
	 *
