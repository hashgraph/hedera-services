...
public class SimplifiedWriteCache<Key, Value> extends AbstractCachedSource<Key, Value> {
    ...
    private static final class SimpleCacheEntry<V> extends SimplifiedWriteCache.CacheEntry<V> {
    ...
    protected volatile Map<Key, SimplifiedWriteCache.CacheEntry<Value>> cache = new HashMap<>();
    ...
    public SimplifiedWriteCache<Key, Value> withCache(Map<Key, SimplifiedWriteCache.CacheEntry<Value>> cache) {
        this.cache = cache;
        return this;
    }
    ...
    private SimplifiedWriteCache.CacheEntry<Value> createCacheEntry(Value val) {
        return new SimplifiedWriteCache.SimpleCacheEntry<>(val);
    }
    ...
    public void put(Key key, Value val) {
        ...
        try (ALock l = writeLock.lock()){
            SimplifiedWriteCache.CacheEntry<Value> curVal = cache.get(key);
            if (curVal == null) {
                curVal = createCacheEntry(val);
                SimplifiedWriteCache.CacheEntry<Value> oldVal = cache.put(key, curVal);
                if (oldVal != null) {
    ...
    public boolean flush() {
        ...
        try (ALock l = updateLock.lock()){
            for (Map.Entry<Key, SimplifiedWriteCache.CacheEntry<Value>> entry : cache.entrySet()) {
    ...
    private Value unknownValue() {
        return (Value) SimplifiedWriteCache.CacheEntry.UNKNOWN_VALUE;
    }

    public Entry<Value> getCached(Key key) {
        try (ALock l = readLock.lock()){
            SimplifiedWriteCache.CacheEntry<Value> entry = cache.get(key);
    ...
    public Map<Key, SimplifiedWriteCache.CacheEntry<Value>> getCache(){
        return this.cache;
    }
