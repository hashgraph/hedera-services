...
public class NetworkCtxManager {
  ...
  private BiPredicate<Instant, Instant> shouldUpdateMidnightRates =
      (now, then) -> !inSameUtcDay(now, then);
  ...
  public void advanceConsensusClockTo(Instant consensusTime) {
    ...
    if (lastMidnightBoundaryCheck != null) {
      ...
      final long elapsedInterval =
          consensusTime.getEpochSecond() - lastMidnightBoundaryCheck.getEpochSecond();
