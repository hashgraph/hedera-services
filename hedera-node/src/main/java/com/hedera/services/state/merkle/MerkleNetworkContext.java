...
public class MerkleNetworkContext extends AbstractMerkleLeaf {
    ...
    if (activeThrottles.size() != usageSnapshots.length) {
      log.warn(
          "There are "
              ...
              + " active throttles, but "
              + usageSnapshots.length
              + " usage snapshots from saved state. "
  ...
  private void whenVersionHigherOrEqualTo0150(final SerializableDataInputStream in)
      throws IOException {
  ...
  public String toString() {
    var sb =
        new StringBuilder("The network context (state version ")
  ...
  private void reset(List<DeterministicThrottle> throttles) {
    var currUsageSnapshots =
        throttles.stream().map(DeterministicThrottle::usageSnapshot).collect(toList());
