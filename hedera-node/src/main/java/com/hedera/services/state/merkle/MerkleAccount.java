...
public class MerkleAccount extends AbstractNaryMerkleInternal implements MerkleInternal {
  ...
  public MerkleAccount() {
    addDeserializedChildren(
        List.of(
            new MerkleAccountState(), new FCQueue<ExpirableTxnRecord>(), new MerkleAccountTokens()),
        MERKLE_VERSION);
  }
  ...
  public MerkleAccount copy() {
    ...
    setImmutable(true);
    return new MerkleAccount(List.of(state().copy(), records().copy(), tokens().copy()), this);
  }
  ...
  public void setReceiverSigRequired(boolean receiverSigRequired) {
    throwIfImmutable(
        "Cannot change this account's receiver signature required setting if it's immutable.");
