...
public class HfsSystemFilesManager implements SystemFilesManager {
  ...
  private <T> T loadFrom(FileID disFid, String resource, GrpcParser<T> parser) {
    ...
    try {
      ...
      log.error(
          "Corrupt {} in saved state ({}), unable to continue!", resource, escapeBytes(contents));
      throw new IllegalStateException(e);
    }
  }

  private void bootstrapInto(FileID disFid, String resource, BootstrapLoader loader) {
    byte[] rawProps;
    try {
      rawProps = loader.get();
    } catch (Exception e) {
  ...
  private byte[] asSerializedConfig(
      String sysFileName, String externalPropsLoc, String defaultResource, String errorLog) {
    ...
    return configBytesFrom(() -> Files.newInputStream(Paths.get(externalPropsLoc)), externalSrcMsg)
        .or(
            () ->
                configBytesFrom(
                    () -> {
  ...
  static void mapOrderedJutilProps(
      Properties jutilProps, StringBuilder intoSb, ServicesConfigurationList.Builder config) {
