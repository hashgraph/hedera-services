...
/**
 * Static helper to move the two (expected) largest {@link com.swirlds.fcmap.FCMap} children to the
 * "binary route" positions in the Merkle tree, hence reducing the size of their Merkle routes.
 */
public class Release0170Migration {
  ...
  public static void moveLargeFcmsToBinaryRoutePositions(
      ServicesState initializingState, int deserializedVersion) {
    log.info(
        "Migrating from 0.16.0 state (version {} to {})",
        deserializedVersion,
        StateVersions.RELEASE_0170_VERSION);
    ...
    log.info(
        "  ↔ Swapped address book ({} to {}) and unique token ({} to {}) child FCMs",
        LegacyStateChildIndices.ADDRESS_BOOK,
        StateChildIndices.ADDRESS_BOOK,
        LegacyStateChildIndices.UNIQUE_TOKENS,
        StateChildIndices.UNIQUE_TOKENS);

    /* Second swap the network context and token associations */
    final var movableContext =
        initializingState.getChild(LegacyStateChildIndices.NETWORK_CTX).copy();
    treeCopier.copyToLocation(
        initializingState,
    ...
    log.info(
        "  ↔ Swapped network context ({} to {}) and token association ({} to {}) child FCMs",
        LegacyStateChildIndices.NETWORK_CTX,
        StateChildIndices.NETWORK_CTX,
        LegacyStateChildIndices.TOKEN_ASSOCIATIONS,
        StateChildIndices.TOKEN_ASSOCIATIONS);
  }
