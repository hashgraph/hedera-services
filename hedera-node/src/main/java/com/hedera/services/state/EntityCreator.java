...
public interface EntityCreator {
   ...
   *
   * @param recordCache record cache
   */
  void setRecordCache(RecordCache recordCache);

  /**
   * Sets needed properties like expiry and submitting member to {@link ExpirableTxnRecord} and adds
   * record to state based on {@code GlobalDynamicProperties.cacheRecordsTtl}. If not it is added to
   * be tracked for Expiry to {@link RecordCache}
   *
   * @param id account id
  ...
  /**
   * Build {@link ExpirableTxnRecord.Builder} when the record is finalized before committing the
   * active transaction
   *
   * @param otherNonThresholdFees part of fees
   * @param hash transaction hash
   * @param accessor transaction accessor
   * @param consensusTime consensus time
   * @param receipt transaction receipt
   * @param explicitTokenTransfers explicit list of token transfers
   * @param ctx services context
   * @param assessedCustomFees the list of assessed custom fees
   ...
   */
  ExpirableTxnRecord.Builder buildFailedExpiringRecord(
