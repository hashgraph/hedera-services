...
public class BackedSystemAccountsCreator implements SystemAccountsCreator {
  ...
  public void createSystemAccounts(
      FCMap<MerkleEntityId, MerkleAccount> accounts, AddressBook addressBook) {
    throw new UnsupportedOperationException();
  }

  @Override
  public void ensureSystemAccounts(
      BackingStore<AccountID, MerkleAccount> accounts, AddressBook addressBook) {
    ...
    var msg =
        String.format("Ledger float is %d tinyBars in %d accounts.", ledgerFloat, allIds.size());
    log.info(msg);
  }
                    ...
                    .setKeyList(
                        KeyList.newBuilder()
                            .addKeys(
                                Key.newBuilder()
                                    .setEd25519(
                                        ByteString.copyFrom(CommonUtils.unhex(getHexedABytes())))))
                    .build());
