...
public class ExpiryManager {
  ...
  private static final Comparator<ExpiryEvent<Pair<Long, Consumer<EntityId>>>> PQ_CMP =
      Comparator.comparingLong(ExpiryEvent<Pair<Long, Consumer<EntityId>>>::getExpiry)
  ...
  /**
   * When payer records are in state (true by default), upon restart or reconnect the expiry manager
   * needs to rebuild its expiration queue so it can correctly purge these records as their
   * lifetimes (default 180s) expire.
   *
   * <p><b>IMPORTANT:</b> As a side-effect, this method re-stages the injected {@code txnHistories}
   * map with the recent histories of the {@link TransactionID}s from records in state.
   */
  public void reviewExistingPayerRecords() {
    ...
    forEach(
        currentAccounts,
        (id, account) -> stageExpiringRecords(id.getNum(), account.records(), _payerExpiries));
    _payerExpiries.sort(
   ...
   *
   * <p>The only entities that currently qualify as "short-lived" are schedule entities, which have
   * a default expiration time of 30 minutes. So this method's only function is to scan the current
   * {@code schedules} FCM and enqueue their expiration events.
   */
  public void reviewExistingShortLivedEntities() {
    shortLivedEntityExpiries.reset();

    final var _shortLivedEntityExpiries =
        new ArrayList<Map.Entry<Pair<Long, Consumer<EntityId>>, Long>>();
    ...
    forEach(
        ...
        (id, schedule) -> {
          ...
          _shortLivedEntityExpiries.add(
              new AbstractMap.SimpleImmutableEntry<>(pair, schedule.expiry()));
        });

    _shortLivedEntityExpiries.sort(
        comparing(Map.Entry<Pair<Long, Consumer<EntityId>>, Long>::getValue)
            .thenComparing(entry -> entry.getKey().getKey()));
    _shortLivedEntityExpiries.forEach(
        entry -> shortLivedEntityExpiries.track(entry.getKey(), entry.getValue()));
  }
  ...
  private void stageExpiringRecords(
      Long num, FCQueue<ExpirableTxnRecord> records, List<Map.Entry<Long, Long>> expiries) {
    long lastAdded = -1;
