...
public class ExpirableTxnRecord implements FCQueueElement {
      ...
      var readable =
          IntStream.range(0, n)
              .mapToObj(
                  i ->
                      String.format(
                          "%s(%s)", tokens.get(i).toAbbrevString(), tokenAdjustments.get(i)))
              .collect(joining(", "));
  ...
  public void deserialize(SerializableDataInputStream in, int version) throws IOException {
    ...
    tokenAdjustments =
        in.readSerializableList(MAX_INVOLVED_TOKENS); /* Schedule references present since v0.8.0 */
  ...
  public static List<TransactionRecord> allToGrpc(List<ExpirableTxnRecord> records) {
    return records.stream().map(ExpirableTxnRecord::asGrpc).collect(toList());
  }
