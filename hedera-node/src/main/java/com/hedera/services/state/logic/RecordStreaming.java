...
/**
 * Publishes records to the Platform-managed record stream, using the {@link com.swirlds.common.stream.StreamAligned}
 * contract to maintain the invariant that a new record file (i.e., block) starts <b>only</b> at a consensus timestamp
 * that was the first assigned to a user transaction. This invariant makes it easy to further guarantee that the block
 * number increases if and only if a new record file begins.
 *
 * <p>This means that all the child records for a user transaction, whether top-level or scheduled, will be in the
 * same record file. In particular, all child records for a contract transaction will be in the same block as the record
 * of the top-level transaction.
 *
 * <p>It also means that all system-created records from a single call to {@code handleTransaction} will be in the
 * same record file. (Although this is less important at this time.)
 */
@Singleton
public class RecordStreaming {
	...
	/**
	 * Resets the block number to reflect that it has not been updated as a side effect
	 * of streaming user-generated records in this call to {@code handleTransaction}.
	 *
	 * <p>This is to handle the edge case that an internal error causes no user record
	 * to be generated for a call to {@code handleTransaction}; we need to keep using the
	 * current block number until the next call, so we preserve the invariant above that
	 * a new record file (i.e., block) starts <b>only</b> at a consensus timestamp that
	 * was the first assigned to a user transaction.
	 */
	...
	/**
	 * Streams a system record such as an auto-renewal or auto-removal record. If any
