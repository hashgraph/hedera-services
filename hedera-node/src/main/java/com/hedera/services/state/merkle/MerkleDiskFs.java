...
/**
 * Save some special system files on the local file system instead of database to improve access
 * efficiency.
 *
 * <p>All that is kept in memory is a map from {@code FileID} to the SHA-384 hash of the current
 * contents.
 *
 * <p><b>IMPORTANT:</b> If running multiple nodes in a process, all of their disk-based file systems
 * will use the same path for a given file! There is no simple way to "scope" the path by node id
 * without creating problems during reconnect.
 *
 * <p>So if testing the update feature locally, please use a single node; or use multiple nodes in a
 * Docker Compose network. Otherwise, updates to file 0.0.150 will be hopelessly interleaved and
 * failure is essentially guaranteed.
 */
public class MerkleDiskFs extends AbstractMerkleLeaf implements MerkleExternalLeaf {
  ...
  public MerkleDiskFs copy() {
    setImmutable(true);
    Map<FileID, byte[]> fileHashesCopy =
  ...
  Path pathToContentsOf(FileID fid) {
    return Paths.get(
        String.format("%sFile%s", separatorSuffixed(DISK_FS_ROOT_DIR), asLiteralString(fid)));
  }
  ...
  private void serializeFidInfo(
      SerializableDataOutputStream out, Function<FileID, byte[]> infoReprFn) {
    orderedFids()
        .forEach(
            fid -> {
              try {
