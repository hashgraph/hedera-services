...
public class QueryFeeCheck {
  ...
  ResponseCodeEnum transfersPlausibility(List<AccountAmount> transfers) {
    ...
    try {
      long net = transfers.stream().mapToLong(AccountAmount::getAmount).reduce(0L, Math::addExact);
  ...
  /**
   * Validates query payment transfer transaction before reaching consensus. Validate each payer has
   * enough balance that is needed for transfer. If one of the payer for query is also paying
