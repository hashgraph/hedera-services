...
public class SmartContractRequestHandler {
   ...
   *
   * @param transaction API request to create the contract
   * @param consensusTime Platform consensus time
   * @param contractByteCode Byte code to execute to get the contract code
   * @param sequenceNum To generate the next ContractID
  ...
  public TransactionRecord createContract(
    ...
    if (createContract.getGas() <= dynamicProperties.maxGas()) {
      ...
      log.debug(
          "Gas offered: {} reduced to maxGasLimit: {} in create",
          () -> createContract.getGas(),
          () -> dynamicProperties.maxGas());
    }
    String contractByteCodeString = new String(contractByteCode);
    if (createContract.getConstructorParameters() != null
        && !createContract.getConstructorParameters().isEmpty()) {
      final var constructorParamsHexString =
          CommonUtils.hex(createContract.getConstructorParameters().toByteArray());
    ...
    try {
      tx =
          new Transaction(
              null, biGasPrice, gas, senderAccountEthAddress, null, value, contractByteCodeString);
    ...
    try {
      ...
      result =
          getFailureTransactionRecord(transaction, consensusTime, CONTRACT_EXECUTION_EXCEPTION);
    }
    if (result.getReceipt().getStatus() == SUCCESS) {
      ...
      if (respCode != SUCCESS) {
        return TransactionRecord.newBuilder()
            ...
            .setMemo(createContract.getMemo())
            .setTransactionFee(transaction.getTransactionFee())
            .build();
      }
   ...
   *
   * @param transaction API request that caused the error
   * @param consensusTime Platform consensus time
   * @param responseCode Error code to be build into the result
  ...
  public TransactionRecord getFailureTransactionRecord(
    ...
    TransactionReceipt transactionReceipt =
        getTransactionReceipt(responseCode, exchange.activeRates());
  ...
  private ContractCallLocalResponse runPure(
    ...
    var header =
        RequestBuilder.getResponseHeader(result.getValue(), 0L, ANSWER_ONLY, ByteString.EMPTY);
  ...
  private TransactionRecord run(
    ...
    var receiptBuilder =
        RequestBuilder.getTransactionReceipt(result.getValue(), exchange.activeRates()).toBuilder();
    var recordBuilder =
  ...
  private ResponseCodeEnum createMemoAdminKey(
      TransactionBody transaction, TransactionRecord txRecord) {
  ...
  private void setParentPropertiesForChildrenContracts(
      AccountID parent, List<ContractID> children) {
    MerkleAccount parentAccount = ledger.get(parent);
    HederaAccountCustomizer customizer =
        new HederaAccountCustomizer()
            .key(parentAccount.getKey())
   ...
   *
   * @param transaction API request to execute the contract method
   * @param consensusTime Platform consensus time
   * @param sequenceNum Incrementing counter in case an entity is created
   * @return Details of contract call result
   */
  public TransactionRecord contractCall(
      TransactionBody transaction, Instant consensusTime, SequenceNumber sequenceNum) {
    Transaction tx = null;
    ...
    AccountID receiverAccount =
        AccountID.newBuilder()
            .setAccountNum(contractCall.getContractID().getContractNum())
            .setRealmNum(contractCall.getContractID().getRealmNum())
            .setShardNum(contractCall.getContractID().getShardNum())
            .build();
    ...
    if (callResponseStatus == ResponseCodeEnum.OK) {
      ...
      if (contractCall.getGas() <= dynamicProperties.maxGas()) {
        ...
        gas = BigInteger.valueOf(dynamicProperties.maxGas());
        log.debug(
            "Gas offered: {} reduced to maxGasLimit: {} in call",
            () -> contractCall.getGas(),
            () -> dynamicProperties.maxGas());
      }
      ...
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "ContractCall gas coefficient could not be found in fee schedule " + e1.getMessage());
        }
        return getFailureTransactionRecord(
            transaction, consensusTime, ResponseCodeEnum.CONTRACT_EXECUTION_EXCEPTION);
      }
      tx =
          new Transaction(
              BigInteger.ZERO,
      ...
      try {
        ...
        var record =
            run(
                ...
                false);
        setParentPropertiesForChildrenContracts(
            receiverAccount, record.getContractCallResult().getCreatedContractIDsList());
        return record;
      } catch (Exception e) {
        log.warn("Unhandled exception during EVM transaction", e);
        return getFailureTransactionRecord(
            transaction, consensusTime, CONTRACT_EXECUTION_EXCEPTION);
      }
    } else {
      com.hederahashgraph.api.proto.java.TransactionReceipt transactionReceipt =
          getTransactionReceipt(
              transaction.getContractCall().getContractID(),
              callResponseStatus,
  ...
  /**
   * Execute a smart contract local call. This does not go through consensus and may not change any
   * state.
   *
   * @param transactionContractCallLocal API request to execute the contract method
   * @param currentTimeMs Execution timestamp, not a platform consensus time
   * @return Details of local execution result
   * @throws Exception Passes through lower-level exceptions; does not generate any.
   */
  public ContractCallLocalResponse contractCallLocal(
    ...
    TransactionBody body =
        SignedTxnAccessor.uncheckedFrom(transactionContractCallLocal.getHeader().getPayment())
            .getTxn();
    AccountID senderAccount = body.getTransactionID().getAccountID();
    String senderAccountEthAddress = asSolidityAddressHex(senderAccount);
    AccountID receiverAccount =
        EntityIdUtils.asAccount(transactionContractCallLocal.getContractID());
    String receiverAccountEthAddress = asSolidityAddressHex(receiverAccount);
    ...
    if (callResponseStatus == ResponseCodeEnum.OK) {
      ...
      if (transactionContractCallLocal.getGas() <= dynamicProperties.maxGas()) {
        ...
        gas = BigInteger.valueOf(dynamicProperties.maxGas());
        log.debug(
            "Gas offered: {} reduced to maxGasLimit: {} in local call",
            () -> transactionContractCallLocal.getGas(),
            () -> dynamicProperties.maxGas());
      }
      String data = "";
      ...
      ResponseHeader responseHeader =
          RequestBuilder.getResponseHeader(callResponseStatus, 0l, ANSWER_ONLY, ByteString.EMPTY);
      responseToReturn = ContractCallLocalResponse.newBuilder().setHeader(responseHeader).build();
      if (log.isDebugEnabled()) {
        log.debug(
            "contractCallLocal  -Invalid Contract ID "
  ...
  public TransactionRecord systemDelete(TransactionBody txBody, Instant consensusTimestamp) {
    ...
    try {
      ...
      receipt =
          getTransactionReceipt(ResponseCodeEnum.FILE_SYSTEM_EXCEPTION, exchange.activeRates());
    }
  ...
  /**
   * System account undoes the deletion marker on a smart contract that has been deleted but not yet
   * removed.
   *
   ...
   *
   * @param transaction API request to delete the contract.
   * @param consensusTime Platform consensus time
   * @return Details of contract deletion result
   */
