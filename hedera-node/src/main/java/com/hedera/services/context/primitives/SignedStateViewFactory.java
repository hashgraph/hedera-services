...
public class SignedStateViewFactory {
	 ...
	 */
	public void tryToUpdateToLatestSignedChildren(
			final MutableStateChildren children
	) throws NoValidSignedStateException {
	...
	/**
	 * Returns a {@link StateView} backed by the {@link StateChildren} of the latest signed state, if available.
	 *
	...
	/**
	 * Uses the last completed signed state from platform to perform a given action. <b>IMPORTANT:</b> should be
	 * called everytime we try to get or update state children, to ensure they reflect the latest state.
	 *
	 * @param action
	 * 		what to do with the latest state
	 * @throws NoValidSignedStateException
	 */
