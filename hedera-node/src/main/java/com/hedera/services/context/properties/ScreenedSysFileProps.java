...
public class ScreenedSysFileProps implements PropertySource {
  ...
  static String MISPLACED_PROP_TPL =
  ...
  static String UNTRANSFORMABLE_PROP_TPL =
      "Value '%s' is untransformable for deprecated '%s' (%s), being ignored!";

  private static Map<String, String> STANDARDIZED_NAMES =
      Map.ofEntries(
  ...
  private static Map<String, UnaryOperator<String>> STANDARDIZED_FORMATS =
      Map.ofEntries(
          entry("defaultFeeCollectionAccount", legacy -> "" + parseAccount(legacy).getAccountNum()),
          entry(
              "accountBalanceExportPeriodMinutes", legacy -> "" + (60 * Integer.parseInt(legacy))));

  @SuppressWarnings("unchecked")
  private static Map<String, Predicate<Object>> VALUE_SCREENS =
      Map.ofEntries(
