...
public class StateView {
  ...
  static BiFunction<StateView, AccountID, List<TokenRelationship>> tokenRelsFn =
      StateView::tokenRels;
  ...
  public static final StateView EMPTY_VIEW =
      new StateView(null, null, null, null, EMPTY_UNIQ_TOKEN_VIEW_FACTORY);
  ...
  public StateView(
    ...
    final Map<String, byte[]> blobStore =
        unmodifiableMap(new FcBlobsBytesStore(MerkleOptionalBlob::new, this::storage));
  ...
  public Optional<TokenInfo> infoForToken(TokenID tokenID) {
    ...
    try {
      ...
      log.warn("Unexpected failure getting info for token {}!", readableId(tokenID), unexpected);
      return Optional.empty();
    }
  }
  ...
  public Optional<ScheduleInfo> infoForSchedule(ScheduleID scheduleID) {
    ...
    try {
      ...
      signatories.forEach(
          a -> signatoriesList.addKeys(Key.newBuilder().setEd25519(ByteString.copyFrom(a))));
      ...
      log.warn(
          "Unexpected failure getting info for schedule {}!", readableId(scheduleID), unexpected);
      return Optional.empty();
    }
  }
  ...
  public Optional<FileGetInfoResponse.FileInfo> infoForFile(FileID id) {
    int attemptsLeft =
        1 + (nodeLocalProperties == null ? 0 : nodeLocalProperties.queryBlobLookupRetries());
    while (attemptsLeft-- > 0) {
      try {
        return getFileInfo(id);
      } catch (com.swirlds.blob.BinaryObjectNotFoundException
          | com.swirlds.blob.BinaryObjectDeletedException e) {
        if (attemptsLeft > 0) {
  ...
  public Optional<List<TokenNftInfo>> infoForAccountNfts(
      @Nonnull AccountID aid, long start, long end) {
  ...
  public Optional<List<TokenNftInfo>> infosForTokenNfts(
      @Nonnull TokenID tid, long start, long end) {
    if (!tokenExists(tid)) {
      return Optional.empty();
    }
  ...
  FCOneToManyRelation<PermHashInteger, Long> treasuryNftsByType() {
    return stateChildren == null
        ? emptyFcotmr()
        : stateChildren.getUniqueOwnershipTreasuryAssociations();
  }
