...
public class ScreenedNodeFileProps implements PropertySource {
  ...
  static Map<String, String> STANDARDIZED_NAMES =
      Map.ofEntries(
  ...
  static Map<String, UnaryOperator<String>> STANDARDIZED_FORMATS =
      Map.ofEntries(
  ...
  static String DEPRECATED_PROP_TPL =
    ...
    if (STANDARDIZED_NAMES.containsKey(name)) {
      ...
      if (STANDARDIZED_FORMATS.containsKey(name)) {
        try {
          ...
          log.warn(
              String.format(
                  UNTRANSFORMABLE_PROP_TPL, value, name, reason.getClass().getSimpleName()));
        }
      }
