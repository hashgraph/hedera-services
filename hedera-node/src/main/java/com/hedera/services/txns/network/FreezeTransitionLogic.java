...
public class FreezeTransitionLogic implements TransitionLogic {
  ...
  public FreezeTransitionLogic(
      FileNumbers fileNums, LegacyFreezer delegate, TransactionContext txnCtx) {
    this.txnCtx = txnCtx;
  ...
  public ResponseCodeEnum validate(TransactionBody freezeTxn) {
    ...
    if (op.hasStartTime()) {
      ...
      if (!isValidTime(op.getStartHour(), op.getStartMin())
          || !isValidTime(op.getEndHour(), op.getEndMin())) {
        return INVALID_FREEZE_TRANSACTION_BODY;
      }
    }
