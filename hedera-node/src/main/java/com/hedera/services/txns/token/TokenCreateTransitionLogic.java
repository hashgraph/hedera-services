...
public class TokenCreateTransitionLogic implements TransitionLogic {
  ...
  private void transitionFor(TokenCreateTransactionBody op) {
    var result =
        store.createProvisionally(
            op, txnCtx.activePayer(), txnCtx.consensusTime().getEpochSecond());
  ...
  private ResponseCodeEnum autoEnableFeeCollectors(
      TokenID created, AccountID alreadyEnabledTreasury, TokenCreateTransactionBody op) {
    ...
    for (var fee : op.getCustomFeesList()) {
      ...
      if (fee.hasFixedFee() && fee.getFixedFee().hasDenominatingTokenId()) {
        collectorEnablementNeeded |=
            (0 == fee.getFixedFee().getDenominatingTokenId().getTokenNum());
  ...
  private ResponseCodeEnum autoEnableAccountForNewToken(
      AccountID id, TokenID created, TokenCreateTransactionBody op) {
  ...
  private ResponseCodeEnum validateAutoRenewAccount(final TokenCreateTransactionBody op) {
    ...
    if (op.hasAutoRenewAccount()) {
      validity =
          validator.isValidAutoRenewPeriod(op.getAutoRenewPeriod()) ? OK : INVALID_RENEWAL_PERIOD;
      return validity;
    } else {
