...
public class SignatoryUtils {
  ...
  /**
   * Attempts to update a stored {@code MerkleSchedule} with the Ed25519 keys witnessed to sign the
   * schedule in the active transaction. The result of the attempt is one of {@code (OK,
   * NO_NEW_VALID_SIGNATURES, SOME_SIGNATURES_WERE_INVALID)}; and a flag that is {@code true} if the
   * linked scheduled transaction is ready-to-execute.
   *
   * @param id the id of the schedule to receive the witnessed keys
   * @param store the store to retrieve the schedule from
   * @param validScheduleKeys if present, a list of the relevant Ed25519 keys with valid signatures
   *     on the active transaction (if absent, a linked Ed25519 key was expanded to an invalid
   *     signature)
   * @param activationHelper an information source on Ed25519 keys prerequisite to the relevant
   *     schedule
  ...
  private static boolean isReady(
      MerkleSchedule schedule, InHandleActivationHelper activationHelper) {
