...
public class TransactionPrecheck {
  ...
  private static final EnumSet<Characteristic> QUERY_PAYMENT_CHARACTERISTICS =
      EnumSet.of(
          Characteristic.MUST_BE_CRYPTO_TRANSFER, Characteristic.MUST_BE_SOLVENT_FOR_SVC_FEES);
  ...
  public Pair<TxnValidityAndFeeReq, Optional<SignedTxnAccessor>> performForTopLevel(
      Transaction signedTxn) {
    return performance(signedTxn, TOP_LEVEL_CHARACTERISTICS);
  }

  public Pair<TxnValidityAndFeeReq, Optional<SignedTxnAccessor>> performForQueryPayment(
      Transaction signedTxn) {
  ...
  private Pair<TxnValidityAndFeeReq, Optional<SignedTxnAccessor>> performance(
      Transaction signedTxn, EnumSet<Characteristic> characteristics) {
    ...
    if (structuralAssessment.getLeft().getValidity() != OK
        || structuralAssessment.getRight().isEmpty()) {
  ...
  private Pair<TxnValidityAndFeeReq, Optional<SignedTxnAccessor>> failureFor(
      TxnValidityAndFeeReq feeReqStatus) {
    return Pair.of(feeReqStatus, Optional.empty());
  }

  private ResponseCodeEnum checkSemantics(
      TxnAccessor accessor, EnumSet<Characteristic> characteristics) {
