...
public class AutoRenewCalcs {
  ...
  private Triple<Map<SubType, FeeData>, Instant, Map<SubType, FeeData>> cryptoAutoRenewPriceSeq =
      null;
  ...
  public void setCryptoAutoRenewPriceSeq(
    ...
    if (cryptoAutoRenewPriceSeq.getLeft() == null) {
      ...
      this.firstConstantCryptoAutoRenewFee =
          constantFeeFrom(cryptoAutoRenewPriceSeq.getLeft().get(SubType.DEFAULT));
      this.secondConstantCryptoAutoRenewFee =
          constantFeeFrom(cryptoAutoRenewPriceSeq.getRight().get(SubType.DEFAULT));

      this.firstServiceRbhPrice =
          cryptoAutoRenewPriceSeq.getLeft().get(SubType.DEFAULT).getServicedata().getRbh();
      this.secondServiceRbhPrice =
          cryptoAutoRenewPriceSeq.getRight().get(SubType.DEFAULT).getServicedata().getRbh();
    }
  }

  public RenewAssessment maxRenewalAndFeeFor(
      MerkleAccount expiredAccount, long reqPeriod, Instant at, ExchangeRate rate) {
    ...
    final long nominalFixed =
        isBeforeSwitch ? firstConstantCryptoAutoRenewFee : secondConstantCryptoAutoRenewFee;
    ...
    final long maxRenewableRbh =
        Math.max(1L, maxRenewableRbhGiven(fixedFee, hourlyFee, reqPeriod, balance));
