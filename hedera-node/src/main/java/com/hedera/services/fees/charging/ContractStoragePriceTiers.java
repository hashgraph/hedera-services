 ...
 * pricing, in which the base price is scaled by a factor of {@code 1 / (1 - d)} where
 * {@code d = (# K/V pairs used) / (max # K/V)}.
 */
public record ContractStoragePriceTiers(long[] usageTiers, long[] prices, int freeTierLimit,
                                        long maxTotalKvPairs, long referenceLifetime) {

  public static final long TIER_MULTIPLIER = 1_000_000L;
  public static final long THOUSANDTHS_TO_TINY = 100_000_000L / 1_000L;

  public static ContractStoragePriceTiers from(final String spec, final int freeTierLimit,
      final long maxTotalKvPairs, final long referenceLifetime) {
    ...
    final List<Pair<Long, Long>> pricedTiers = Arrays.stream(spec.split(","))
        .map(literal -> fromTwoPartDelimited(literal, "til", (price, usageTier) -> {
                  manageValidatedNonDecreasing("price", lastPrice, price);
                  manageValidatedNonDecreasing("usage tier", lastUsageTier, usageTier);
                }, ContractStoragePriceTiers::parsePrice, ContractStoragePriceTiers::parseUsageTier,
                Pair::of)
        ).toList();
    return new ContractStoragePriceTiers(
        ...
        freeTierLimit, maxTotalKvPairs, referenceLifetime);
  }
  ...
  public long priceOfPendingUsage(
    ...
    if (leftoverLifetime > 0) {
      ...
      fee = cappedAddition(fee,
          nonDegenerateDiv(cappedMultiplication(leftoverLifetime, price), referenceLifetime));
    }
  ...
  public boolean equals(Object o) {
    ...
    return freeTierLimit == that.freeTierLimit && maxTotalKvPairs == that.maxTotalKvPairs
        && referenceLifetime == that.referenceLifetime && Arrays.equals(usageTiers,
        that.usageTiers) && Arrays.equals(prices, that.prices);
  }
  ...
  public String toString() {
    return "ContractStoragePriceTiers{" +
        ...
        ", maxTotalKvPairs=" + maxTotalKvPairs +
  ...
  private static void manageValidatedNonDecreasing(final String thing, final AtomicLong last,
      final long current) {
    if (last.get() > current) {
