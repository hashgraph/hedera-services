...
public class PureRatesValidation {
  ...
  public static boolean isNormalIntradayChange(
      ExchangeRates midnightRates, ExchangeRateSet proposedRates, int limitPercent) {
  ...
  private static boolean canonicalTest(long bound, long oldC, long oldH, long newC, long newH) {
    ...
    return LongStream.of(bound, oldC, oldH, newC, newH).allMatch(i -> i > 0)
        && oC.multiply(nH).multiply(b100).subtract(nC.multiply(oH).multiply(ONE_HUNDRED)).signum()
            >= 0
        && oH.multiply(nC).multiply(b100).subtract(nH.multiply(oC).multiply(ONE_HUNDRED)).signum()
            >= 0;
  }
}
