...
public class SystemOpPolicies {
  ...
  private final EnumMap<HederaFunctionality, Function<TransactionBody, SystemOpAuthorization>>
      functionPolicies;
  ...
  private SystemOpAuthorization checkFreeze(TransactionBody txn) {
    ...
    boolean isAuthorized =
        payer == entityNums.accounts().treasury()
  ...
  private SystemOpAuthorization checkContractUpdate(TransactionBody txn) {
    ...
    return entityNums.isSystemContract(target)
        ? (canPerformNonCryptoUpdate(payerFor(txn), target.getContractNum())
            ? AUTHORIZED
            : UNAUTHORIZED)
        : UNNECESSARY;
  }
  ...
  private SystemOpAuthorization checkFileUpdate(TransactionBody txn) {
    ...
    return entityNums.isSystemFile(target)
        ? (canPerformNonCryptoUpdate(payerFor(txn), target.getFileNum())
            ? AUTHORIZED
            : UNAUTHORIZED)
        : UNNECESSARY;
  }
