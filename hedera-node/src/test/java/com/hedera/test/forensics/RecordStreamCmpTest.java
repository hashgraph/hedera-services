...
class RecordStreamCmpTest {
  ...
  void compareConsAndIssRecords() throws ConstructableRegistryException {
    ...
    System.out.println(
        " -> "
            + fromCons.size()
            + " record files from node0, "
            + fromIss.size()
            + " record files from node3 (using "
            + n
            + ")");
  ...
  private void compareTimestamps(
      List<RecordStreamObject> consRecs, List<RecordStreamObject> issRecs, int m) {
