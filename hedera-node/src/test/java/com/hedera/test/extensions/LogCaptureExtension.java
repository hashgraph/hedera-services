 ...
 * which must be a field on the test instance and having a NoLookUpExploit will just enable the NoLookUp
 * pattern for the appender in {@link LogCaptor}.
 *
...
public class LogCaptureExtension implements TestInstancePostProcessor, AfterEachCallback {
	...
	public void postProcessTestInstance(Object o, ExtensionContext extensionContext) throws Exception {
		...
		boolean isNoLookUpEnabled = false;
		...
		for (var field : testCls.getDeclaredFields()) {
			if (subject == null && isSubject(field)) {
				...
				logCaptor = field;
			} else if (isNoLookUpExploit(field)) {
				isNoLookUpEnabled = true;
			}
		}
		...
		injectCaptor(o, subject, logCaptor, isNoLookUpEnabled);
	}

	private void injectCaptor(Object test, Field subject, Field logCaptor, final boolean isNoLookUpEnabled) throws IllegalAccessException {
		logCaptor.setAccessible(true);
		injectedCaptor = new LogCaptor(LogManager.getLogger(subject.getType()), isNoLookUpEnabled);
	...
	private boolean isNoLookUpExploit(Field field) {
