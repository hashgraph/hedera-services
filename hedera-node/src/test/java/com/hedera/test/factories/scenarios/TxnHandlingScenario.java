...
public interface TxnHandlingScenario {
  ...
  default FCMap<MerkleEntityId, MerkleAccount> accounts() throws Exception {
    return newAccounts()
        ...
        .withAccount(DEFAULT_NODE_ID, newAccount().balance(0L).accountKeys(DEFAULT_PAYER_KT).get())
        .withAccount(
            DEFAULT_PAYER_ID,
            newAccount().balance(DEFAULT_PAYER_BALANCE).accountKeys(DEFAULT_PAYER_KT).get())
        .withAccount(
            MASTER_PAYER_ID,
            newAccount().balance(DEFAULT_PAYER_BALANCE).accountKeys(DEFAULT_PAYER_KT).get())
        .withAccount(
        ...
        .withAccount(
            ...
            newAccount()
                ...
                .get())
        .withAccount(
            SYS_ACCOUNT_ID, newAccount().balance(DEFAULT_BALANCE).accountKeys(SYS_ACCOUNT_KT).get())
        .withAccount(
            MISC_ACCOUNT_ID,
            newAccount().balance(DEFAULT_BALANCE).accountKeys(MISC_ACCOUNT_KT).get())
        .withAccount(
            COMPLEX_KEY_ACCOUNT_ID,
            newAccount().balance(DEFAULT_BALANCE).accountKeys(COMPLEX_KEY_ACCOUNT_KT).get())
        .withAccount(
            TOKEN_TREASURY_ID,
            newAccount().balance(DEFAULT_BALANCE).accountKeys(TOKEN_TREASURY_KT).get())
        .withAccount(
            DILIGENT_SIGNING_PAYER_ID,
            newAccount().balance(DEFAULT_BALANCE).accountKeys(DILIGENT_SIGNING_PAYER_KT).get())
        .withAccount(
  ...
  default TokenStore tokenStore() {
    ...
    var frozenToken =
        new MerkleToken(
            Long.MAX_VALUE, 100, 1, "FrozenToken", "FRZNTKN", true, false, new EntityId(1, 2, 4));
    ...
    var kycToken =
        new MerkleToken(
            Long.MAX_VALUE, 100, 1, "KycToken", "KYCTOKENNAME", false, true, new EntityId(1, 2, 4));
  ...
  Timestamp DEFAULT_EXPIRY =
      Timestamp.newBuilder().setSeconds(System.currentTimeMillis() / 1_000L + 86_400L).build();
