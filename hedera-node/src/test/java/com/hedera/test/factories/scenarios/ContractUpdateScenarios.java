...
public enum ContractUpdateScenarios implements TxnHandlingScenario {
  CONTRACT_UPDATE_EXPIRATION_ONLY_SCENARIO {
    public PlatformTxnAccessor platformTxn() throws Throwable {
      return new PlatformTxnAccessor(
          from(newSignedContractUpdate(MISC_CONTRACT_ID).newExpiration(DEFAULT_EXPIRY).get()));
    }
  },
  ...
  CONTRACT_UPDATE_WITH_NEW_ADMIN_KEY {
    public PlatformTxnAccessor platformTxn() throws Throwable {
      return new PlatformTxnAccessor(
          from(newSignedContractUpdate(MISC_CONTRACT_ID).newAdminKt(SIMPLE_NEW_ADMIN_KT).get()));
    }
  }
}
