...
class ResourceUsageSubtypeHelperTest {
  ...
  void presentValuesAreAsExpected() {
    // expect:
    assertEquals(
        TOKEN_FUNGIBLE_COMMON, subject.determineTokenType(Optional.of(TokenType.UNRECOGNIZED)));
    assertEquals(
        TOKEN_FUNGIBLE_COMMON, subject.determineTokenType(Optional.of(TokenType.FUNGIBLE_COMMON)));
    assertEquals(
