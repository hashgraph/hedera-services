...
class CodeOrderResultFactoryTest {
  ...
  void errorReportsReturnSingletons() {
    ...
    assertSame(
        CodeOrderResultFactory.MISSING_AUTORENEW_RESULT, subject.forMissingAutoRenewAccount());
    ...
    assertSame(
        CodeOrderResultFactory.UNSCHEDULABLE_TRANSACTION_RESULT, subject.forUnschedulableTxn());
    assertSame(
        CodeOrderResultFactory.MISSING_FEE_COLLECTOR_RESULT, subject.forMissingFeeCollector());
  }
}
