...
public class UniqueTokenMapAdapterTest {
	...
	void testContainsKey() {
		when(merkleMap.containsKey(EntityNumPair.fromNftId(NftId.withDefaultShardRealm(1,2)))).thenReturn(true);
		when(virtualMap.containsKey(UniqueTokenKey.from(NftId.withDefaultShardRealm(0, 3)))).thenReturn(true);
	...
	void testGet() {
		...
		assertThat(merkleResult.merkleUniqueToken()).isSameInstanceAs(merkleValue.merkleUniqueToken());

		final var virtualResult = virtualMapAdapter.get(virtualKey);
		assertThat(virtualResult).isEqualTo(virtualValue);
		assertThat(virtualResult.uniqueTokenValue()).isSameInstanceAs(virtualValue.uniqueTokenValue());
	}
