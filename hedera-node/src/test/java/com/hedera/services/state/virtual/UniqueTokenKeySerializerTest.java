...
public class UniqueTokenKeySerializerTest {
	...
	public void deserializeToUniqueTokenKey_whenValidVersion_shouldMatch() throws IOException {
		UniqueTokenKey src = new UniqueTokenKey(Long.MAX_VALUE);
		...
		assertThat(dst.getTokenNum()).isEqualTo(Long.MAX_VALUE);
	}
	...
	public void serializeUniqueTokenKey_shouldReturnExpectedBytes() throws IOException {
		ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
		UniqueTokenKeySerializer serializer = new UniqueTokenKeySerializer();
		int len = serializer.serialize(new UniqueTokenKey(Long.MAX_VALUE), new SerializableDataOutputStream(byteStream));

		assertThat(len).isEqualTo(9);
	...
	public void serializerEquals_whenCorrectDataVersion_shouldReturnTrue() throws IOException {
		...
		new UniqueTokenKey(Long.MAX_VALUE).serialize(buffer);
		buffer.rewind();
		UniqueTokenKeySerializer serializer = new UniqueTokenKeySerializer();

		assertThat(serializer.equals(buffer, UniqueTokenKey.CURRENT_VERSION, new UniqueTokenKey(Long.MAX_VALUE)))
				.isTrue();
		...
		buffer.rewind();
		assertThat(serializer.equals(buffer, UniqueTokenKey.CURRENT_VERSION, new UniqueTokenKey(10)))
				.isFalse();
	}
	...
	public void serializerEquals_whenIncorrectDataVersion_shouldThrowException() throws IOException {
		...
		Assertions.assertThrows(
				AssertionError.class,
				() -> serializer.equals(buffer, UniqueTokenKey.CURRENT_VERSION + 1, new UniqueTokenKey(Long.MAX_VALUE))
		);
	}
	...
	public void deserializeKeySize_shouldReturnExpectedSize() throws IOException {
		...
		buffer.rewind();
		UniqueTokenKeySerializer serializer = new UniqueTokenKeySerializer();
		assertThat(serializer.deserializeKeySize(buffer)).isEqualTo(9);
	}
	...
	public void serializer_estimatedSize() {
		UniqueTokenKeySerializer serializer = new UniqueTokenKeySerializer();
		assertThat(serializer.getTypicalSerializedSize()).isEqualTo(9);
	}
