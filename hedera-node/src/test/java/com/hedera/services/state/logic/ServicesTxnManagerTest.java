...
class ServicesTxnManagerTest {
  ...
  void setsFailInvalidAndWarnsOnProcessFailure() {
    ...
    inOrder
        .verify(warning)
        .accept(any(IllegalStateException.class), argThat("txn processing"::equals));
  ...
  void warnsOnFailedRecordRecreate() {
    ...
    inOrder
        .verify(warning)
        .accept(any(IllegalStateException.class), argThat("creating failure record"::equals));
    inOrder.verify(ledger).rollback();
    inOrder.verify(recordStreaming, never()).run();
  }
  ...
  void warnsOnFailedRollback() {
    ...
    inOrder
        .verify(warning)
        .accept(any(IllegalStateException.class), argThat("txn rollback"::equals));
    inOrder.verify(recordStreaming, never()).run();
  }
}
