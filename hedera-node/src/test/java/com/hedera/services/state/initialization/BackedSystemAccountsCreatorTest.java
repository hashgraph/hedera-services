...
class BackedSystemAccountsCreatorTest {
  ...
  void setup() throws DecoderException, NegativeAccountBalanceException, IllegalArgumentException {
    ...
    subject = new BackedSystemAccountsCreator(hederaNums, accountNums, properties, legacyReader);
  }
  ...
  void throwsOnNegativeBalance() {
    ...
    // expect:
    assertThrows(
        IllegalStateException.class, () -> subject.ensureSystemAccounts(backingAccounts, book));
  }
  ...
  private void givenAllPresentSpecialAccounts() {
    given(
            backingAccounts.contains(
                argThat(
                    accountID ->
                        (900 <= accountID.getAccountNum() && accountID.getAccountNum() <= 1000))))
        .willReturn(true);
  }
  ...
  private void givenMissingSpecialAccounts() {
    given(
            backingAccounts.contains(
                argThat(
                    accountID ->
                        (accountID.getAccountNum() != 900 && accountID.getAccountNum() != 1000))))
        .willReturn(true);
