...
class ServicesStatsManagerTest {
  ...
  void setup() throws Exception {
    ...
    subject =
        new ServicesStatsManager(counters, runningAvgs, miscSpeedometers, speedometers, properties);
  }
  ...
  void initsAsExpected() {
    ...
    verify(thread).start();
    verify(thread)
        .setName(String.format(ServicesStatsManager.SPEEDOMETER_UPDATE_THREAD_NAME_TPL, 123L));
    // and when:
    captor.getValue().run();
    // then:
