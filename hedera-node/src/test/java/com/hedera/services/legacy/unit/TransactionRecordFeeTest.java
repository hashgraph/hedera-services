...
public class TransactionRecordFeeTest {
	...
	public void init() {
		...
		ExchangeRateSet exchangeRateSet = ExchangeRateSet.newBuilder().setCurrentRate(currentRate).setNextRate(
				nextRate).build();
		...
		TransactionID txId = TransactionID.newBuilder().setAccountID(firstAccount).setTransactionValidStart(
				commonTimeStamp).build();
		...
		TransferList transferList = TransferList.newBuilder().addAccountAmounts(accountAmount1).addAccountAmounts(
				accountAmount2).build();
