...
class AwareProcessLogicTest {
  ...
  void setup() {
    ...
    final com.hederahashgraph.api.proto.java.Transaction signedTxn =
  ...
  void cyclesAsyncVerificationWhenAppropros() {
    ...
    verify(rationalization)
        .performFor(txnAccessor, syncVerifier, keyOrder, sigBytesFn, bodySigningFactory);
  ...
  void noVerificationCyclesWhenRationalizationNotOk() {
    ...
    verify(bodySigningFactory).resetFor(txnAccessor);
    verify(rationalization)
        .performFor(eq(txnAccessor), eq(syncVerifier), eq(keyOrder), eq(sigBytesFn), any());
  ...
  void shortCircuitsOnInvariantFailure() throws InvalidProtocolBufferException {
    ...
    given(expandHandleSpan.accessorFor(platformTxn))
        .willReturn(new PlatformTxnAccessor(platformTxn));
  ...
  void decrementsParentConsensusTimeIfCanTrigger() throws InvalidProtocolBufferException {
    ...
    given(invariantChecks.holdFor(any(), eq(consensusNow.minusNanos(1L)), eq(666L)))
        .willReturn(true);
  ...
  private void setupTriggeringTxn() {
    TransactionBody nonMockTxnBody =
        TransactionBody.newBuilder()
            .setTransactionID(TransactionID.newBuilder().setAccountID(IdUtils.asAccount("0.0.2")))
            .setScheduleSign(
                ScheduleSignTransactionBody.newBuilder()
