...
public class FeeDataLookups {
	...
	static public FeeData getConsensusUpdateTopicTransactionFeeMatrices(
			TransactionBody txBody, SigValueObj sigValObj,
			FCMap<MerkleEntityId, MerkleTopic> topicFCMap) throws Exception {
		MerkleTopic merkleTopic = topicFCMap.get(
				MerkleEntityId.fromTopicId(txBody.getConsensusUpdateTopic().getTopicID()));
	...
	public static FeeData computeUsageMetrics(
		...
		try {
			if (txBody.hasCryptoTransfer()) {
				...
				MerkleEntityId accountIDMerkleEntityId = MerkleEntityId.fromAccountId(
						txBody.getTransactionID().getAccountID());
				...
				feeMatrices = getCryptoUpdateTransactionFeeMatrices(txBody, expirationTimeStamp, sigValObj,
						existingKey);
				...
				Timestamp expirationTimeStamp = FeeCalcUtils.lookupAccountExpiry(
						MerkleEntityId.fromContractId(contractID), accountFCMap);
				...
				Timestamp expirationTimeStamp = getFileExpirationTimeStamp(fileHandler,
						txBody.getFileAppend().getFileID());
				feeMatrices = getFileAppendTransactionFeeMatrices(txBody, expirationTimeStamp, sigValObj);
			} else if (txBody.hasFileUpdate()) {
				Timestamp expirationTimeStamp = getFileExpirationTimeStamp(fileHandler,
						txBody.getFileUpdate().getFileID());
	...
	public static Timestamp getTopicExpirationTimeStamp(MerkleTopic merkleTopic) {
		try {
			...
			// In this case, the rbh charges will be zero, passing the expiration timestamp as zero, so rbh wont be
			// charged
	...
	public static Timestamp getFileExpirationTimeStamp(
		...
		try {
			...
			// In this case, the sbh charges will be zero, passing the expiration timestamp as zero, so sbh wont be
			// charged
			return Timestamp.newBuilder().setSeconds(0).build();
		}
	}
