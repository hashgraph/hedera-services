...
public class FileServiceHandler {
	...
	private ResponseCodeEnum validateExchangeRateData(AccountID accountID, byte[] fileData) {
		try {
			...
			// If is not small change, if this Transaction is signed by Account 0.0.50(Master Account) or 0.0.2
			// (Treasury), we should log a message and update Exchange Rate File; else we should log a error message
			// and return ResponseCodeEnum.EXCHANGE_RATE_CHANGE_LIMIT_EXCEEDED
			if (!isSmallChange(exchangeRateSet)) {
				if (isMasterAccount(accountID) || isTreasury(accountID)) {
					log.info(
							"Exchange Rate File Update changes the Exchange Rate by a percentage greater than " +
									"Exchange_Rate_Allowed_Percentage by {}",
							() -> EntityIdUtils.readableId(accountID));
				} else {
					log.error(
							"Invalid Exchange Rate File Update, because Account {} is not allowed to change the " +
									"Exchange Rate by a percentage greater than Exchange_Rate_Allowed_Percentage",
							() -> EntityIdUtils.readableId(accountID));
		...
		try {
			...
			if (fi.isDeleted()) {
				...
				if (fileData != null && !fileData.isEmpty()) {
					...
					if (1024 * 1024L < fileSize) {
						throw new MaxFileSizeExceeded(
								String.format("The file size %d (bytes) is greater than allowed %d (bytes) ",
										fileSize,
										1024 * 1024L));
					}
					...
					if (validateCode.equals(ResponseCodeEnum.OK) || validateCode
						...
						if (validateCode.equals(ResponseCodeEnum.OK)
							...
							// When MasterAccount updates file 0.0.112, it should also overwrite the ExchangeRateSet
							// value which is saved in HGCAppState
							...
							ApplicationPropertiesInterceptor appPropertiesInterceptor =
									new ApplicationPropertiesInterceptor();
