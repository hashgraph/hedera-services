...
class DissociatePrecompilesTest {
	...
	void dissociateTokenHappyPathWorks() {
		...
		given(accountStoreFactory.newAccountStore(
				validator, dynamicProperties, accounts, aliasManager)).willReturn(accountStore);
		...
		// and:
		verify(dissociateLogic).dissociate(account, singletonList(nonFungible));
	...
	void computeMultiDissociateTokenHappyPathWorks() {
		...
		given(sigsVerifier.hasActiveKey(accountAddr, contractAddr, contractAddr, senderAddr, aliases))
				.willReturn(true);
		given(accountStoreFactory.newAccountStore(validator, dynamicProperties, accounts, aliasManager))
		...
		// then:
		assertEquals(successResult, result);
		verify(dissociateLogic).dissociate(account, multiDissociateOp.tokenIds());
		verify(wrappedLedgers).commit();
		verify(worldUpdater).manageInProgressRecord(recordsHistorian, mockRecordBuilder, mockSynthBodyBuilder);
	}
