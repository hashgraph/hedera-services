...
class ViewExecutorTest {
  ...
  public static final AccountID treasury = EntityIdUtils.accountIdFromEvmAddress(Bytes.fromHexString("0x00000000000000000000000000000000000005cc").toArray());
  ...
  void computeGetTokenInfo() {
    ...
    given(decodingFacade.decodeGetTokenInfo(input))
  ...
  void computeGetFungibleTokenInfo() {
    ...
    final var tokenInfoWrapper = TokenInfoWrapper.forToken(fungible);
    given(decodingFacade.decodeGetFungibleTokenInfo(input))
        .willReturn(tokenInfoWrapper);
  ...
  void computeGetNonFungibleTokenInfo() {
    ...
    final var tokenInfoWrapper = TokenInfoWrapper.forToken(fungible);
    given(decodingFacade.decodeGetNonFungibleTokenInfo(input))
        .willReturn(tokenInfoWrapper);
  ...
  Bytes prerequisites(final int descriptor, final Bytes tokenAddress) {
    ...
    this.subject =
        new ViewExecutor(
            input, frame, encodingFacade, decodingFacade, viewGasCalculator, networkInfo);
    return input;
  }
