...
class AbstractUniqTokenViewTest {
  ...
  void setUp() {
    subject =
        new AbstractUniqTokenView(() -> tokens, () -> nfts, () -> nftsByType) {
          @Override
  ...
  void interpolatesTreasuryIdForWildcard() {
    ...
    final var interpolatedInfo =
        GrpcUtils.reprOf(grpcTokenId, wildcardSerial, wildcardNft, treasuryId.toGrpcAccountId());
    setupFirstMockRange();
  ...
  void throwsCmeIfIdHasNoMatchingTokenInTokenAssociations() {
    ...
    // and:
    final var desired =
        "MerkleUniqueTokenId{tokenId=0.0.6, serialNumber=1} was removed during query answering";
  ...
  private final MerkleUniqueToken someExplicitNft =
      new MerkleUniqueToken(someOwnerId, someMeta, someCreationTime);
  ...
  private final MerkleUniqueTokenId wildcardNftId =
      new MerkleUniqueTokenId(tokenId, wildcardSerial);
  private final MerkleToken someToken =
      new MerkleToken(1_234_567L, 1, 2, "THREE", "Four", true, false, treasuryId);
}
