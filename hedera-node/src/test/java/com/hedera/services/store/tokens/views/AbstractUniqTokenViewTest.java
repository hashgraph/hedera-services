...
class AbstractUniqTokenViewTest {
	...
	void setUp() {
		subject = new AbstractUniqTokenView(() -> tokens, () -> nfts, () -> nftsByType) {
			@Override
			public List<TokenNftInfo> ownedAssociations(EntityNum owner, long start, long end) {
	...
	void interpolatesTreasuryIdForWildcard() {
		...
		final var interpolatedInfo = GrpcUtils.reprOf(
				grpcTokenId, wildcardSerial, wildcardNft, treasuryId);
		setupFirstMockRange();
	...
	private final EntityNum treasuryId = EntityNum.fromLong(3);
	...
	private final MerkleToken someToken = new MerkleToken(
			1_234_567L, 1, 2,
			"THREE", "Four",
			true, false, treasuryId.toEntityId());
}
