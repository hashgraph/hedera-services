...
class AssociatePrecompileTest {
	...
	void computeAssociateTokenHappyPathWorksWithDelegateCall() {
		...
		verify(associateLogic).associate(accountMerkleId, Collections.singletonList(tokenMerkleId));
	...
	void computeMultiAssociateTokenHappyPathWorks() {
		...
		// then:
		assertEquals(successResult, result);
		verify(associateLogic).associate(accountMerkleId, multiAssociateOp.tokenIds());
		verify(wrappedLedgers).commit();
		verify(worldUpdater).manageInProgressRecord(recordsHistorian, mockRecordBuilder, mockSynthBodyBuilder);
	}
