...
class SemanticVersionsTest {
  private static final SemanticVersion FROZEN_PROTO_SEMVER =
      SemanticVersion.newBuilder()
          ...
          .setBuild("2b26be40")
          .build();
  private static final SemanticVersion FROZEN_SERVICES_SEMVER =
      SemanticVersion.newBuilder()
          .setMajor(4)
          .setMinor(2)
          .setPatch(1)
  ...
  @LoggingSubject private SemanticVersions subject;
  ...
  void canParseWithoutBuildMeta() {
    ...
    // and:
    var expected =
        SemanticVersion.newBuilder().setMajor(1).setMinor(2).setPatch(4).setPre("alpha.1").build();
  ...
  void canParseWithoutJustReqFields() {
    ...
    var literal = "1.2.4";
    // and:
    var expected = SemanticVersion.newBuilder().setMajor(1).setMinor(2).setPatch(4).build();
