...
class SerializableSemVersTest {
	...
	void comparatorPrioritizesOrderAsExpected() {
		assertTrue(SEM_VER_COMPARATOR.compare(
				semVerWith(1, 9, 9, "pre", "build"),
				semVerWith(2, 0, 0, null, null)) < 0);
		assertTrue(SEM_VER_COMPARATOR.compare(
				semVerWith(1, 0, 9, "pre", "build"),
				semVerWith(1, 9, 0, null, null)) < 0);
		assertTrue(SEM_VER_COMPARATOR.compare(
				semVerWith(1, 0, 0, "pre", "build"),
				semVerWith(1, 0, 1, null, null)) < 0);
		assertTrue(SEM_VER_COMPARATOR.compare(
				semVerWith(1, 0, 1, "pre", null),
				semVerWith(1, 0, 1, null, "build")) < 0);
		assertTrue(SEM_VER_COMPARATOR.compare(
	...
	void canOnlyCompareIfDeserializedAndToOwnType() {
		...
		final var preparedSubject = new SerializableSemVers(
