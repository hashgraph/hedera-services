...
class ExpiryThrottleTest {
    ...
    void getsExpectedBucketFromTestResource() throws JsonProcessingException {
        ...
        assertGroupContents(
                10_000L,
                List.of(ACCOUNTS_GET, NFTS_GET, TOKENS_GET, TOKEN_ASSOCIATIONS_GET), groups.get(0));
        assertGroupContents(
                1000L,
                List.of(ACCOUNTS_GET_FOR_MODIFY, NFTS_GET_FOR_MODIFY, TOKEN_ASSOCIATIONS_GET_FOR_MODIFY, ACCOUNTS_REMOVE, NFTS_REMOVE, TOKEN_ASSOCIATIONS_REMOVE ),
                groups.get(1));
        assertGroupContents(
                500L,
                List.of(STORAGE_GET),
        ...
        assertGroupContents(
                50L,
                List.of(        BLOBS_REMOVE, STORAGE_PUT, STORAGE_REMOVE ),
                groups.get(3));
      }
    ...
    void fallsBackToDefaultResourceIfProvidedIsUnavailable() {
        ...
        given(resourceLoader.readAllBytesIfPresent("expiry-throttle.json")).willReturn(getTestResource(VALID_RESOURCE_LOC));
    ...
    private void assertGroupContents(final long opsPerSec, final List<MapAccessType> ops, final ThrottleGroup<MapAccessType> group) {
