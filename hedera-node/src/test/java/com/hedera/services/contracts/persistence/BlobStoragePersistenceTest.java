...
class BlobStoragePersistenceTest {
  ...
  void delegatesExistence() {
    given(storage.containsKey(argThat((byte[] bytes) -> Arrays.equals(address, bytes))))
        .willReturn(true);
  ...
  @Test
  void delegatesGet() {
    given(storage.get(argThat((byte[] bytes) -> Arrays.equals(address, bytes))))
        .willReturn(addressStorage);
