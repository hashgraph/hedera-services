...
class AutoRenewCalcsTests {
    ...
    void warnsOnMissingAccountFeeData() {
        ...
        assertThat(
                logCaptor.warnLogs(),
                contains(
                        Matchers.startsWith("No prices known for CryptoAccountAutoRenew, will charge zero" + " fees")));
    }
    ...
    void computesExpectedUsdPriceForThreeMonthAccountRenewal() {
        ...
        final long expectedFeeInTinybars = getTinybarsFromTinyCents(activeRates, expectedFeeInTinycents);
        final long threeMonthsInSeconds = 7776000L;
        setupSuperStandardAccountWith(Long.MAX_VALUE);

        // when:
        final var maxRenewalAndFee =
                subject.assessCryptoRenewal(expiredEntity, threeMonthsInSeconds, preCutoff, activeRates, expiredEntity);
        final var percentageOfExpected = (1.0 * maxRenewalAndFee.fee()) / expectedFeeInTinybars * 100.0;
    ...
    void computesExpectedUsdPriceInFreeTierForThreeMonthContractRenewal() {
        ...
        final var postCutoffAssessment = subject.assessCryptoRenewal(
    ...
    void computesExpectedUsdPriceInNonFreeTierForThreeMonthContractRenewal() {
        final long expectedFeeInTinycents = 986561196830L; // the storage fee is added since KV pairs are > 100M
        final long expectedFeeInTinybars = getTinybarsFromTinyCents(activeRates, expectedFeeInTinycents);
        final long threeMonthsInSeconds = 7776000L;
    ...
    void returnsZeroZeroIfBalanceIsZero() {
        ...
        // when:
        final var maxRenewalAndFee =
                subject.assessCryptoRenewal(expiredEntity, 7776000L, preCutoff, activeRates, expiredEntity);
    ...
    void knowsHowToBuildCtx() {
        ...
        // given:
        final var expectedCtx = ExtantCryptoContext.newBuilder()
                .setCurrentExpiry(0L)
    ...
    private Triple<Map<SubType, FeeData>, Instant, Map<SubType, FeeData>> frozenPricesFrom(
        ...
        final var prePrices = schedules.getCurrentFeeSchedule().getTransactionFeeScheduleList().stream()
    ...
    private Map<SubType, FeeData> toPostPrices(final Map<SubType, FeeData> feeDataMap) {
        ...
        for (final FeeData feeData : feeDataMap.values()) {
            final var postPrices = feeData.toBuilder()
                    .setServicedata(feeData.getServicedata().toBuilder()
