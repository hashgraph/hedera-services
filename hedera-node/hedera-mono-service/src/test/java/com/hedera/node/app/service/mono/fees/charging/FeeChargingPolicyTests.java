...
class FeeChargingPolicyTests {
    ...
    void chargesNonServicePenaltyForUnableToCoverTotal() {
        ...
        // when:
        final ResponseCodeEnum outcome = subject.apply(fees);
    ...
    void chargesDiscountedFeesAsExpectedForDuplicate() {
        // setup:
        final ArgumentCaptor<FeeObject> captor = ArgumentCaptor.forClass(FeeObject.class);

        givenPayerWillingAndAbleForAllFees();

        // when:
        final ResponseCodeEnum outcome = subject.applyForDuplicate(fees);
    ...
    void requiresWillingToPayServiceWhenTriggeredTxn() {
        ...
        // when:
        final ResponseCodeEnum outcome = subject.applyForTriggered(fees);
