...
public enum TokenBurnScenarios implements TxnHandlingScenario {
    ...
    BURN_WITH_MISSING_TOKEN {
        @Override
        public PlatformTxnAccessor platformTxn() throws Throwable {
            return PlatformTxnAccessor.from(newSignedTokenBurn().burning(MISSING_TOKEN).get());
        }
    },
    BURN_FOR_TOKEN_WITHOUT_SUPPLY {
        @Override
        public PlatformTxnAccessor platformTxn() throws Throwable {
            return PlatformTxnAccessor.from(newSignedTokenBurn().burning(KNOWN_TOKEN_NO_SPECIAL_KEYS).get());
        }
    },
}
