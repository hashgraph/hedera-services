...
public class HapiOpCounters {
    ...
    public HapiOpCounters(
        ...
        Arrays.stream(allFunctions.get())
                .filter(function -> !ServicesStatsConfig.IGNORED_FUNCTIONS.contains(function))
                .forEach(
                        function -> {
                            receivedOpsConfig.put(
                                    ...
                                    counterConfigFor(
                                            function,
                                            ServicesStatsConfig.COUNTER_RECEIVED_NAME_TPL,
                                            ServicesStatsConfig.COUNTER_RECEIVED_DESC_TPL));
                            if (QUERY_FUNCTIONS.contains(function)) {
                                answeredQueriesConfig.put(
                                        function,
                                        counterConfigFor(
                                                function,
                                                ServicesStatsConfig.COUNTER_ANSWERED_NAME_TPL,
                                                ServicesStatsConfig.COUNTER_ANSWERED_DESC_TPL));
                            } else {
                                submittedTxnsConfig.put(
                                        function,
                                        counterConfigFor(
                                                function,
                                                ServicesStatsConfig.COUNTER_SUBMITTED_NAME_TPL,
                                                ServicesStatsConfig.COUNTER_SUBMITTED_DESC_TPL));
                                handledTxnsConfig.put(
                                        function,
                                        counterConfigFor(
                                                function,
                                                ServicesStatsConfig.COUNTER_HANDLED_NAME_TPL,
                                                ServicesStatsConfig.COUNTER_HANDLED_DESC_TPL));
                            }
                        });
    ...
    public long receivedSoFar(final HederaFunctionality op) {
        return ServicesStatsConfig.IGNORED_FUNCTIONS.contains(op) ? 0 : receivedOps.get(op).get();
    }
    ...
    public long handledSoFar(final HederaFunctionality txn) {
        return ServicesStatsConfig.IGNORED_FUNCTIONS.contains(txn) ? 0 : handledTxns.get(txn).get();
    }
    ...
    private void safeIncrement(
        ...
        if (!ServicesStatsConfig.IGNORED_FUNCTIONS.contains(function)) {
