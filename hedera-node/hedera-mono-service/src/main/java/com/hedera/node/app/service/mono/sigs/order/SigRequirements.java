...
public class SigRequirements {
    private static final Set<KeyOrderingFailure> INVALID_ACCOUNT_CODES =
            EnumSet.of(KeyOrderingFailure.MISSING_ACCOUNT, KeyOrderingFailure.IMMUTABLE_ACCOUNT);
    ...
    private <T> SigningOrderResult<T> contractUpdate(
        ...
        if (hasAutoRenewId(op)) {
            ...
            if (!autoRenewAccountResult.succeeded()) {
                return accountFailure(KeyOrderingFailure.INVALID_AUTORENEW_ACCOUNT, factory);
            }
            required = mutable(required);
            ...
            for (NftTransfer adjust : xfers.getNftTransfersList()) {
                ...
                if ((failure =
                                nftIncludeIfNecessary(
                                        ...
                                        true))
                        != KeyOrderingFailure.NONE) {
                    return (failure == KeyOrderingFailure.MISSING_TOKEN)
    ...
    private <T> SigningOrderResult<T> contractFailure(
        ...
        if (type == KeyOrderingFailure.INVALID_CONTRACT) {
            return factory.forInvalidContract();
        } else if (type == KeyOrderingFailure.IMMUTABLE_CONTRACT) {
    ...
    private <T> SigningOrderResult<T> accountFailure(
        ...
        } else if (type == KeyOrderingFailure.INVALID_AUTORENEW_ACCOUNT) {
    ...
    private <T> SigningOrderResult<T> topicFailure(
            KeyOrderingFailure type, SigningOrderResultFactory<T> factory) {
        if (type == KeyOrderingFailure.INVALID_TOPIC) {
    ...
    private <T> SigningOrderResult<T> tokenCreate(
        ...
        final var failure =
                addAccount(
                        ...
                        required,
                        linkedRefs);
        if (failure != KeyOrderingFailure.NONE) {
            return accountFailure(failure, factory);
        }
    ...
    private KeyOrderingFailure includeOwnerIfNecessary(
        ...
        if (!owner.equals(AccountID.getDefaultInstance()) && !payer.equals(owner)) {
            ...
            if (!ownerResult.succeeded()) {
                return KeyOrderingFailure.INVALID_ACCOUNT;
            }
    ...
    private KeyOrderingFailure includeIfNecessary(
        ...
        if (!payer.equals(account)) {
            ...
            if (result.succeeded()) {
                ...
                // not valid.
                if (reason == KeyOrderingFailure.IMMUTABLE_ACCOUNT && isCredit && isForHbar) {
                    return KeyOrderingFailure.NONE;
                } else if (reason == KeyOrderingFailure.MISSING_ACCOUNT
                        && autoCreationAllowed
                        && isCredit
                        && isAlias(account)) {
                    return KeyOrderingFailure.NONE;
    ...
    private KeyOrderingFailure nftIncludeIfNecessary(
        ...
        if (!payer.equals(party)) {
            ...
            if (!result.succeeded()) {
                ...
                if (reason == KeyOrderingFailure.MISSING_ACCOUNT
                        && autoCreationAllowed
                        && isAlias(party)
                        && !isSender) {
                    return KeyOrderingFailure.NONE;
