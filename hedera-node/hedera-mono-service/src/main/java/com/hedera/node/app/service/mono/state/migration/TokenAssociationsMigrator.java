...
public class TokenAssociationsMigrator {
    private static final Logger LOG = LogManager.getLogger(TokenAssociationsMigrator.class);

    /**
     * Migrate token associations from MerkleMap data structure to VirtualMap data structure.
     *
     * @param initializingState the ServicesState containing the MerkleMap to migrate.
     */
    public static void migrateFromTokenAssociationsMerkleMap(final ServicesState initializingState) {
        final var virtualMapFactory = new VirtualMapFactory();
        final var currentData = initializingState.tokenAssociations();
    ...
    private TokenAssociationsMigrator() {
