...
public class ClassificationWork {
    ...
    public ClassificationResult classify(final EntityNum candidateNum, final Instant now) {
        if (!expiryThrottle.allow(CLASSIFICATION_WORK)) {
            return ClassificationResult.COME_BACK_LATER;
        }
        ...
        if (lastClassified == null) {
            ...
            if (lastClassified.isDeleted()) {
                return isContract
                        ? ClassificationResult.DETACHED_CONTRACT_GRACE_PERIOD_OVER
                        : ClassificationResult.DETACHED_ACCOUNT_GRACE_PERIOD_OVER;
            }
            if (isContract) {
                if (!expiryThrottle.allow(CLASSIFICATION_WORK)) {
                    return ClassificationResult.COME_BACK_LATER;
            ...
            if (payer.getBalance() > 0) {
                return isContract
                        ? ClassificationResult.EXPIRED_CONTRACT_READY_TO_RENEW
                        : ClassificationResult.EXPIRED_ACCOUNT_READY_TO_RENEW;
            }
