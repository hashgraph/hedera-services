...
record Token(
    ...
    String getKeyProfile() {
        final var adminKeyOk = TokenTypesDumpUtils.jkeyPresentAndOk(adminKey);

        return getKeyDescription((c, ojk) -> {
            if (!TokenTypesDumpUtils.jkeyPresentAndOk(ojk)) return "    ";
            if (!adminKeyOk) return c + "   ";
            if (c == 'A') return "A   ";
            if (TokenTypesDumpUtils.jkeyDeepEqualsButBothNullIsFalse(ojk.get(), adminKey.get())) return c + "=A ";
            return c + "   ";
        });
    }
    ...
    String getKeyComplexity() {
        return getKeyDescription((c, ojk) -> {
            if (!TokenTypesDumpUtils.jkeyPresentAndOk(ojk)) return "   ";
            if (TokenTypesDumpUtils.jkeyIsComplex(ojk.get())) return c + "! ";
            return c + "  ";
        });
    }
