    ...
    public Pair<TxnValidityAndFeeReq, SignedTxnAccessor> assess(final Transaction signedTxn) {
        ...
        if (hasSignedTxnBytes) {
            if (hasDeprecatedBodyBytes || hasDeprecatedSigMap) {
                return PresolvencyFlaws.WELL_KNOWN_FLAWS.get(INVALID_TRANSACTION);
            }
        } else if (!hasDeprecatedBodyBytes) {
            return PresolvencyFlaws.WELL_KNOWN_FLAWS.get(INVALID_TRANSACTION_BODY);
        }

        if (signedTxn.getSerializedSize() > maxSignedTxnSize) {
            return PresolvencyFlaws.WELL_KNOWN_FLAWS.get(TRANSACTION_OVERSIZE);
        }

        try {
            final var accessor = accessorFactory.constructSpecializedAccessor(signedTxn);
            if (accessor.hasConsequentialUnknownFields()) {
                return PresolvencyFlaws.WELL_KNOWN_FLAWS.get(TRANSACTION_HAS_UNKNOWN_FIELDS);
            }
            ...
            if (hasTooManyLayers(signedTxn) || hasTooManyLayers(accessor.getTxn())) {
                return PresolvencyFlaws.WELL_KNOWN_FLAWS.get(TRANSACTION_TOO_MANY_LAYERS);
            }
