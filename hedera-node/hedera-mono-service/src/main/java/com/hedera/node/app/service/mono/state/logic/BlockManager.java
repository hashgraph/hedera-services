 ...
 * period and incrementing the block number via a call to {@link MerkleNetworkContext#finishBlock(org.hyperledger.besu.datatypes.Hash,
 * Instant)}.
 */
@Singleton
public class BlockManager {
  ...
  /**
   * Accepts the {@link RunningHash} that, <i>if</i> the corresponding user transaction is the last
   * in this 2-second period, will be the "block hash" for the current block.
   *
  ...
  /**
   * Returns the block metadata (hash, number, and timestamp) given the consensus time of the active
   * transaction and an applicable gas limit.
   *
