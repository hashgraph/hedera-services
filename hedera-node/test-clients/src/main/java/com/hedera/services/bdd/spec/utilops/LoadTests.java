...
public class LoadTests extends HapiSuite {
    private static final Logger log = LogManager.getLogger(LoadTests.class);
    ...
    public static RunLoadTests defaultLoadTest(Supplier<HapiSpecOperation[]> opSource, PerfTestLoadSettings settings) {
        return runLoadTest(opSource)
                .tps(targetTPS.isPresent() ? LoadTests::getTargetTPS : settings::getTps)
                ...
                .lasting(
                        (testDurationMinutes.isPresent() ? LoadTests::getTestDurationMinutes : settings::getMins),
                        () -> MINUTES);
    }
