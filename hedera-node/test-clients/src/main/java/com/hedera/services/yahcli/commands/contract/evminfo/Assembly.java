...
import com.hedera.services.yahcli.commands.contract.evminfo.DataPseudoOpLine.Kind;
...
public class Assembly {
    ...
    public interface Line {
        ...
        default @NonNull String formatLine() {
            final var sb = new StringBuilder(MAX_EXPECTED_LINE);
            formatLine(sb);
            return sb.toString();
        }
         ...
         * columns.
         */
        default void extendWithBlanksTo(@NonNull final StringBuilder sb, final int goalColumn) {
         ...
         * opcode.
         */
        default boolean thisLineIsA(final int opcode) {
    ...
    public Assembly(
            @NonNull final Map</*@NonNull*/ String, /*@NonNull*/ Object> metrics, @NonNull final Variant... options) {
        ...
        for (final var o : options) {
            switch (o) {
                ...
                case WITHOUT_DECODE_BEFORE_METADATA -> Variant.WITHOUT_DECODE_BEFORE_METADATA.setOptionOn();
     ...
     */
    @NonNull
    private final Map</*@NonNull*/ String, /*@NonNull*/ Object> metrics;
    ...
    enum Columns {
        ...
        Columns(final int columnModificationGroup, final int column) {
        ...
        static void adjustFieldColumns(@NonNull final Columns after, final int byWidth) {
            for (final var e : Columns.values())
     ...
     */
    public NextInstruction getNextInstruction(@NonNull final byte[] bytecode, final int codeOffset) {

        // Check end conditions
        if (codeOffset < 0) return null;
        if (codeOffset >= bytecode.length) return new NextInstruction(new DirectiveLine(DirectiveLine.Kind.END), -1);
        ...
        try {
            while (null != (nextInstruction = getNextInstruction(bytecode, currentOffset))) {
                ...
                if (atMetadataOffset.apply(nextInstruction.nextOffset()) > 0) {
                    ...
                    final boolean sanityCheck = nextInstruction.line() instanceof Code code && code.getCodeSize() > 0;
                    if (!sanityCheck)
    ...
    List<Line> elideCodeData(@NonNull final List<Line> lines, @NonNull final byte[] bytecode) {
        if (!Variant.WITHOUT_DECODE_BEFORE_METADATA.getValue()) {
            ...
            if (!lineRange.isEmpty()) {
                ...
                // IMO (very) arguable Java design decision: See "Why is Java's `AbstractList`'s
     ...
     */
    @NonNull
    public Range<Byte> locateMetadata(@NonNull final byte[] bytecode) {
    ...
    @NonNull
    public DataPseudoOpLine getBytecodeRangeAsData(
    ...
    @NonNull
    public CodeRecognizer.Results analyze(@NonNull List<Line> lines) {
