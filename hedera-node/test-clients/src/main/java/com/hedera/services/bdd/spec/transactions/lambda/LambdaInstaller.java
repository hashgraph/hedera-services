...
public class LambdaInstaller {
    ...
    private record ExplicitInitSource(@Nullable String lambda, @Nullable Bytes bytecode, @NonNull Supplier<List<LambdaStorageSlot>> slotsSupplier) {
        public LambdaExplicitInit op() {
            final var builder = LambdaExplicitInit.newBuilder()
                    .storageSlots(slotsSupplier.get());
            if (bytecode != null) {
    ...
    public LambdaInstallation op() {
        final var builder = LambdaInstallation.newBuilder()
                .type(type)
                .chargingPattern(chargingPattern);
