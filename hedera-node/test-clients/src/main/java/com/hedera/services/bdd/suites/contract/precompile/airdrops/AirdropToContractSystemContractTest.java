...
public class AirdropToContractSystemContractTest {
    ...
    class AirdropToAssociatedReceiverContract {
        ...
        public Stream<DynamicTest> airdropToContract(
                @Contract(contract = "EmptyOne") SpecContract receiverContract,
                @FungibleToken(initialSupply = 1000L) SpecFungibleToken token) {
            return hapiTest(
                    sender.associateTokens(token),
                    receiverContract.associateTokens(token),
        ...
        public Stream<DynamicTest> airdropTokensToContract(
            ...
            return hapiTest(withOpContext((spec, opLog) -> {
                ...
                allRunFor(
                        ...
                        receiverContract.associateTokens(token1, token2, token3, nft1, nft2, nft3));
                allRunFor(
                        spec,
        ...
        public Stream<DynamicTest> canAirdropTokensToContractWithSomeAssociations(
            ...
            return hapiTest(withOpContext((spec, opLog) -> {
                allRunFor(
                        spec,
                        // In order to get contract features need explicit call with it beforehand
                        receiverContract.getInfo(),
                        // Explicitly set auto association to 2
                        contractUpdate(receiverContract.name()).newMaxAutomaticAssociations(2));
                allRunFor(
                        spec,
                ...
                allRunFor(
                        ...
                        nft1.treasury().transferNFTsTo(sender, nft1, 1L),
                        nft2.treasury().transferNFTsTo(sender, nft2, 1L),
                        receiverContract.associateTokens(token1, nft1));
        ...
        public Stream<DynamicTest> canAirdropTwoTokensToContractWithNoAutoAssocSlots(
            ...
            return hapiTest(withOpContext((spec, opLog) -> {
                allRunFor(
                        ...
                        // In order to get contract features need explicit call with it beforehand
                        receiverContract.getInfo(),
                        // Explicitly set auto association to 0
                        contractUpdate(receiverContract.name()).newMaxAutomaticAssociations(0));
                allRunFor(
                        spec,
                        sender.associateTokens(token1, token2),
                        token1.treasury().transferUnitsTo(sender, 1_000L, token1),
                        token2.treasury().transferUnitsTo(sender, 1_000L, token2));
                allRunFor(spec, receiverContract.associateTokens(token1));
        ...
        public Stream<DynamicTest> airdropWithCustomFeesToContractCollector(
                ...
                @NonNull
                        ...
                        final SpecFungibleToken token) {
            return hapiTest(withOpContext((spec, opLog) -> {
                allRunFor(spec, sender.associateTokens(token), airdropContract.associateTokens(token));
                allRunFor(
                        spec,
                        receiverContract.associateTokens(token),
                        tokenUpdate(token.name()).treasury(receiverContract.name()),
                        // Initial check for receiver balance
                        receiverContract
                                .getBalance()
                                .andAssert(balance -> balance.hasTokenBalance(token.name(), 1_000_000L)),
                        cryptoTransfer(moving(1_000L, token.name()).between(receiverContract.name(), sender.name())),
                        tokenFeeScheduleUpdate(token.name())
                                .withCustom(fractionalFeeNetOfTransfers(
                                        1L, 10L, 1L, OptionalLong.of(100L), receiverContract.name())));
                allRunFor(
                        spec,
                        airdropContract
                                .call("tokenAirdrop", token, sender, receiverContract, 10L)
                        ...
                        receiverContract
                                .getBalance()
                                .andAssert(balance -> balance.hasTokenBalance(token.name(), 999_010L)));
            }));
        }
    }
}
