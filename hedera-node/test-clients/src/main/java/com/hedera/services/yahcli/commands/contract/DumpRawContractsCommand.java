...
import static com.hedera.services.yahcli.commands.contract.SignedStateHolder.getContracts;

import com.hedera.services.yahcli.commands.contract.SignedStateHolder.Contract;
import com.hedera.services.yahcli.commands.contract.SignedStateHolder.Contracts;
...
public class DumpRawContractsCommand implements Callable<Integer> {
    static final int ESTIMATED_NUMBER_OF_CONTRACTS = 1_000;
    ...
    private @NonNull Collection</*@NonNull*/ String> formatContractLines(@NonNull final Contracts contracts) {
    ...
    private @NonNull Contracts uniquifyContracts(@NonNull final Contracts contracts) {
        ...
        var contractsByBytecode = new HashMap<ByteArrayAsKey, Set<Integer>>(ESTIMATED_NUMBER_OF_CONTRACTS);
        for (var contract : contracts.contracts()) {
     ...
     */
    private @NonNull Contracts getNonTrivialContracts(@NonNull final Path inputFile, @NonNull int[] outZeroLengthCount)
            throws Exception {
