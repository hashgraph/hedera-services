...
public class MintTokenFailableCall extends AbstractFailableNonStaticCall {
    private static final Function SIGNATURE = new Function("mintToken(address,int64,bytes[])", "(int64)");

    public MintTokenFailableCall() {
    ...
    public byte[] encodedCall(@NonNull final ClassicFailureMode mode, @NonNull final HapiSpec spec) {
        ...
        return SIGNATURE
                .encodeCallWithArgs(INVALID_TOKEN_ADDRESS, 1L, new byte[0][])
                .array();
    }
}
