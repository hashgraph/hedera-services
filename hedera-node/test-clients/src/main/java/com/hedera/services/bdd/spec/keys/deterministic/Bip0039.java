...
public class Bip0039 {
    ...
    public static byte[] entropyFrom(String mnemonic) {
        ...
        boolean[] entropyBits = new boolean[264];
        int nextBit = 0;
        for (int i = 0; i < 24; i++) {
            int j = Arrays.binarySearch(wordList, words[i]);
            assert (j >= 0);
            for (int k = 10; k >= 0; k--) {
                entropyBits[nextBit + k] = (j % 2) == 1;
                j /= 2;
            }
            nextBit += 11;
        }
    ...
    public static String mnemonicFrom(byte[] entropy) throws NoSuchAlgorithmException {
        assert (entropy.length == 32);
        byte[] hash = MessageDigest.getInstance("SHA-256").digest(entropy);
        byte[] withChecksum = Arrays.copyOf(entropy, 33);
        withChecksum[32] = hash[0];
        boolean[] indexBits = new boolean[264];
        int nextBit = 0;
        for (byte b : withChecksum) {
            System.arraycopy(asBits(b), 0, indexBits, nextBit, 8);
            nextBit += 8;
        }
        ...
        for (int i = 0; i < 24; i++) {
            int index = 0;
            for (int j = 0; j <= 10; j++) {
                index |= (indexBits[i * 11 + j] ? (1 << (10 - j)) : 0);
            }
            mnemonic.append(wordList[index]).append((i < 23) ? " " : "");
        }
        return mnemonic.toString();
    }

    public static boolean[] asBits(byte b) {
