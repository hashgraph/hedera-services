...
public class RecordFileChecker extends UtilOp {
    ...
    public RecordFileChecker(
            String recordFileName, List<Transaction> transactions, TransactionRecord... transactionRecord) {
    ...
    private void lookForFile(HapiSpec spec) throws IOException {
        ...
        for (var address : addressBook.getNodeAddressList()) {
            ...
            final var actualRecordsInFile = recordStreamFile.getRecordStreamItemsList().stream()
            ...
            for (int i = 0; i < actualRecordsInFile.size(); i++) {
                ...
                Assertions.assertEquals(
                        actualRecordsInFile.get(i).getLeft(), transactions.get(i), TRANSACTION_ERROR_MESSAGE);
            }
        }
    }
