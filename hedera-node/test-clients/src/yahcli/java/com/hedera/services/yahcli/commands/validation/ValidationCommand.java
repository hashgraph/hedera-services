...
public class ValidationCommand implements Callable<Integer> {
    ...
    private String[] usuallyPersistentTokenEntities(String persistenceDir) {
        return new String[] {
            EntityManager.accountLoc(persistenceDir, yaml(PAYER)),
            EntityManager.tokenLoc(persistenceDir, yaml(TOKEN)),
            EntityManager.accountLoc(persistenceDir, yaml(TREASURY)),
            EntityManager.accountLoc(persistenceDir, yaml(RECEIVER)),
        };
    }

    private void ensurePayerBalance(Map<String, String> specConfig) {
        var persistenceDir = specConfig.get("persistentEntities.dir.path");
        var persistent = new String[] {EntityManager.accountLoc(persistenceDir, yaml(PAYER))};
        if (shouldResetEntities) {
    ...
    private String[] usuallyPersistentScheduleEntities(String persistenceDir) {
        return new String[] {
            EntityManager.accountLoc(persistenceDir, yaml(PAYER)),
            EntityManager.accountLoc(persistenceDir, yaml(SENDER)),
            EntityManager.accountLoc(persistenceDir, yaml(RECEIVER)),
            EntityManager.topicLoc(persistenceDir, yaml(TOPIC)),
            EntityManager.scheduleLoc(persistenceDir, yaml(MUTABLE_SCHEDULE)),
            EntityManager.scheduleLoc(persistenceDir, yaml(IMMUTABLE_SCHEDULE))
        };
    }
