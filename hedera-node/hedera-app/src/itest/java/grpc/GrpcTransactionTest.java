...
/**
 * This test generates and sends *real* GRPC transactions using the Google protobuf library
 * and the Helidon gRPC client. This is basically a smoke-test. Nothing is done with these
 * transactions, other than to make the call and verify that the call succeeds
 */
class GrpcTransactionTest extends GrpcTestBase {
    ...
    void setUp() throws InterruptedException, UnknownHostException {
        ...
        consensusClient = GrpcServiceClient.create(channel,
                ClientServiceDescriptor.builder(ConsensusServiceGrpc.getServiceDescriptor()).build());
        networkClient = GrpcServiceClient.create(channel,
                ClientServiceDescriptor.builder(NetworkServiceGrpc.getServiceDescriptor()).build());
        cryptoClient = GrpcServiceClient.create(channel,
                ClientServiceDescriptor.builder(CryptoServiceGrpc.getServiceDescriptor()).build());
    }
    ...
    protected void configureRouting(GrpcRouting.Builder rb) {
        final var iw = new IngestWorkflow() {
            @Override
            public void handleTransaction(@Nonnull SessionContext session, @Nonnull ByteBuffer requestBuffer, @Nonnull ByteBuffer responseBuffer) {
                if (ingestWorkflow != null) {
                    ingestWorkflow.handleTransaction(session, requestBuffer, responseBuffer);
                }
            }
        };
        ...
        final var qw = new QueryWorkflow() {
            @Override
            public void handleQuery(@Nonnull SessionContext session, @Nonnull ByteBuffer requestBuffer, @Nonnull ByteBuffer responseBuffer) {
        ...
        serviceBuilder = new GrpcServiceBuilder("proto.CryptoService", iw, qw)
                .query("getAccountRecords");
        rb.register(serviceBuilder.build(metrics));
    }
    ...
    void sendTransactionAndServerFails() {
        ...
        final var e = assertThrows(StatusRuntimeException.class,
    ...
    void sendTransactionToUnknownEndpoint() {
        // I never registered "createTopic" with the server end. The client understands it (because it is
        // part of the actual protobuf service definition), but since I didn't register it with the server,
        // I can verify the behavior should a client call a method on a service that the server doesn't know about
    ...
    void sendTransactionToUnknownService() {
        // I never registered Network service endpoints with the server end. The client understands it (because it is
        // part of the actual protobuf service definition), but since I didn't register it with the server, I can
    ...
    void sendQueryToUnknownEndpoint() {
        // I never registered "getLiveHash" with the server end. The client understands it (because it is
        // part of the actual protobuf service definition), but since I didn't register it with the server,
        // I can verify the behavior should a client call a method on a service that the server doesn't know about
