...
class KnownLengthStreamTest {
    ...
    /**
     * Given a {@link ByteBuffer} with the position set to some value (possibly 0,
     * possibly another value) and the capacity being some value (possibly 0), validate
     * that we can read each byte using the "read" method and eventually get -1.
     *
     ...
     * Given a {@link ByteBuffer} with the position set to some value (possibly 0,
     * possibly another value) and the capacity being some value (possibly 0), validate
     * that we can read each byte using the "read" method with a byte array.
     *
     * @param capacity The number of bytes in the buffer
     * @param position The position within the buffer to start reading from
     * @throws IOException should not be thrown
    ...
    void readBytes(int capacity, int position) throws IOException {
        ...
        try (final var stream = new KnownLengthStream(buf)) {
            ...
            for (int i = 0; i < numBytesToRead; i += numBytesRead) {
                ...
                assertTrue(Arrays.equals(arr, position + i, position + i + numBytesRead,
                        chunk, 0, numBytesRead));
            }
     ...
     * Given a {@link ByteBuffer} with the position set to some value (possibly 0,
     * possibly another value) and the capacity being some value (possibly 0), validate
     * that we can read each byte using the "read" method with a byte array, offset, and len.
     *
     * @param capacity The number of bytes in the buffer
     * @param position The position within the buffer to start reading from
     * @throws IOException should not be thrown
     ...
     * Given a {@link ByteBuffer} with the position set to some value (possibly 0,
     * possibly another value) and the capacity being some value (possibly 0), validate
     * that we can read each byte using the "readAllBytes".
     *
     * @param capacity The number of bytes in the buffer
     * @param position The position within the buffer to start reading from
     * @throws IOException should not be thrown
    ...
    void readAllBytes(int capacity, int position) throws IOException {
        ...
        try (final var stream = new KnownLengthStream(buf)) {
            ...
            assertTrue(Arrays.equals(arr, position, position + remaining,
                    allBytes, 0, allBytes.length));
     ...
     * Given a {@link ByteBuffer} with the position set to some value (possibly 0,
     * possibly another value) and the capacity being some value (possibly 0), validate
     * that we can read each byte using the "readNBytes".
     *
     * @param capacity The number of bytes in the buffer
     * @param position The position within the buffer to start reading from
     * @throws IOException should not be thrown
     ...
     * Given a {@link ByteBuffer} with the position set to some value (possibly 0,
     * possibly another value) and the capacity being some value (possibly 0), validate
     * that we can read each byte using the "readNBytes" method with offset, and len.
     *
     * @param capacity The number of bytes in the buffer
     * @param position The position within the buffer to start reading from
     * @throws IOException should not be thrown
