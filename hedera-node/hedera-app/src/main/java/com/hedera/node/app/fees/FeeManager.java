...
public final class FeeManager {
    ...
    public FeeData getFeeData(
        ...
        if (result == null) {
            // There is no point in adding a fee schedule entry for a privileged operation type,
            // since these are not charged fees in the first place
            if (functionality != GET_ACCOUNT_DETAILS && functionality != FREEZE) {
