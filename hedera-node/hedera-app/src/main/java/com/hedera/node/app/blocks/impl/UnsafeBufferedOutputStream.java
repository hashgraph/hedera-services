 ...
 */
public class UnsafeBufferedOutputStream extends OutputStream {
     ...
     */
    public UnsafeBufferedOutputStream(@NonNull final OutputStream outputStream, final int bufferCapacity) {
        if (bufferCapacity <= 0) {
    ...
    public UnsafeBufferedOutputStream(@NonNull final OutputStream outputStream, @NonNull final byte[] buffer) {
        this.outputStream = Objects.requireNonNull(outputStream, "outputStream must not be null");
        this.buffer = ByteBuffer.wrap(Objects.requireNonNull(buffer));
    }
     ...
     */
    @Override
    public void write(@NonNull final byte[] bytes) throws IOException {
