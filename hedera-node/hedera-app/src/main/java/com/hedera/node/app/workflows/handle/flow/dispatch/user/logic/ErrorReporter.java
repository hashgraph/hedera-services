...
import com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport;
...
public class ErrorReporter {
     ...
     */
    public com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport errorReportFor(
            @NonNull final Dispatch dispatch) {
        final var creatorError = creatorErrorIfKnown(dispatch);
        if (creatorError != null) {
            return com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport.withCreatorError(
                    dispatch.creatorInfo().accountId(), creatorError);
        } else {
            final var payer =
            ...
            if (requiresPayerSig && !isHollow(payer)) {
                ...
                if (verification.failed()) {
                    return com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport.withCreatorError(
                            dispatch.creatorInfo().accountId(), INVALID_PAYER_SIGNATURE);
                }
            }
            ...
            return switch (duplicateCheckResult) {
                ...
                case SAME_NODE -> com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport.withCreatorError(
     ...
     */
    @NonNull
    private com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport checkSolvencyOfPayer(
            final Account payer, boolean isDuplicate, final Dispatch dispatch) {
        ...
        try {
            ...
            return com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport.withPayerError(
                    creatorId, payer, e.responseCode(), true, isDuplicate);
        } catch (final InsufficientNonFeeDebitsException e) {
            return com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport.withPayerError(
                    creatorId, payer, e.responseCode(), false, isDuplicate);
        } catch (final PreCheckException e) {
            // Includes InsufficientNetworkFeeException
            return com.hedera.node.app.workflows.handle.flow.dispatch.logic.ErrorReport.withCreatorError(
                    creatorId, e.responseCode());
        }
