...
public interface ThrottleServiceModule {
    ...
    @Singleton
    @ScheduleThrottle
    static ThrottleAccumulator provideScheduleThrottleAccumulator(
            @NonNull final IntSupplier frontendThrottleSplit,
            @NonNull final ConfigProvider configProvider,
            @NonNull final Metrics metrics) {
        final var throttleMetrics = new ThrottleMetrics(metrics, SCHEDULING_THROTTLE);
        // todo check what should be the capacity split value and use config
        final IntSupplier splitter = () -> 30;
        return new ThrottleAccumulator(splitter, configProvider, SCHEDULING_THROTTLE, throttleMetrics);
    }
