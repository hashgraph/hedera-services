...
public class PreHandleWorkflow {
    ...
    private TransactionMetadata preHandle(com.swirlds.common.system.transaction.Transaction platformTx) {
        try {
            ...
            final var signedTransaction = ctx.signedParser().parseFrom(tx.getSignedTransactionBytes());
            ...
            final var accountOpt = query.get().getAccountById(txBody.getTransactionID().getAccountID());
            ...
            checker.checkSignatures(tx.getSignedTransactionBytes(), signedTransaction.getSigMap(), key);
