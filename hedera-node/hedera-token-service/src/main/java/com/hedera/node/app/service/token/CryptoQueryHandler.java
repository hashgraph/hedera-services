...
/**
 * Defines APIs for responding to queries on state. Most of these APIs are defined in "CryptoService" in the protobuf.
 * Some APIs are defined for the use of other modules, or the Hedera application. Some queries are paid, some are free.
 */
public interface CryptoQueryHandler {
    /**
     * Retrieves an {@link Account} given an {@link AccountID}. This method is not defined in "CryptoService", but
     * exists for the use of other modules, including the Hedera application.
     *
    ...
    /**
     * Returns all transactions within the persistence period of consensus time for which the given account was the
     * effective payer <b>and</b> network property {@code ledger.keepRecordsInState} was {@code true}.
     */
    ...
    /**
     * Retrieves the latest receipt for a transaction that is either awaiting consensus, or reached consensus in the
     * last 180 seconds
     */
    void getTransactionReceipts(@NonNull TransactionGetReceiptQuery query);

    /**
     * Retrieves the record of a transaction that is either awaiting consensus, or reached consensus in the last 180
     * seconds
     */
