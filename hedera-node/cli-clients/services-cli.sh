...
# This script provides a convenient wrapper for launching the services CLI plugins on top of
# platform's PCLI. (Copied from `platform-sdk/swirlds-cli/pcli.sh and overrides added.)
...
# overrides to specifically run services CLI
IGNORE_JARS=true
SERVICES_CLI_PACKAGE_NAME='com.hedera.services.cli'
PROGRAM_ARGS+=("--cli" "${SERVICES_CLI_PACKAGE_NAME}")
add_to_classpath .
JVM_ARGS+=("-Xmx40g")
...
if [[ "$IGNORE_JARS" = false ]]; then
  ...
  DEFAULT_LIB_PATH="${SCRIPT_PATH}/../../hedera-node/data/lib"
