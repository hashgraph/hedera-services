...
public class UpdateCall extends AbstractHtsCall {
    @NonNull
    final TransactionBody syntheticUpdate;
    ...
    public UpdateCall(
            ...
            @NonNull final TransactionBody syntheticUpdate,
            @NonNull final VerificationStrategy verificationStrategy,
            @NonNull final org.hyperledger.besu.datatypes.Address spender,
            @NonNull final AddressIdConverter addressIdConverter) {
        super(enhancement);
        this.syntheticUpdate = requireNonNull(syntheticUpdate);
    ...
    public @NonNull PricedResult execute() {
        ...
        final var recordBuilder = systemContractOperations()
                .dispatch(syntheticUpdate, verificationStrategy, spenderId, CryptoCreateRecordBuilder.class);
        if (recordBuilder.status() != ResponseCodeEnum.SUCCESS) {
            return gasOnly(revertResult(recordBuilder.status(), 0L));
        } else {
            final var encodedOutput = UpdateTranslator.TOKEN_UPDATE_INFO_FUNCTION
                    .getOutputs()
