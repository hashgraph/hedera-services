...
public class DeleteCall extends AbstractHtsCall {
    @Nullable
    ...
    public PricedResult execute() {
        ...
        final var recordBuilder = systemContractOperations()
                .dispatch(syntheticBurnUnits(tokenId), verificationStrategy, spenderId, TokenDeleteRecordBuilder.class);
        if (recordBuilder.status() != ResponseCodeEnum.SUCCESS) {
            return gasOnly(revertResult(recordBuilder.status(), 0L));
        } else {
            final var encodedOutput = DeleteTranslator.DELETE_TOKEN
                    .getOutputs()
    ...
    private TransactionBody syntheticBurnUnits(@NonNull final TokenID tokenId) {
