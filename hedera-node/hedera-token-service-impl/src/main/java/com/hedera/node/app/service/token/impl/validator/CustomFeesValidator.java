...
public class CustomFeesValidator {
    ...
    public Set<CustomFee> validateCreation(
        ...
        final Map<CustomFee, Boolean> fees = new HashMap<>();
            ...
            switch (fee.fee().kind()) {
                case FIXED_FEE -> {
                    ...
                    if (fixedFee.hasDenominatingTokenId()) {
                        validateExplicitTokenDenomination(tokenType, collector.accountNumber(), fixedFee.denominatingTokenId().tokenNum(), tokenRelationStore, tokenStore);
                    }
                }
                case FRACTIONAL_FEE -> {
                    validateTrue(isFungibleCommon(tokenType), CUSTOM_FRACTIONAL_FEE_ONLY_ALLOWED_FOR_FUNGIBLE_COMMON);
                ...
                case ROYALTY_FEE -> {
                    ...
                    if (fee.royaltyFee().hasFallbackFee()
                        ...
                        final var tokenNum = fee.royaltyFee().fallbackFee().denominatingTokenId().tokenNum();
                        validateExplicitTokenDenomination(tokenType, collector.accountNumber(), tokenNum, tokenRelationStore, tokenStore);
                    }
                }
            }
        }
    ...
    private void validateRoyaltyFeeForCreation(final TokenType tokenType,
        ...
        if (fee.royaltyFee().hasFallbackFee()) {
            ...
            if (fallbackFee.hasDenominatingTokenId()) {
                final var denominatingTokenNum = fallbackFee.denominatingTokenId().tokenNum();
