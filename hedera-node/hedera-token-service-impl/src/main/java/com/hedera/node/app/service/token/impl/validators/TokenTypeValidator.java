...
/**
 * Provides validation for token types and token supply type. It is used in pureChecks for token creation.
 */
public class TokenTypeValidator {
    @Inject
    public TokenTypeValidator() {}

    public ResponseCodeEnum validateType(final TokenType type, final long initialSupply, final int decimals) {
        switch (type) {
            case FUNGIBLE_COMMON -> {
                return initialSupply < 0 ? INVALID_TOKEN_INITIAL_SUPPLY : (decimals < 0 ? INVALID_TOKEN_DECIMALS : OK);
            }
    ...
    public ResponseCodeEnum validateSupplyType(final TokenSupplyType supplyType, final long maxSupply) {
        switch (supplyType) {
            case INFINITE:
