...
final class V0490TokenSchemaTest {
    ...
    void testMigrateForNonNullAcctsFs() throws NoSuchFieldException, IllegalAccessException, IOException {
        ...
        // --- NFTS
        final var keySerializerNFTs = new UniqueTokenKeySerializer();
        final var valueSerializerNFTs = new UniqueTokenValueSerializer();
        ...
        VirtualMap<UniqueTokenKey, UniqueTokenValue> nftVirtualMap = new VirtualMap<>(
                ...
                new MerkleDbDataSourceBuilder<>(
                        buildTemporaryDirectory("merkleDbNfts"), keySerializerNFTs, valueSerializerNFTs, merkleDbTableConfigNfts));
        acctFsField.set(initialModServiceTokenSchema, nftVirtualMap);
        initialModServiceTokenSchema.setNftsFromState(nftVirtualMap);

        // ---- Token Relations
        final var keySerializerTokens = new EntityNumVirtualKeySerializer();
        final var valueSerializerTokens = new OnDiskTokenRelValueSerializer();
        final var merkleDbTableConfigTokenRels = new MerkleDbTableConfig<>(
                        ...
                        keySerializerTokens,
                        (short) 1,
                        valueSerializerTokens)
                .maxNumberOfKeys(1_024);

        VirtualMap<EntityNumVirtualKey, OnDiskTokenRel> tokenRelsVirtualMap = new VirtualMap<>(
                "TokenRelsVMap",
                new MerkleDbDataSourceBuilder<>(
                        buildTemporaryDirectory("merkleDbTokenRels"), keySerializerTokens, valueSerializerTokens, merkleDbTableConfigTokenRels));
