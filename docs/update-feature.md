...
-	If the field is present, then after quiescing, each node will do the following:
     o	For clarity, we assume here that the sdk directory is at location "./sdk"
     o	Make a copy of the sdk directory
     o	If "./temp" exists, delete (recursive) everything in it, otherwise create it
     o	Copy the given Hedera file to the local hard drive as "./temp/sdk.zip"
     o	Unzip "./temp/sdk.zip"
     o	If the unzip created a directory "./temp/sdk", then copy each file such as "./temp/sdk/a/b/c.txt" to the corresponding location "./sdk/a/b/c.txt"
     o	If the unzip created a file "./temp/delete.txt", then look at each row of it, and if a row is "a/b/c.txt", then delete the file "./sdk/a/b/c.txt".  WARNING: we must first ensure that this file does not contain the string "..", because we want to sandbox our deletions to entirely be inside of "./sdk/"
     o	Delete (recursive) the "./temp/sdk" directory and the "./temp/delete.txt" file (if they exist)
     o	If the unzip create a file "./temp/exec.sh" then do an operating system call to run that as a script
...
1.	Create a zip file includes two parts
...
2.	Created the zip file on hedera network through a file update transaction to update pre-exisiting file 0.0.150
3.	Network is put on a freeze mode with the update file ID 0.0.150
4.	HGCApp launches the update script as an independent process
5.	Java process is killed by the update script
6.	The update script creates new hedera service directory HapiApp2.0-Date-HHMM
7.	The update script copies everything from previous hedera service directory to the new directory
8.	The update script copies newly extracted file from sdk directory to the new hedera service directory
9.	The update script creates new symbolic link to the new hedera service directory
10.	The update script relaunches hedera service 
